...
- **依据：** 在线探查（15 只大盘股、66 次请求）找到 21 行 `f_ann_date` 晚于 `ann_date` 的财报。有可比版本的 19 行里，18 行的数值与更早公开的版本（同键较早的一行，或调整前合并报表 `report_type=5`）不同。例：`000001.SZ` 2005 年中报 2005-08-19 公布净利润 207,486,792，2006-07-05 更正为 157,834,800。所以晚到的 `f_ann_date` 标记的是一次真实的数值修订；不算它，就是在 `ann_date` 当天给出之后才公布的数。样本不大（15 只、21 行），但信号一边倒；探查记录不在仓库里，数字照录于此。
- **落地：**
  - 同一个谓词用在九处：`is_visible_at`、列式批次（改为取修订时钟的最大值那一行）、证据库查询、面板三条 SQL、File 一处与 Tushare 两处入库过滤；
  ...
  - 文字：四份面向用户的文档、四份 API 与数据文档、四张图、台账里讲可见性与财报时钟的各行，以及 `src/` 的说明与限制条目，改为如实描述；退役守卫新增一族「a record revised after as_of stays visible」，原「只读当时可知的版本」一族保留，前提改为同日更正对的时钟。
- **剩余的有损情形：**
  - 上游通常只给现行版本。面板只存了更正后那一版的，在 `[ann_date, f_ann_date)` 内该期不可见——是缺失，不是前视。只有同键还留着较早 `f_ann_date` 那一行的，窗口内才能答出旧版（2026-08-09 的 53 只样本里，两行 `f_ann_date` 不同的重复键只有 income 633 个中的 5 个、balancesheet 1,244 个中的 5 个，这是上限）。要补上这部分，需另外入库调整前版本（`report_type=5`），属后续改进。
  - 同日只差 `update_flag` 的更正对，四个时钟逐字节相同，谓词分不开：两版从同一时刻（`max(ann_date, f_ann_date)`）起同时可见，按字段拒答。
