...
| `V2-P5-047` | ~~**`--json` 在拒绝路径上一个字节都不输出**，机器调用方只拿到一个退出码~~ **已修（`_panel_refusal_payload`，经 `_panel_fail` 一个漏斗覆盖 20 条命令）** | 技 | P5-046 | **验收只在一条命令上量到（`panel doctor`），而本仓库的枚举此前短过（有一次短了 20 条），所以先全量测量再动手**：从**活的 Typer 树**枚举出 33 条叶子命令、其中 **22 条收 `--json`**，逐条驱动进一次真正的拒绝（不是用法错误），**15 条以非零退出且 stdout 写了零字节** —— `data-check`、`factor build`、`factor run`、`jobs due`、`jobs run`、`model daily-run`、`model evaluate`、`panel build`、`panel doctor`、`portfolio construct`、`portfolio turnover-variants`、`shortlist compare`、`shortlist run`、`validation segmented`、`validation statistics`。余下七条在那次扫描里没走到拒绝，所以 15 是这个缺陷的**下界**而不是健康者的计数。**`_panel_fail` 自己的文档早就写下了这条规则然后没有实现它**：「`--json` output has to stay parseable on stdout even when the command is on its way to a non-zero exit, which is precisely when a caller most needs the structured reasons」—— 它把那句话写到 stderr，stdout 什么都没有。修法是那一个漏斗：`_PANEL_JSON` 这个 ContextVar 由 `_panel_command` 置位（不是把 `json_output` 穿过八十多个调用点 —— 那是八十个各自可以填错的地方，而两帧之下的 helper 根本无从知道），`_panel_fail` 读它，人类那句话原样留在 stderr、结构化文档进 stdout。payload 只有三个字段：`detail` 与人类读到的**是同一句**（两份措辞会漂移，那正是 `panel_view.py` 逐字共用渲染的全部理由）、`exit_code` 是进程即将退出的那个数、`status` 是 `refused` 而非 `error`（这是命令做出的裁决而不是它自身的缺陷；`internal_error` 那一行以 `exit_code` 的形式到达而不是换一个 `status`）。守卫走活的 Typer 树 + 一份写明理由的豁免名单（`doctor` 与 `migrate status`：两者都不经 `_panel_fail`，且各自本来就把整份 payload 打在 stdout 上）。**顺带记下守卫自己的一次假绿**：第一版从 `parameter.default` 读选项声明、第二版从 `OptionInfo.param_decls` 读，两次都对全部 33 条命令返回空集合而测试照样通过 —— 是那条「guard the guard」的下界断言（`>= 20`）把它抓住的；最终改为读 Typer **构建出来的** click 树上的 `Option.opts` | 27 |
| `V2-P5-048` | ~~**`test_cli_and_api_return_the_same_evidence_snapshot` 在每次跑套件时往开发者真实的 `runtime/` 里写一份证据和一次迁移备份**~~ **已修（补上 `--runtime-dir`）** | 测 | P5-013, P4-111 | **本行是本次工作在跑 `tests/integration` 时自己撞出来的**：worktree 的 `runtime/` 从零长到 4 个文件 —— `state.sqlite3`、两份 `backups/state.sqlite3.v*.bak`、以及 `evidence/part-dcaf81407e363fb937766ad0.parquet`。把 `OPENALPHA_RUNTIME_DIR` 指向探针目录后逐文件二分，写入者是 `tests/integration/test_evidence_interfaces.py` 单独一个文件（3 个文件），再定位到 `test_cli_and_api_return_the_same_evidence_snapshot` 里那次 `runner.invoke(app, ["evidence", "build", ...])` **没有 `--runtime-dir`**。**讽刺之处正是它的证据**：同一个测试往下八行的 `create_app(...)` 上挂着一段注释，写着那个默认值「is the repository's own `runtime/`, so this line initialised real storage and took a migration backup on **every run of the suite**」、「Measured: that directory held 135 files」、并自称「the last executable `create_app()` in `tests/` with no runtime directory」—— `create_app` 那一半修了，正上方 `runner.invoke` 那一半漏了，而 `V2-P5-013` 此后让 `evidence build` **开始持久化**它打印的东西，于是这一半从那天起成了写入者。打印出来的 payload 不受影响（断言比的是 CLI 文档与 API 文档，从来不关心 CLI 把副本存到哪），所以这是一次纯粹的收容修复。**顺带把这一类扫了一遍**：从活的 Typer 树取出 28 条收 `--runtime-dir` 的命令，静态扫描 `tests/` 里 108 处字面量 `runner.invoke(app, [...])` 调用，修前 11 处不带 `--runtime-dir`；逐文件实测后只有这一处真的会写（其余要么在建库前就拒绝，要么正是 `test_cli_runtime_dir_env.py` 里**故意**省略以测 `OPENALPHA_RUNTIME_DIR` 回退的四处）。**记下一次假阳性，免得再追一遍**：把 `OPENALPHA_RUNTIME_DIR` 指向探针目录重跑整套集成，仍有两个文件落进去，来自 `test_transport_hardening.py::test_openalpha_serve_does_not_announce_its_server_software` —— `openalpha serve` 没有 `--runtime-dir`，确实会开库。但那**不是**泄漏：该测试先 `monkeypatch.chdir(tmp_path)`，相对的 `./runtime` 默认值就落在它自己的沙箱里；它能碰到探针目录，只是因为一个导出的绝对 `OPENALPHA_RUNTIME_DIR` 正确地压过了相对默认值 —— 那是 `config.py` 写明的优先级而不是缺陷。已验证：不设该变量单跑该文件，真实 `runtime/` 按名字、大小、mtime 逐项比对**完全一致**。**已存在的文件没有删**，`V2-P4-111` 自己定下的规矩：「the existing backups are **not** deleted. They are the user's data and nothing here removes them」 | — |
