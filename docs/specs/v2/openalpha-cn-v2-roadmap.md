...
| `V2-P5-004` | ~~Paper Portfolio（前瞻模拟，绝不连券商）~~ **已完成** | 技 | 003 | 已交付 `backtest/paper.py`（第十三个纯 stdlib 叶子）。**「绝不连券商」是运行期强制，不是注释**：`PaperPortfolio.advance` 复用 `PortfolioBacktestRunner` 走一个已观测会话（现金/T+1/FIFO/费用/各上限全是回测那套代码），整段跑在一个 CPython 审计钩子（PEP 578）里，拒绝八个事件：`socket.__new__`、`socket.connect`/`sendto`/`sendmsg`、`subprocess.Popen`、`os.posix_spawn`、`os.exec`、`ctypes.dlopen`。**拒绝「创建」socket 是主力** —— 实测它覆盖 `V2-P4-105` 当年要潜到类图之下才拦住的三条逃逸路径，含用 `_socket.socket(fileno=fd)` 重新包裹一个 `detach()` 出来的描述符。**静态方案是先量后弃而非略过**：`openalpha_cn.backtest` 的 `__init__` 经 `replay → runtime → agents → models → models/openai_compatible.py` 到达其第 11 行的 `from urllib.request import Request, urlopen`，故 **import 任何一个 `backtest/` 模块都已把 `_socket`/`ssl`/`http.client`/`urllib.request` 装进 `sys.modules`** —— 「本模块 import 闭包无网络模块」是**假命题**；何况 `lint-imports` 读静态图，看不见运行期传进来的对象。**而那正是本守卫要拦的那一例**：`PortfolioLedger` 是 `Protocol`，带 `append` 的任何东西都满足它，一个在 `append` 里开 socket 的账本会**在它自己的 append 内**被拒。**深度是线程局部的**（与 `tests/offline_guard.py` 的进程级不同）：进程级标志会让某个请求里的 paper 会话拒掉另一个请求的行情抓取，该差异由一条并发测试单独锁住（把它改回进程级，恰好且仅有那一条变红）。**禁令对合法路径零代价**：实测一整个会话跑在真 `SQLitePortfolioLedger` 上只发 `sqlite3.connect` 与 `sqlite3.connect/handle`。**账本是必填**（回测可选）—— 没人记下来的模拟单不是模拟单；会话必须**严格前移**，且不得晚于调用方声明的 `observed_on`（该声明与 `target_weight` 同性质：本模块不持时钟，`backtest/` 下无一模块持，故它可被调用方绕过，此事写进注册簿而非藏起来）。结果上的 `execution_venue` 是 `Literal`（`V2-P5-001` 的手法）：不再说这句话的记录根本过不了校验。`KNOWN_PAPER_LIMITATIONS` 是第三十七个注册簿（7 条；表行 35 → 36、表内条目 264 → 271、总注册簿 36 → 37、总条目 334 → 341）；运行时依赖仍**九个**，`lint-imports` 仍 **8 kept / 0 broken**（`paper.py` 到岸即入两条 `backtest-studies-*` 的 source 清单）。**本行尚无独立命令/路由**：`cli.py`/`sdk.py`/`api/app.py` 由并行 agent 持有，故按约定改为上报确切改动；已证的是 paper 账本直接拿 `OpenAlphaSDK.portfolio_ledger` 写入，并从 `OpenAlphaSDK.list_portfolio_transitions()` 与 `GET /api/v1/portfolio/ledger` 两个已发布读面原样读回 | 单元：调用方账本在会话中伸手必须被具名拒绝；八个事件必须**逐个被本测试激发**而非挂名；守卫必须在会话结束后释放；守卫必须只绑当前线程（并发测试）；真会话必须不触发任何被禁事件。集成：paper 写入必须在 SDK 与 REST 两个读面上读回 | S57, D19 |
...
| `V2-P5-007` | ~~多重检验控制（BH）+ 记录被检验假设数~~ **已完成** | 技 | 006 | 已交付 `backtest/multiple_testing.py`（纯 stdlib 叶子；`math` + dataclasses + pydantic，**一个 `openalpha_cn` 导入都没有**），两条 `backtest-studies-*` 契约的 source 各加一行。**两半里被忘掉的是第二半，所以它是契约强制的**：`family_size` 在 request 与 report 上都是**必填且存下来的字段**，永不由 `len(tests)` 推出——同一对 p 值 `(0.0625, 0.375)` 在声明家族为 2 时是 q 值 `(0.125, 0.375)`、两个发现，在声明家族为 8 时是 `(0.5, 1.0)`、一个发现，数据一个字节没动。**闭式对照四臂，全部二进分数、全部 `==`**：`FAMILY_*`（家族大小决定答案，数行数的实现在第二臂上死）、`STEP_UP_*`（只有第三名过自己的线，BH 仍拒绝全部三个；逐行比较的实现只给一个）、`BOUNDARY_*`（四个 p 值**恰等于**自己的临界值 `rank/16`，`<=` 全拒、`<` 一个不拒）、`DEPENDENCE_*`（`H_2 = 1.5` 精确，同一族两种声明两个答案）。**可检查的方向只有一个而且被检查了**：`family_size < len(tests)` 在契约上拒绝并同时点出两个数；反方向不可检查，`the_family_size_is_declared_and_no_check_can_confirm_it` 直说而不是暗示有个检查。**截断的那一半有精确保证**：被观测行在整族里的秩不小于它在报出行里的秩，BH 的线随秩上升，故报出秩的拒绝集是整族拒绝集的**子集**、q 值是上界——保守，不是错，且由 `test_a_withheld_hypothesis_only_ever_shrinks_the_rejection_set` 驱动为**真子集**。**依赖假设是输入不是标签**：`dependence` 无默认值（宽松的读法不该同时最省事），`arbitrary` 走 Benjamini–Yekutieli 的 `H_m` 惩罚，改的是算术。`KNOWN_MULTIPLE_TESTING_LIMITATIONS` 是第 37 个注册簿（6 条）。产品面与 `008` 同一条命令，见下行。**驱动**：单元：同一批 p 值在两个声明家族下必须给两个拒绝集；恰压线的 p 值必须被拒；只有第三名过线时三个都要被拒；`arbitrary` 必须改变答案 |  | S63, D20 |
| `V2-P5-008` | ~~gross/net 并列 + **cost drag 单列** + 置信区间 + 样本数~~ **已完成** | 技 | 007 | 已交付 `backtest/outcome_statistics.py` + `openalpha validation statistics` + `OpenAlphaSDK.outcome_statistics()`（新 `validation` 子 app）。一个 signal = 一个同期群 = 一个假设，行取自已经落库的 `ValidationResult`。**五列，每列各自 `math.fsum` 求均值、没有一列是从另两列推出来的**——推导出来的列永远和父列一致、因此永远测不出东西，那正是 `V2-P5-005` 从归因里拿掉的自由变量。第五列 `unexplained_return` 不在本行要求里，加它是因为不加就会把 `V2-P5-006` 的缺陷（残差算了又在出口丢掉）在高一层原样重演。**区间说得出它假设了什么**：ADR-0003 排除了 t 分位数，故发的是百分位 bootstrap，`method`/`confidence_level`/`bootstrap_samples`/`random_seed`/`distinct_bootstrap_means` 全印在区间上，且**逐位复用 `backtest/event_study.py` 的百分位约定**（由一条要求两面端点 `==` 的测试钉住）。**少于两个观测就不给区间也不给 p 值**：n=1 时每次重采样都是那个样本，任何置信水平下 `lower == upper`，发出去就是 20/30/50 的统计学版本；该行照样有五列和样本数，`absence_reason` 说明原因，并且**不进入家族**。p 值是符号翻转随机化检验（n ≤ 12 穷举全部 `2**n` 种符号，故闭式对照能用 `==` 断言 `2/8` 与 `6/8`；超过则用 Phipson–Smyth 的 `(1+hits)/(1+draws)`，不可能报 0）。`KNOWN_OUTCOME_STATISTICS_LIMITATIONS` 是第 38 个注册簿（7 条），两处审计同步（36→38 / 334→347 / 35→37 / 264→277）。运行时依赖仍**九个**，`lint-imports` 仍 **8 kept / 0 broken**。**变异扫描**（先证基线 72 passed 再开跑）：**38 个变异体、38 个被杀**——其中四个存活体是被这次扫描找出来的真缺口，见下。**驱动**：单元 + 集成（CliRunner/SDK，真 migrations、真 `ResearchEngine`、真 `SQLiteValidationStore`）：gross+drag 必须逐位等于 net；`--family-size 2` 与 `8` 必须给两个判决而五列一个不动；n=1 必须具名缺席且不入家族 |  | S61, S62 |
| `V2-P5-009` | ~~分段报告（行业/市值/流动性/市场状态）+ 多市场状态 walk-forward + 基准对照（**等权基线**、naive factor、v1 基线三者并列）~~ **已完成** | 技 | 008 | 已交付 backtest/segmented_reporting.py（第 40 个 KNOWN_* 注册表，7 条）。**四条轴全部是声明输入**：ValidationResult 只带 signal_id/decision_id，不带任何证券标识，所以行业与市值、流动性一样无从 join——不是数据缺失（daily_prices 有 total_mv/circ_mv/turnover_rate），是被聚合的记录不说这是哪只票；市场状态更必须由调用方定义，本模块不内置任何分类器或阈值。**N 条轴的全部桶 + 基准 + 配对差共用一个 family**，而不是每轴一个——切三刀就是多测十几个假设，分家族等于用一次的代价换几次显著。每桶附 can_ever_reject：`2**(1-n)` 的可达 p 值下界 对 家族最宽临界值，把「样本量根本不够」与「确实没有 alpha」分开。regime 覆盖按可检验桶数实测，单一状态的 walk-forward 直说 spans_multiple_regimes=false。基准按观测窗多重集配对后发布配对差，配不上就具名缺席 | 单元：28 项（tests/unit/backtest/test_segmented_reporting.py），变异 12 杀 11（第 12 个模式已随死代码删除）；集成：13 项全部经 CliRunner 与 OpenAlphaSDK 走真实 store（tests/integration/test_segmented_reporting_interfaces.py） | S59, S60, S64 |
| `V2-P5-010` | 调度原语：持久作业表（next-fire-time + lease/lock + 按交易日幂等键 + catch-up 政策 + 日历依赖 + 崩溃恢复） | 技 | P0B-004 | **原语已建成，但尚无任何调用方，故本行只关一半、另一半明写在此**。三个新模块：`job_contracts.py`（耐久形状，理由与 `batch_contracts.py` 逐字相同 —— `storage.jobs` 要持久化它而 `storage-no-upward-deps` 禁止向上）、`storage/jobs.py`（`SQLiteJobStore`）、`scheduler.py`（`TradingDayScheduler`）。**零新增运行时依赖**，ADR-0003 的九个不动；锁是 `BEGIN IMMEDIATE` 而不是 broker。**幂等键就是主键**（`job_id@YYYY-MM-DD`），同一交易日的第二次开跑是 SQLite 的 `IntegrityError`，而不是两个进程都能赢的 `SELECT`-then-`INSERT` 竞态。**崩溃恢复即租约过期**，不设清扫器 —— 清扫器自己也需要被调度；`claim()` 对过期租约与无租约一视同仁。**`due()` 刻意不读 `next_fire_time`**：存下来的触发时刻是从一个会变的日历派生出来的，交易所事后宣布一个假期就让它变成一句无人复核的旧话；真正的判据是问 `panel_ingest.newest_published_session`（拥有 16:30 `DAILY_AVAILABILITY_TIME` 那条规则的唯一函数）再与 `last_fired_session` 比对，该列只作轮询索引、每次推进都重算。**面板面新增唯一一个函数 `session_publication_instant`**：`_sessions_published_through` 的逆，就放在它旁边、读同一个常量 —— `V2-P4-063` 发现该规则被复述三处且两处不一致，`V2-P4-114` 一行之后又发现第四处，调度器自己写 `time(16, 30)` 就是第五处；两者由一年 17,520 个半小时刻度的往返恒等式互钉，而不是钉在字面量上。**建造过程中的一条实测，并且它决定了形状**：**全新** `state.sqlite3` 上 `create_app()` 停在 `schema_version: 2` —— 迁移 3（`demo_add_runs_archived_at`）因 `runs` 尚不存在而抛 `MigrationNotYetApplicable`，`run_migrations` 就此 `break`，**迁移 4 到 8 在新库上从不执行**；若把这两张表写成第九个迁移，它同样永远不会跑。故用 `CREATE TABLE IF NOT EXISTS`，这也正是 `_baseline_apply` 的 docstring 早就写下的规则。~~**未做且明说**：三个模块没有 CLI 命令、没有 REST 路由、不在 `build_storage` 里，出货产品中没有任何东西调用它们，故本行不作任何产品面主张，需要后续行给它一个面~~ **`V2-P5-013` 就是那一行，三处全部关闭**：`SQLiteJobStore` 成为 `build_storage` 的第十三个 store；`openalpha jobs register\|list\|due\|run` 是操作面，`run` 抢租约后对每个欠下的会话在**该会话自己的发布时刻**跑一次点位健康报告并按主键记账；`GET /api/v1/jobs`／`/{job_id}` 是只读半边（本服务至今零认证，声明计划与抢租约留在持有 runtime 目录的那台机器上）。**只出一种作业体而不是一套作业词表，这是实测结论**：本 build 其它每个按会话的动作都要 8–20 个声明参数，`scheduled_jobs` 没有列能装下它们，加一列就是改已落盘契约（AGENTS.md 规则 3 只在已关闭的 `V2-P4-001` 窗口里允许）；它也是唯一一个**不碰网络**的按会话动作。**`due()` 不读存下的触发时刻，现在这条从命令行可证**：测试往行里写一个一年后的 `next_fire_time`，命令欠下的会话一个不差。**追补在第一个失败的会话处停下**：失败不推进`last_fired_session`，但同一轮里后一个成功会把水位线推过它——周一失败、周三成功的日度摄入会自称「已到周三」而周一的洞还在。**顺带修掉一个从建成之日就在的死路**：`finish_session` 的 docstring 一直点名 `retry_session`，而 `SQLiteJobStore` 上**根本没有这个方法**；失败的会话既永远被 `due()` 欠着、又因主键被 `start_session` 永远拒绝，于是该会话及其之后的每一个会话都再也到不了。没人撞上是因为 `tests/unit/` 之外没有任何调用方，而 `openalpha jobs run` 在一个普通的周二就会撞上 | 单元：两进程不可同持一租约；过期租约被回收且未到期不被抢；同一交易日第二次开跑被主键拒；跨假期的 catch-up 数的是会话不是自然日；存下的旧触发时刻不得左右答案；比日历更早的 `last_fired` 必须具名拒绝而非只答它看得见的部分 | S5, S67, D22 |
...
| `V2-P5-013` | ~~SDK/CLI 补齐 REST 缺口~~ **已按实测关闭（差集变成一条相等断言，不再是一段散文）** | 产 | P4-021 | **先量后信，本行四条声明有两条在 `2746663` 上被实测证伪**：①「`OutcomeValidator` 在 SDK 中完全缺失」——**假**，`OpenAlphaSDK.validate_outcome`／`.list_validations_by_decision`／`.list_validations_by_signal` 三个方法都在，且 `sdk.py:40` 就 import 了 `backtest.validation`；②「CLI 只覆盖 20 个能力域中的 4 个」——**假**，实测 25 条命令、覆盖 44 条出货路由中 **14** 条的能力（改后 47 条中的 17 条）。**两条成立**：`F30` 的 REST-only 清单成立（但它自己写「7 项」，逐条数是 **9 项**），`F31` 成立且已修。**实测面数**：改前 **44 路由 / 48 SDK 方法 / 25 CLI 命令**，改后 **47 / 48 / 29**。**三件已关闭的产品面**：(a) `POST /api/v1/portfolio/construct` —— `V2-P5-001` 出了 CLI 与 SDK 却没有路由，本行补上，正文只有三行（`held_shortlist`→`construct_portfolio`→`construction_view`，与另外两面同一组函数）；请求体的 `policy` 字段**就是 `PortfolioConstructionPolicy` 本身**而不是路由重新拼装的一堆数字，所以三个面读的是 pydantic 的同一份校验。按 `V2-P4-101` 的标准而非子串检查钉住：`200` 正文与 `--json` **逐字节相等**，两条拒绝与 CLI 打在 stderr 的那句**逐字相等**。路径用 `/portfolio/` 而非 `/portfolios/`：`portfolio/execute`／`portfolio/ledger` 已经把这个名词写成单数，同一个 API 上同一个名词两种拼法正是本行要关的漂移。(b) `openalpha jobs register\|list\|due\|run` + `GET /api/v1/jobs`／`/{job_id}` + `build_storage` 第十三个 store —— `F98` 的调用方半边，详见 `V2-P5-010` 行。(c) `openalpha evidence build --runtime-dir` 落库，关 `F31`。**真正的交付物是 `tests/unit/test_surface_parity.py`**：47 条路由逐条写明另外两面的对应方法／命令，或写明 `None` 与理由；`SDK_ONLY`／`CLI_ONLY` 反向同理；三者都是对**活的** app、`OpenAlphaSDK` 与 Typer 树的**相等**断言。只在一个面上新增的路由会红并点名该路由；写错的方法名会红并点名；缺口关闭而理由没删也会红。五个计数按 `REGISTRY_ENTRY_COUNTS` 的办法钉住。**这才是本行的教训**：那四条声明是**静默**过期的，因为全库没有任何东西在检查它们，补救是一条测试而不是一段更好的散文。**仍未关闭并已具名**：`WITHOUT_SDK_REASONS` 里 11 条（`F30` 的 9 条 + 两条新 jobs 路由），其中 `market/events`／`themes` 的过滤逻辑内联在路由函数里，没有库级可调用物给 SDK 方法转发，补它是 `api/app.py` 的重构而不是本行做的接线。**跨行接住的一条 500**：`POST /api/v1/portfolio/execute` 与 `POST /api/v1/backtests/portfolio` 都往 `SQLitePortfolioLedger` 追加，而该 store 在 `order_id` 被换内容重用时抛裸 `ValueError`（`storage/portfolio.py:44`），两条路由都没接住 —— 重交一次订单或一次回测就是 `500 text/plain`。两条现在都答 `422` 并**逐字相等**。**诊断在实测下改过**：这条是作为「`V2-P5-003` 引入的回归」转来的，但在 `2746663`（该行的任何代码都还不存在）上实测，`500` 本来就在，`V2-P5-003` 只是给一个已有的坑多修了一条路，它的严格升序校验合并后会落进同一个 `except`。**没人发现的原因单独值得点名**：改动前 `grep -rn "backtests/portfolio" tests/` 只有**一行**命中，而且是路由表里的一格 —— runner 有库级测试，路由一个都没有。**捕获按类型与位置都收窄**：`PortfolioSimulator` 对市场事实的每一次不同意都是**返回** `status: "rejected"` 的 `200` 且照样入账，单笔那条只包住入账那一步；另有一条对照测试驱动「卖掉账上没有的股票」并要求它保持 `200`，防止以后有人把捕获扩成「所有不顺利的答案都是坏请求」。**并行 agent 随补丁一起给的 paper 组合面（`OpenAlphaSDK.advance_paper_portfolio` + `POST /api/v1/portfolio/paper/advance`）本行不收**，理由是实测的：它 import `openalpha_cn.backtest.paper`，而该模块在本 worktree 不存在（它在 `V2-P5-003`/`004` 的分支上），所以在这里它连 import 都过不去，更谈不上按 `V2-P4-101` 的标准去实测两个面逐字节相等 —— 一个我无法运行的面不该由我声称已交付。等那条分支合并后由它自己或后续行给面。**`F33`「默认值分叉」也逐条量过**：`limits` 那半条**不成立**（`PortfolioSimulator` 写着 `limits or PortfolioLimits()`，两个签名构造同一个 simulator）；`criteria` 那半条**成立**，且按本仓库自己的规则该改的是 **REST** 一侧（`ShortlistRunApiRequest` 的 docstring 已写明「a browser that omitted ... would otherwise get a bar nobody chose」）——但删掉默认值会让此前被接受的请求体开始 `422`，是对出货路由的破坏性改动，而消费它的 `web/` 此刻由并行 agent 持有，故**本行不单方面做**，转出 | 单元：`tests/unit/test_surface_parity.py`；集成：`test_portfolio_construction_interfaces.py`（三面同一构造，逐字节）、`test_scheduled_job_faces.py`（11 条，全部经 `CliRunner`／`TestClient`）、`test_evidence_interfaces.py`（落库经第二个面读回） | S83, S84 |
| `V2-P5-014` | ~~前端路由 + 数据层~~ **已完成，与 `015`/`016` 同落。两个库分开作答：React Router 取用，TanStack Query 不取。**<br><br>**① 上一版留下的"路由测试分不开两个答案"已被本次同落解除，并且是实测解除的。** 三个页面在场时，"只渲染地址所指的那一个"用**排除法**可断言（断言另外两个页面的标题**不在**屏幕上），而这正是"应用渲染了"通不过的那条。先写了一个把三个页面全部堆叠、完全不做路由的 shell 作为探针：**9 条路由测试中失败 7 条**，含全部三条 `renders only …` 与 `carries the shortlist id from the URL into the request`（后者证明路径参数是承重的 —— 一个把 fixture 写死或忽略 `useParams` 的详情页在"详情页渲染了"上是绿的）。**React Router 取用**的另一半理由是 PRD **决策 24 明文**要求四个路由区域，属产品需求而非偏好；且服务端的内容地址天然就是 URL（`shortlist_id` 是对成品答案体的摘要），路由把它变成可收藏、可转发的地址。代价实测：包体 **205.74 kB → 245.87 kB**（gzip 64.93 → 78.58），**传递依赖仅 `cookie-es` 一个**，`pnpm audit --audit-level high` 的高危条数**加它之前之后都是 3**（见下）。<br><br>**② TanStack Query 不取，四条实测理由。** **(a) 去重价值实测为零** —— `client.ts` 六个函数各**恰好一个**调用点，没有任何端点被两个组件取用，也就没有可合并的并发请求，而去重正是它的头号卖点。**(b) 状态模型只覆盖 9 分之 3** —— 它能产出 `loading`/`ready`/`failed`；`empty`/`degraded`/`stale`/`blocked` 四个由 `contractState.ts` 从**契约 payload 字段**导出（`checks_waived`、`cross_checks[].ran`、`is_blocked`、`admitted === null`），取数库结构上看不见；`succeeded` 需要 `useMutation`，即第二套状态模型；且 **v5 已删除 `idle` 状态**（`enabled: false` 时 `status` 就是 `pending`），恰好抹掉 `panelState.ts` 特意保留不并的"还没问"与"正在问"之别。**(c) `stale` 一词在两边语义相反** —— 它的 `isStale` 意为"缓存过了 `staleTime`，该去重取"，且**在此期间照常把 `data` 当作普通成功渲染**；本仓的 `stale` 意为"这份数据答的是旧问题，界面必须标注"。同一个词，一个是缓存提示、一个是渲染义务，放进同一个代码库是可预期的混淆。**(d) 它要加的那层缓存，服务端已经有了而且是内容寻址的** —— `POST /api/v1/shortlists/run` 把答案按 `shortlist_id` 存下，`GET /api/v1/shortlists/{id}` 原样奉还（源码原话："the body is the stored answer and not a re-run"），客户端缓存只会是这份存储的一个更弱的副本；而**页面①上缓存新鲜度报告，等于缓存"这份数据有多新"的答案**，是唯一一个缓存结果会自我否定的查询。包体代价 +33.19 kB（gzip +9.61），累计较基线 +35.6%。<br><br>**③ 本行原文两处经实测更正。** ⓐ "只消费 6/28 条路由" —— 分母已过期，实测 `api/app.py` 上 `application.{get,post,put,delete}("/…")` 共 **47** 条；分子由 6 增至 **9**（+`panel/health`、`shortlists`、`shortlists/{id}`），即 9/47。ⓑ **`pnpm audit --audit-level high` 并非一道正在通过的闸门** —— 在**未加任何依赖的基线 `3d3f8c6`** 上，CI 的原样命令即 **exit 1**（8 项：5 moderate + 3 high）。三条高危全部经 `jsdom→undici`、`eslint→…→brace-expansion`、`vite→postcss→nanoid` 这些**开发依赖**进入，与 `react-router` 无关（已逐条比对解析路径），也都进不了产物。故本行的依赖决策不受其影响，但该闸门本身是红的，已另行提出。<br><br>**④ 另发现一处会让本行的成果在生产上失效的缺口，已作为阻塞依赖提出而未自行修改（属 Python 面）**：`api/app.py` 用 `StaticFiles(directory=web_dir, html=True)` 挂载前端，而 Starlette 的 `html=True` 只对**目录**请求回落到 `index.html`，未命中路径一律 404。用真 `create_app(web_dir=web/dist)` 实测：`/` → 200 且是 SPA 外壳，`/data-health`、`/shortlists`、`/shortlists/sl_abc` **全部 404**。即本行新增的可收藏地址在 `vite dev` 下成立、在 `openalpha serve` 下不成立；前端 e2e 跑的是 dev server，结构上看不见这一点，故该测量写进了 `web/e2e/routing.spec.ts` 的文件头 | 产 | — | **【以下为落地前的论证，保留存证；其提出的每一问已由左栏以实测作答，勿再当作未决】** **先回答「本仓该不该拿这两个依赖」（`V2-P4-015` 立下的规矩：本行点名了库，「这次用不上」不是可用的答案）。① 治理面实测：npm 依赖在本仓是**按数量不受治理**的 —— ADR-0003 通篇**零次**提到前端（唯一的 grep 命中是 "chain **react**ion"），它约束的是 `pyproject.toml` 的 `[project].dependencies`，`test_repository_assets.py` 的 `RUNTIME_DEPENDENCIES`/`NUMERICAL_STACK_PACKAGES` 三条断言也只读 `pyproject.toml`；全库**没有任何**测试去数、去钉或去限 `web/package.json` 的依赖表。PRD §1.1 那行「前端运行时依赖 = 仅 `react` + `react-dom`」在标题为**实测基线**、带**验证方式**列的表里，是 2026-07-29 的一次**测量**而非规则 —— 同表相邻那行「后端依赖 `duckdb` `fastapi` `pydantic` `pytz` `typer` `tzdata` `uvicorn`」今天已经**过时**（实际九个），而没有任何东西因此变红，这本身就证明该表是描述性的。② **受治理的是另外两件事**，加依赖必须同时满足：`pnpm install --frozen-lockfile`（CI 与 `test_quality_workflow_covers_supported_platforms_and_locked_dependencies` 都钉了）与 `pnpm audit --audit-level high`（CI `web` job；`pnpm-workspace.yaml` 的 `brace-expansion: 5.0.8` override 就是这条闸门留下的先例）。`scripts/verify_publication.py` 不扫 npm 许可。③ **所以没有任何东西禁止这两个库**，但本行**不宜单独落地**，理由是可测量的而非口味：本行自陈的指标是「只消费 6/28 条路由」，而路由器本身**一条路由也不会多消费** —— 让这个数字动的是 `015`–`018` 四个页面行；本行唯一已存在的下游 `019` 已在**没有**这两个库的情况下交付完毕，即路线图上 `019 dep 014` 这条边被实测证伪；且本仓要求 TDD，而单页应用里的路由测试**分不开两个答案**（「路由生效」与「应用渲染了」在只有一条路由时是同一件事），`App.test.tsx` 现在就已经断言了后者。④ **分开回答两个库**：**React Router 该拿** —— 四个页面各有可收藏、可分享的 URL 是研究终端的真实需求，但应与 `015`/`016` 同一次落地，那时才有导航行为可驱动测试；**TanStack Query 未被论证** —— 它的价值（缓存、去重、后台重取、stale-while-revalidate）是对**远端**API 的延迟与带宽而言的，而 ADR-0001 把本仓定为 local-first、前端打到 `127.0.0.1:8000`，该价值在此处未被测量过；且它自带一套状态模型（`isPending`/`isError`/`isStale`），而 `019` 刚把 `PanelState` 立为**表现层契约**，两者如何映射只能对着第一个真实页面设计，不能悬空决定。**注意 ADR-0003 的论据不可直接搬运**：它谈的是 wheel 体积、BLAS 线程固定与数值可复现，这些对一个被打包进 205.74 kB JS 制品的前端依赖并不成立；前端该量的是包体积与供应链面 |  | D24 |
| `V2-P5-015` | ~~页面①数据体检~~ **已完成** | 产 | 014, P1-012 | 交付 `web/src/components/DataHealthPanel/`（纯组件，九态齐备，跑共用的 `describePanelStateContract`）+ `web/src/pages/DataHealthPage.tsx`（路由容器）+ `contractState.panelHealthStateFrom`。数据面为 `GET /api/v1/panel/health`，**一条端点即覆盖本行全部四项**（覆盖 = `datasets[].years_present` 对 `years_requested`、新鲜度 = `cadence`/`event_age_seconds`/`fetch_age_seconds`、缺失修正 = `findings[]` 按 `blocking`/`warning`/`notice` 三档、就绪 = `state`/`is_ready`）。**本行真正的判别式在别处**：一份报告可以 `is_clean: true`、三项计数全 0、数据集 `state: "ready"`，**而它干净只是因为没人问** —— 故 `checks_waived` 非空或 `cross_checks[].ran === false` 判 `degraded` 而非 `ready`，这是 `replayStateFrom` 先读 `look_ahead_violations` 的同形。实测一份读 `is_clean` 的朴素实现在此夹具上答 `ready`。**caveat 一项半兑现**：`limitations[]` 独立成节（与 `findings` 平级，不合并 —— 结构性边界与本次抓取的缺陷补救方式不同），夹具用的正是 `current_universe_is_not_pit_universe`；但**"公开数据 vs 授权数据"在 `panel/health` 契约里没有对应字段**，故未渲染、也未杜撰 |  | S73, S48, S72 |
| `V2-P5-016` | ~~页面②候选清单 + 个股详情~~ **已完成（本行八项中七项交付，第八项契约上不可能，说明在下）** | 产 | 014, P4-005 | 交付 `ShortlistIndexPanel`（`GET /api/v1/shortlists`，按内容地址列出，每条即一个可收藏 URL）+ `ShortlistDetailPanel`（`GET /api/v1/shortlists/{id}`）+ 两个路由容器 + `contractState.shortlistStateFrom`。**本行的判别式**：渲染的是 `admitted` 而**永远不是** `funnel.shortlist` —— 被拒清单上前者为 `null`、后者满员，读错字段就会把一份被拒清单的名字当结果打在屏幕上；且 `admitted: null`（拒绝）与 `admitted: []`（放行但市场无货）保持为**两个**答案（`blocked` 对 `empty`），即服务端 `V2-P4-062` 专门重建过的那个区别。实测一份读 `funnel.shortlist` 的朴素实现在两条头号夹具上都答 `succeeded`。已交付：排序（`rank`）、分数（`score`）、置信度（`confidence`）、证据链（`run_manifest_id` 逐条 + `unresearched`/`evidence_without_a_stored_run` 单列）、失效条件（`blocks[]` 带 `measured`/`required` 两侧，**放行的清单也照列** —— 擦线过与远远过是两个事实）、可交易性告警（`funnel.untradeable[]` **并列 `untradeable_not_named` 残差**，只渲染数组会少报一个被服务端 `MAX_NAMED_UNTRADEABLE` 截断的量）、股票池标注（`declaration` 的 tier/transform/exchange/years/components + `cross_section.universe_count`）。**未交付两项，均因契约无字段而非遗漏**：① **排名变化** —— `ShortlistAnswer` 只有 `rank`，没有 `previous_rank`，端点也按内容地址单份取用而非序列，画一个箭头就是把比较**编**出来，屏幕上编的"▲2"与实测的无从分辨；② **数据授权来源**（S72 后半）—— 该 payload 不带任何 redistribution/licence 字段，故只陈述其确实携带的口径，不冒充许可声明 |  | S74, S75, S78, S72 |
| `V2-P5-017` | ~~页面③因子与模型实验室~~ **已完成（七项中五项交付，两项契约无字段并已具名）** | 产 | 014, P3-015 | 交付 `web/src/pages/FactorLabPage.tsx`（`/factor-lab` 与 `/factor-lab/:experimentId` 两个容器）+ `FactorExperimentIndexPanel`／`FactorExperimentPanel`／`PredictionRegisterPanel` 三个纯组件（各跑共用的 `describePanelStateContract`，九态齐备）+ `contractState.factorExperimentStateFrom`／`predictionRegisterStateFrom`。数据面全部是 **GET**：`/api/v1/factors/experiments`、`/api/v1/factors/experiments/{id}`、`/api/v1/predictions`。**本行的判别式来自服务端自己的措辞**：`factor_view.everything_is_unmeasured` 把「六格全 `not_measured`」称作「the quietest bad answer」，并记着验收评审把它点名为本面上最危险的一件事——这样一次运行 exit `0`、answer `200`，而「grep `removed` 没找到就停」的读者已经对两个从未算出数的档位下了「该因子通过了中性化」的结论。**CLI 会为此打一行具名提示、`--json` 打在 stderr，而它刻意不是信封上的字段**，所以浏览器是唯一什么都收不到的面；`factorExperimentStateFrom` 因此自己重算这条性质，并接住更近的一次擦肩：五格 `survives` 而 `ACCEPTANCE_STEP`（`processed→neutralized`）单独 `not_measured` 仍判 `degraded`，因为验收判据只由那一步决定。**`removed` 明确不降级**——测出来了、消息是坏的，与「测不出来」是两个答案。三个分类器**先写朴素版并实测：19 例失败 9 例**。已交付：因子定义（`spec.ic.definition` 全字段）、IC（逐档 `mean_ic`/`icir`/`sign_consistency`，**coverage 与统计量并列**，故 `null` 不会与实测 0 同形）、分组（`group_mean_net_returns` 与毛收益并列，差额即成本拖累）、三档对比（三档行 + 六格归因网格，**验收步骤单独标注**，因为 `factor_view` 明说「a face that printed six equal rows left the reader to know which」）、模型样本外指标（预测登记；逐条渲染 `standing_proves`/`standing_does_not_prove`，那是序列化器为「印了 standing 就停」这一缺陷专门加的两句）。**未交付两项，均为契约无字段**：① **衰减**——`ICDecayCurve`/`ICDecayRung`/`FactorICStudy.decay` 是完整契约但**全 `src/` 零调用方**，不进 artifact、不进 view、不进路由；客户端也拼不出来，因为 `TierReport` 的校验器强制一次实验一个 `horizon_sessions`，而 `ICDecayCurve` 要求各档同一样本；② **相关性**——HTTP 上唯一的相关系数是 `tiers[].survival`，左右是**同一因子**的 raw 与本档（`left_key == right_key`），故标题写作「档位存活相关性」而非「因子相关性」；跨因子那个是 `ICSeriesCorrelation`，同样零调用方。两项都以 `FACTOR_LAB_CONTRACT_GAPS` 渲染在页面上并双向断言（缺口具名、图不存在） |  | S76, S77 |
| `V2-P5-018` | ~~页面④组合与验证~~ **已完成（七项中四项交付，三项无任何浏览器可达路由并已作为阻塞依赖具名）** | 产 | 014, P0B-010 | 交付 `web/src/pages/PortfolioPage.tsx` + `PortfolioConstructionPanel` + `contractState.portfolioConstructionStateFrom`，数据面为 `POST /api/v1/portfolio/construct`（`V2-P5-013` 补的那条路由）。与页面①同理**开局 `idle` 而不 mount 即取**：`shortlist_id` 是某一份具体答案的内容地址，替用户挑一个就是给他从未选过的清单报组合。**本行最要命的一条是浮点**：`construction_view` 把每个 Decimal 渲染成字符串，理由写在源码里——「so a JSON reader cannot silently take a weight through a float, which is the one conversion that would make `sum(weights) == invested_weight` stop being exactly true」。故 `types.ts` 把这些字段镜像为 `string`，面板**从字段渲染 `invested_weight` 而不重算**，请求也把小数当字符串发（pydantic 对 `"0.1"` 精确解析，而 JSON 数字 `0.1` 先是 float）。**证明它的夹具是实测改过的**：最初的 0.4/0.35/0.25 在 IEEE-754 下正好等于 `1`，断言存在却分不出两个答案；现为 0.7/0.2/0.1，按契约发出的顺序左折求和得 `0.9999999999999999`，另有一条守卫测试钉住这条性质不被改回去。分类器同样朴素先行，两处被实测抓出：`targets.length > 0` 把「换手预算缩放后仍突破上限」的构建当干净组合（服务端为此专门有一条 `the_turnover_budget_can_leave_a_cap_breached_and_says_so_instead_of_retrimming`），以及 `if (view.unallocated_weight)` 把**每一份**干净构建都判降级——因为线上值是**字符串** `"0"`，非空字符串恒真。已交付：权重（`targets[].weight`／`untrimmed_weight`／`was_adjusted`，逐字符串）、换手（`turnover` 与 `turnover_before_budget`/`turnover_budget`/`turnover_damping` 并列）、暴露（**只有总量口径**：声明的四个上限与 `invested_weight`）、归因（`method` 这句 `heuristic, not optimized` 逐字渲染 + 服务端自带的 `limitations[]` 逐条）。**未交付三项，均为无任何浏览器可达路由（属阻塞依赖，未自行加路由，`api/app.py` 不归本行）**：① **容量**——`/portfolio` 与 `/backtests` 下无任何容量字段，构建面自己声明 `no_capacity_liquidity_or_cost_term_enters_a_weight`；真数字在 `factor_tradeability.py`，只经 `POST /api/v1/factors/run` 出现在**因子实验**上，按因子计量而非按组合；② **Paper 净值**——`backtest/paper.py` 在，但实测 `api/app.py` **不 import 它**，没有 `POST /api/v1/portfolio/paper/advance`，且全库检索不到 `nav`/`net_value`；最近的是 `PortfolioBacktestReport.equity_curve[].equity`，那是回测不是 paper 账本；③ **分段**——`segmented_reporting.py` 与 `OpenAlphaSDK.segmented_outcomes` 在，但无路由，同一条缺口盖住 `outcome_statistics`（P5-008）与 BH `family` 块（P5-007）。**暴露的分解半边同样缺**：`targets[].industry_code` 在出货路径上结构性为 `null`（`an_industry_cap_is_unenforceable_on_the_shipped_shortlist_face`），故不做任何替代推算。三项以 `PORTFOLIO_CONTRACT_GAPS` 渲染在页面上并双向断言 |  | S79 |
| `V2-P5-019` | ~~判别式 8 态联合扩到全部面板~~ **已完成（依赖 `014` 被实测证伪）** | 产 | ~~014~~ | 交付 `web/src/panelState.ts`（判别式联合 + 两条编译期穷尽性断言）、`web/src/contractState.ts`（契约 → 状态的纯分类器）、`web/src/components/PanelNotice/`（全库唯一发出 `role="alert"` 的地方，四个面板共用一份实现而不是各抄一遍 JSX —— 本行的病症就是「四个面板对失败的表达已经分叉」，再抄四遍是让它再分叉一次）。**本行原文两处与实测不符**：① `EvidencePanel.tsx:9` 上的不是「8 态」而是**五态** `"idle"\|"loading"\|"ready"\|"empty"\|"error"`，八态联合在 `web/src` 中**从未存在过**；八个名字的出处是 PRD 决策 14（`loading/ready/empty/degraded/stale/blocked/failed/succeeded`），而那八个**不是**这五个的超集 —— `idle` 与 `error` 都不在其中。故交付的是**九**态：PRD 的八个，加上 `idle` 保留不并（`empty` 是「问过了，没有可见证据」，`idle` 是「还没问」，为凑数合并会删掉信息），`error` 更名为 `failed`，`ready`（读取有结果）与 `succeeded`（作业跑完了）分列但共用同一条数据渲染路径，所以没有任何一个分支不可达。② 本行声明依赖 `014`，实测**不成立**：整行在既无 React Router 也无 TanStack Query 的情况下交付，`web/package.json` 运行时依赖仍只有 `react` + `react-dom`。**三个新态全部由真实契约字段构造，不是装饰**（一个只在组件测试里构造得出来的态，就是 `V2-P5-020` 那句「分支从未被渲染」换了一层）：`blocked` ← `decision.risk_decision === "block"`（`types.ts` 已镜像的闭合枚举）与 `look_ahead_violations > 0`（PRD 决策 8/19 的 fail-closed 闸门 —— 该报告的成功计数可以全部完美而违规数非零，`replayStateFrom` 因此先读违规数，测试里那条 `succeeded === total_cases && success_rate === 1 && look_ahead_violations === 1` 的夹具正是用来分开这两个答案的）；`degraded` ← `redistribution !== "allowed"`（`restricted` 与 `unknown` 同等对待：「不知道许可」不是许可）、`signal.direction === "abstain"`（携 `abstention_reason` 原文）、非空 `risk_flags`、非空 `failures[]`、空 `attribution`（**刻意不设残差阈值** —— 全库没有量过任何「可接受残差」的分界，凭空造一个就是没有证据的数字；「一个具名项都没有」不需要阈值）；`stale` ← 表单的 (subject, clock) 与屏幕上结果所对应的那一对不符，以及证据代次变更。**顺带修一个真实缺陷**：`loadEvidence` 会清空下游结果而 `importBatch` 不会，于是导入批次后上一次研究的结论仍以「当前」的姿态留在屏幕上，描述着它从未见过的证据集合；现在标为 `stale` 并给出原因（保留而非清除：那次运行是真的，只是关于另一组输入）。**变异实测**：把两条 staleness 计算改成 `false`，恰好新增的两条测试红、其余三条绿；把 `PANEL_STATE_KINDS` 删掉一个成员，`tsc -b` 在 `panelState.ts:81` 报 `Type 'true' is not assignable to type 'never'`。**`V2-P5-020` 点名的「4 个面板都有 `role="alert"` 分支但 `error` 态从未被渲染」在本行已全部渲染**：`src/test/panelStateContract.tsx` 是**同一套**契约测试对四个面板逐一执行（而不是手写四份，手写四份正是当初分叉的方式），九个态逐个渲染，并断言 alert 态必带非空文案、且**不带任何 payload**（拒绝不能被打扮成结论）。实测门：`pnpm test` **150 passed / 9 files**（基线 53 / 4）；覆盖率 statements 80.9 → **91.35%**、branches 73.47 → **83.67%**、functions 77.58 → **89.88%**、lines 82.12 → **92.55%**，`vite.config.ts` 的门按其自身「只上调」的规则相应上调至 91/83/89/92；`pnpm lint`、`tsc -b` 干净；`pnpm build` 成功（205.74 kB）；`pnpm test:e2e` **4 passed**（离线：浏览器本机缓存、`page.route` stub、本地 vite webServer） |  | D25, T14 |
| `V2-P5-020` | ~~组件隔离测试 + 前端覆盖率门~~ **已完成（本行原文三处与实测不符，改的也不是原文说的那个东西）** | 测 | 019 | **本行原文与 `F57`/`F63` 三处在 `3d3f8c6` 上被实测证伪**：①「当前**无任何组件被隔离渲染**」——**假**，`V2-P5-019` 已让四个面板经 `src/test/panelStateContract.tsx` 逐一隔离渲染九个态，本行开工时 `pnpm test` 就是 **150 passed / 9 files**；②「`web/vite.config.ts` 无 coverage 键」——**假**，coverage 门自 `2026-08-07` 起就在，且 `019` 已按其自身「只上调」的规则把 ratchet 提到 91/83/89/92；③「4 个面板都有 `role="alert"` 分支但 `error` 态从未被渲染」——`019` 已全部渲染，且 `role="alert"` 全库只从 `PanelNotice` 一处发出。**真正的缺陷是门的量程，不是门的数值**：vitest 的 v8 provider 在没有 `coverage.include` 时只量「被某条测试 import 过的文件」，分母由测试套件挑而不是由源码树定。实测——往 `src/` 丢一个两个导出函数、零测试的模块，四个数字**逐字节不变**（370/405 statements、246/294 branches、80/89 functions、348/376 lines）。一个看不见未测代码的覆盖率门，是一道对着空气设的门。补上 `include: ["src/**/*.{ts,tsx}"]` 后，同一个探针把四条门全部打红。**分母同时改正了两处**：`src/main.tsx` 从来没被计过（没有测试 import 它），现在按 0% 计入而**不**做豁免——它只有 1 条语句，由 `pnpm test:e2e` 覆盖，排除它只把 statements 从 89.88 抬到 90.14，动不了任何一条门，所以让它留在报表里可见；`src/test/**` 则**一直**被当产品代码计（52 条语句、50 条被覆盖），那是拿测试给测试打分，现已排除。**一条百分比门永远抓不到的东西，已实测**：把 `ReplayPanel.test.tsx` 整个删掉（16 个测试，150 → 134），statements、functions、lines **一个都没动**（320/356、64/73、299/328 前后相同），只有 branches 掉了 1.06pp。原因是 `App.test.tsx` 挂载整棵树，每个面板的行都会被路过执行，无论有没有人对它断言——**覆盖率量的是执行，不是断言**，所以它看不见断言被删掉。反方向同样成立：本行新增的 31 个测试只买到 **1 条语句、4 条分支、1 个函数、1 行**。这是 `V2-P4-038`「相等抓得到删除，下限抓不到」的前端版本。**因此 ratchet 定在实测取整的 90 / 84 / 89 / 91**（实测 90.16% / 84.85% / 89.04% / 91.46%），余量按整数记是 **0 条语句、2 条分支、0 个函数、1 行**——四项里两项为零，多一个未覆盖函数就红，它不是「设在今天数字下方因而什么都不 ratchet」的那种门。**并且把同一套测试放回旧量程重量了一遍**，`91.60% / 85.03% / 91.01% / 92.81%`（371/405、250/294、81/89、349/376），对 `019` 的 91.35 / 83.67 / 89.88 / 92.55 **四项全升**——打印出来的下降是分母换了，不是测试变少了。**能点名一个文件的那半交给 `web/src/testDiscipline.test.ts`，不由百分比模拟**：vitest 4.1.10 实测**无法同时**表达聚合门与逐文件门——`thresholds.perFile` 只有全局一个，glob 条目的类型是 `Pick<Thresholds, 100 \| "statements" \| "functions" \| "branches" \| "lines">`（没有 `perFile`），而且 glob 是**聚合**而非逐文件：实测 `"src/components/**": { functions: 60 }` 在 `ReplayPanel.tsx` 只有 50% 时照样绿。该测试的形状是**两个方向的相等**：每个产品模块必须有同级 `*.test.*`，否则进豁免表并写清理由；豁免了却已有测试、或豁免了一个已不存在的模块，同样红。另有两条钉住量程本身——`include` 那行被删、`exclude` 名单被放宽，都是把门做绿的最省事办法，现在会红而不是让某个数字悄悄上去。**顺带补了三个此前根本没有同级测试的模块**：`PanelNotice`（全库唯一发 `role="alert"` 的地方，靠四个面板路过就拿到 100% 覆盖率，却没有一条属于自己的断言——覆盖率报表对此一个字都没说）、`StatusBar`（唯一不在契约套件射程内的面板级组件，补上的正是它此前未覆盖的那条分支：后端可达但自称 `status="error"`）、`api/client.ts`（钉住 task 17 那条真实缺陷——请求体**不得**携带 `code_commit`/`config_digest`，那是浏览器替服务端伪造出处，改回去只要一行）。**剩下没被覆盖的是什么，逐条量过**：8 个未覆盖函数**全部**是事件处理器（`AttributionPanel` 的四个 `onChange`、`EvidencePanel` 的 `as_of` `onChange`、`ReplayPanel` 的 `onFile`、`App` 的 health `.catch` 与 `startReplay`）；`schemaDrift.ts` 的 17 条未覆盖语句几乎全是防御性 `throw`（`cannot classify schema node`、`is not an array-of-object type`）。九个态的**渲染**已被证明，**交互**一条都没有——这是诚实的下一行，不是本行糊上去的东西。**未动 `web/src/**` 的任何实现文件**（并行 agent 持有 `014`/`015`/`016`），本行只增测试文件与 `vite.config.ts`。实测门：`pnpm test` **181 passed / 13 files**；`pnpm lint`、`tsc -b` 干净 |  | T14 |
| `V2-P5-021` | ~~Playwright page object + 4 页金流~~ **已完成（并先修掉它依赖的那个生产缺陷）** | 测 | 015-018 | **基线实测：6 tests / 1 project / 2 个 spec 文件、零 page object。交付后 25 tests / 2 projects / 4 个 spec + 6 个 page object 模块。** `F64` 的两条都真：`golden-flow.spec.ts` 从未 stub `/api/v1/backtests/validate`，**因而 e2e 从未走到归因**。根因不是漏了一行 stub —— 是**这套件里没有一条断言是关于「请求发生过」的**：`toBeVisible()` 在面板从未跟服务端说过话时同样绿。故 `stubs.ts` 计数每一次 `page.route` 命中，`expectRequested("validate")` 才是 `F64` 真正需要而当时写不出来的那条断言。**stub 载荷改从 `src/test/fixtures.ts` 取**：该文件自称与 `docs/api/schemas/` 同形、「与 `App.test.tsx` 和 `e2e/golden-flow.spec.ts` stub 的是同一批形状」—— 那句话此前靠意图成立、靠构造不成立（e2e 各抄各的字面量），现在是可执行的。**实测推翻了「挂载只发一次请求」这个假设**：首版 `expectReached(..., 1)` 在三条金流上同时红，`Expected: 1, Received: 2` —— `main.tsx` 的 `<StrictMode>` 在开发下**故意**双跑 effect。因此断言分成两种而不是把界放松：点击触发的用 `expectRequested`（**恰好一次**，双提交按钮仍会红），挂载触发的用 `expectRequestedOnMount`（**≥1 且 ≤2**，双侧界，取数循环仍会红）。**四页金流各自钉住自己的契约**：①④ 起于 `idle`（端点有必填参数，不得替用户编造），断言的是「问之前一个请求都没发」；②③ 挂载即取（端点无参数），断言的是取到了；另有一条只有浏览器测得到的：让 `/api/v1/predictions` 500，因子实验索引必须仍然出现（`FactorLabPage.tsx` 不用 `Promise.all` 的那个理由，是关于两个在途请求而非两个已渲染状态）。**顺带补上 `tsc -b` 从未读过 `e2e/`**（三个 tsconfig 的 `include` 都不含它），新增 `tsconfig.e2e.json` 并挂进 references。实测门：`pnpm test:e2e` **25 passed**（离线：浏览器本机已缓存、全部 `page.route` 本地 fulfill、两个 webServer 都在 127.0.0.1）；`pnpm lint`、`tsc -b` 干净；`pnpm test` **438 passed / 30 files**、覆盖率 93.19/87.52/94.41/94.36 与 ratchet 一致未动 |  | T15 |
| `V2-P5-022` | ~~报告导出许可过滤（不导出 Tushare 原始 payload）~~ **已完成（本行被判定为「被缺失许可字段挡住」，实测不成立）** | 产 | 018 | **交接里记录的障碍是真的，但挡不住本行**：页③④ 确实没有任何一行携带许可 —— 因子实验信封、预测登记、`construction_view` 都没有，`ShortlistDetailPanel.tsx` 里也白纸黑字写着 `ShortlistAnswer` 不带 `redistribution`。**但本行不是关于那些面的**：PRD §5.9 `S72`、§5.10 `S81` 与决策 27 三处说的都是**报告**与**原始 payload**，而报告的证据正是全仓唯一真的带许可的地方 —— `EvidenceSnapshot.source_license` 与 `redistribution: Literal["allowed","restricted","unknown"]`，由 `evidence/builder.py` 直接抄自 `ProviderMetadata`，三个已发货 provider 全部声明 `restricted`。故本行**按原文可closed**，并且不需要任何契约变更。**扣下什么、留下什么，是在真适配器上量的**：`providers/tushare.py:4038` 建的是 `payload=cast(JsonValue, row)` —— 上游响应行**逐字**，那就是决策 27 说的「原始 payload」；同一个构造器建的 `summary` 是 `f"Tushare {kind} record for {subject} on {date}."` —— 本仓自己写的模板，只含数据集名、标的与日期，**不含任何 provider 字段值**，故 summary 保留，并把这条边界写进模块而不是默默决定。**扣下是一个 tag，永远不是一个缺席**：`ExportedEvidence.body` 是判别式联合而非可空 `payload` —— `JsonValue` 允许 `null`，「payload 本来就是 null」与「payload 被拿走了」在任何用缺席表示扣留的设计里**逐字节相同**，而它们是两件事（一件关于证据，一件关于许可）。`test_the_disposition_is_a_tag_and_not_a_guess_at_a_missing_payload` 就是那条能分开两个答案的夹具。**`unknown` 不是许可**，与 `restricted` 同等对待，fail-closed 方向。**证据引用取不到时具名上报**（`evidence_not_recovered`），不是让证据列表悄悄变短。**三面同时交付**：`GET /api/v1/reports/{report_id}/export`、`OpenAlphaSDK.export_report`、`openalpha report export`，`test_surface_parity.py` 的三个计数因此各 +1（47/54/32 → 48/55/33），`without_sdk` 与 `rest_only` 不动 —— 一条能力上了三张脸，而不是一张脸加两条缺口。实测门：单元 10 项（`tests/unit/product/test_report_export.py`）、集成 5 项（`tests/integration/test_report_export_interfaces.py`，全部经 `CliRunner`/`TestClient`/`SDK`，三面导出**逐字节相等**、原始行标记在三面输出里**都不出现**）|  | S72, S81, D27 |
| `V2-P5-024` | ~~**buffered / turnover-controlled 对照版本**：默认与无缓冲版并列出报，避免把高换手因子的 gross edge 误读为可执行 alpha~~ **已完成** | 技 | 001,008 | 已交付 backtest/turnover_variants.py（第 41 个 KNOWN_* 注册表，6 条）。**两臂都是必填字段，单臂报告在类型上不可表达**——「默认并列出报」写成类型而不是约定。no-trade band 与 V2-P5-001 的 turnover_budget 是两种装置：预算按比例阻尼所有名字，band 让小变动完全不交易、大变动整笔吃下；策略同时声明两者时先预算后 band。**实测推翻了本模块自己的第一版设计**：tracking_deviation 与 turnover_reduction 恒等（20 万组随机账本零反例），派生列无法与父列分歧因而检测不到任何东西，故删列改为 property，并把该恒等式本身当结论报出——band 省下的每一单位换手，就是离目标账本的每一单位距离。成本率无默认值：未声明则只报换手并具名说明为何没有金额 | 单元：33 项（tests/unit/backtest/test_turnover_variants.py），变异 13 杀 13；集成：缓冲版换手 0.100000 显著低于无缓冲版 0.155000（band 0.030，四级台阶全部实测，tests/integration/test_portfolio_construction_interfaces.py） | S53, S56 |
| `V2-P5-023` | ~~测试树重组的三制品同步~~ **已完成（做成可执行的相等，而不是执行一次移动）** | 测 | P0A-001 | **本行原文与 `F53` 的「`features.csv` 点名全部 34+2 个测试文件」两处实测不符**：逐条数，`features.csv` 点名 **182** 个不同测试文件（180 个 Python + `web/src/App.test.tsx` + `web/e2e/golden-flow.spec.ts`——「2 个 web」对，「34 个 Python」差 5.3 倍）；「**全部**」也不成立——磁盘上有 273 个 `tests/**/test_*.py`、9 个 `web/src/**/*.test.*`、1 个 playwright spec，被点名的是 182/283。**「`--check` 报 `0 unknown, 0 unreviewed`」这两个数，量程是空的**：它们是 `_summary` 里的两个**字面量 `0`**（`:242-243`），从台账第一次提交起就没有量过任何东西，也不可能是别的值，而 `--check` 把它们当作结论打进 stdout、`_markdown` 把它们排进发布文档。现已改为从行计算，并挂到 `_load` 强制的同一个 `TERMINAL_STATUSES` 上，放宽那个集合会让计数动而不是让它冻在一个让人安心的字面量上。今天仍是 0——区别是它现在是**测量**而不是**断言**。**「unknown 与 missing 不是同一个问题」实测如下**：*missing*（台账点名了、磁盘上没有）今天确实是 **0**，但只被**部分**强制——存在性检查整块坐在 `if status in TRUE_COMPLETE` 里，185 行中的 5 行 `EXCLUDED`/`DEFERRED` 完全豁免，而有 3 条路径**只**经这 5 行到达（`SECURITY.md`、`docs/data/providers.zh-CN.md`、`docs/specs/openalpha-cn-v1-spec.md`）。实测把 `SECURITY.md` 整个删掉，`--check` 照样打印 `{"unknown": 0, ...}` 并**退出 0**。检查移出该分支后，同一次删除红并点名：`OA-BOUND-004 references missing evidence: ['SECURITY.md']`。*unknown*（磁盘上有、台账一个字都没点）是 **106**，没有任何东西在管。**刻意不把它做成相等**：这是 **v1** 功能台账，v2 的 P0–P5 测试本就不属于它，要求 283 个测试文件全部被点名是把噪音当纪律。**交付 `tests/unit/test_feature_ledger_test_tree.py`**，形状照抄本仓库自己最成功的那个——`REGISTRY_ENTRY_COUNTS` 的**每组一个精确计数**：20 个目录的计数字典 + 总数 182，都是**相等**而非下限或成员检查，再加一条覆盖全 CSV 全字段全状态的存在性断言。实测两种移动都红：只移文件不改 CSV → `features.csv references paths that do not exist: ['OA-PANEL-006 -> tests/unit/domain/test_adjustment.py']`；移文件**并**同步改了 CSV → 计数相等红并点名两端 `tests/unit/domain: 23 != 24` 与 `tests/unit: 30 != 29`（总数 182 不变，正说明逐目录计数才是承重的那半）。**明写它抓不到什么**：同一目录内的对调（删掉点名 `a.py` 的行、加一条点名同目录 `b.py` 的行）逐目录计数与总数都不变——正是 `V2-P4-038` 那个形状；`acceptance_kind="pytest"` 的行由 `_validate_pytest_acceptance` 的 AST 检查接住（函数必须真存在），其余接受，理由写在模块 docstring 里而不是留给下一个人重新发现。**`summary.json` 与 ledger md 逐字节未变**，证明改的是那两个数的出处而不是它们的值；`--check` 仍退出 0。实测门：`PYTHONDONTWRITEBYTECODE=1 uv run pytest tests/unit -q` **3146 passed / 1 skipped**；`ruff check`、`ruff format --check`、`mypy src scripts` 干净 |  | S87 |
...
| `V2-P5-026` | ~~**版本号被改编号后，已迁移的库永久卡死**：`create_validation_results` 被插进已被占用的版本 2，导致真实库停在 `user_version=4` 再也无法前进~~ **已修（按 schema 实测的对账 + 不可判定即上报 + 冻结版本号→名字映射）** | 技 | P0B-004, P0B-010, P4-111 | **在用户真实 `runtime/state.sqlite3` 的副本上实测（原件全程只读拷贝，从未打开写入）**：`user_version=4`；`schema_migrations` 为 `[(1,baseline),(2,demo_add_runs_archived_at),(3,demo_add_runs_archived_at),(4,create_query_path_indexes)]`；无 `validation_results` 表；`create_app(runtime_dir=…)` 连跑三次仍是 `4,4,4`。**成因已定位到提交与时刻**：`1e54104`（2026-08-07 08:40 EDT）发布的注册表里 `DEMO_ADD_RUNS_ARCHIVED_AT_VERSION = 2`，用户库在 12:33:46 UTC 就是按这份编号盖上 `(1,baseline)+(2,demo)` 的；`6eba39c`（同日 15:01 EDT）把 `create_validation_results` **插进版本 2** 并把 demo 改编号 2→3，于是同一个库在 18:50:18 UTC 把 demo **按新编号 3 又记了一次**（幂等空操作），08-08 13:02:20 记下 4，此后 `_pending()` 只按 `version > user_version` 过滤，版本 2 永远在水位线以下、`validation_results` 永远不会被建，而 `_rewrite_contract_identities` 正以该表为前提 —— **每次进程启动都 defer，无终止条件**。**三条原始判断被实测推翻**：①「重复行说明 `schema_migrations` 没约束住它 docstring 假设的东西」——**错**，`version` 是 PRIMARY KEY 且完好，没有任何版本被记两次；重复的是**名字**，出现在两个不同版本上，因为同一个迁移确实按两套编号各跑过一次，该表约束的正是它声明要约束的。②「版本计数器与审计表对历史各执一词」——**它们彼此完全一致**：`user_version=4`，审计表恰好是连续的 1–4；不一致的是审计表里的**名字**与当前注册表对同一批数字的命名，这只有拿注册表来比才看得见。③「是否是一次性事故」——**不是**：`DROP TABLE validation_results` 后重启会到达同一状态（已作为 CLI 用例固化），任何把迁移**插入**已发布编号的改动都会复现。**同时复核并否定了另一个 agent 的说法**「迁移 4–8 对任何新库都不生效」：全新 runtime 目录实测 `start1→user_version=2 applied=[baseline,create_validation_results]`、`start2→8`、`start3→8`，`require_table` 的延迟按设计工作，**未做任何改动**。**修法与明确拒绝**：(a) `Migration` 增加 `effect_present` 谓词，只许查 `sqlite_master`/`PRAGMA table_xinfo`，**不许信任任何一个计数器**（**这条原则当时只落实了一半，`V2-P5-029` 补齐**：`_unrecorded` 按 `name not in (recorded \| repaired)` 取集合，所以谓词只对「历史没提过」的迁移问过；「效果没了但审计行还在」这一类从来没有被检查过，也就是说在两者唯一可能分歧的问题上，审计表每次都赢）；三个数据改写迁移（`rewrite_contract_identities`/`split_batch_task_items`/`rewrite_manifest_component_planes`）**故意留 `None`**，因为它们的效果是payload 字节，schema 看不见。(b) `run_migrations` 在 pending 循环**之前**跑 `_reconcile`：效果缺失才真跑 DDL 并记 `applied`，效果已在则只记 `verified` 不重跑，**绝不写 `user_version`**（版本早已越过，没有可前进的目标）。(c) 修复记在**新表 `schema_repairs`**，不是塞进 `schema_migrations` —— 后者 `version` 是 PRIMARY KEY，版本 2 **物理上不可能**同时叫 `create_validation_results`；审计表的表结构本身就编码了「版本号含义永不变」这个被打破的假设，**一行历史都没有删改或重写**。(d) `migrate status` 新增 `repaired`/`unrecorded` 两段（文本与 `--json` 同步）；`migrate run` 打印修了什么。**明确拒绝**：不为不可判定的迁移猜答案 —— 重跑身份改写可能损坏记录，不查就记又正是本引擎存在的意义所在，两个都比卡死更糟，于是停下来并上报，交给人。**「停下来」的范围已按实测改写（`V2-P5-029` 复核）**：停的是对账循环，不是进程 —— 把 v5 `rewrite_contract_identities`（无谓词）置为 unrecorded 后实测，`build_storage` 成功、`create_app` 成功、`GET /health` 返回 `200 {"status":"ok"}`，可见信号只有一条 WARNING 日志 `migration_repair_undecidable` 与 `migrate status` 里的一行。**并且它挡住的是它后面的每一次修复**：同时把 v6 `add_runs_mode_projection`（有谓词、效果确实缺失）也置为 unrecorded，五次 `migrate run` 之后 `unrecorded` 仍是 `[(5,…),(6,…)]`、`repaired=[]` —— 一个不可判定的迁移永久地挡住排在它后面的、本来完全可判定的迁移。**该排序纪律保留**（修复是按版本号的链，与 pending 循环同理），但 `V2-P5-029` 修掉了由它派生的一句假话：`migrate status` 过去对 v6 也印「其效果无法通过检查 schema 确定」，而 v6 的效果完全可以确定，只是循环在 v5 就断了、从没问过。**根因守卫**：`test_no_shipped_migration_may_be_renumbered_or_renamed` 冻结版本号→名字映射；既有的「唯一且递增」守卫在 `6eba39c` 当天是**全绿**的，因为唯一性是单个快照的性质，而编号含义是**跨版本**、由已落盘的库持有的性质，全库没有任何东西在检查它。**顺带修掉自己的一个真 bug**（由变异存活项挖出）：`PRAGMA table_info` **不列生成列**，`runs.mode` 正是 `GENERATED ALWAYS AS … VIRTUAL`，谓词若按 `table_info` 写就永远只会答「缺失」并对着已存在的列反复重跑 —— 改用 `table_xinfo`，并补一条专门钉这条 SQLite 行为的测试。**实测门**：`pytest tests/unit` 3040 passed、1 skipped；`pytest tests/integration` 全绿；`ruff`/`mypy` 干净；`lint-imports` 8 kept / 0 broken；**变异清扫 22 个变异体、22 个被杀、0 存活**（基线 43 passed 先证绿，每个变异体 180s 硬超时，`finally`+`atexit`+信号处理保证不留残体）；真实库副本实测 `4 → 8`，第一次启动即到位，第二三次稳定 |  | S48, T16 |
| `V2-P5-027` | ~~**四页的每个可收藏 URL 在 `openalpha serve` 下都是 404**：`StaticFiles(html=True)` 只对*目录*请求回落 `index.html`~~ **已修（服务端 SPA history 回落）** | 技 | — | **由 `V2-P5-021` 立的行：它是 014-018 七行的共同阻塞，且在 `web/` 里修不了。** 两个 agent 各自独立量过同一件事并记在 `web/e2e/routing.spec.ts` 里；本行在 `12532e3` 上用 `TestClient` 驱真 `create_app` + 真 `pnpm build` 第三次复现：`/` 得 200，`/data-health`、`/shortlists`、`/shortlists/sl_abc`、`/factor-lab`、`/factor-lab/fxp_abc`、`/portfolio` **六个全 404**。e2e 套件结构上看不见它 —— 回答的是 dev server，不是应用。**规则**：build 答不了的 `GET`/`HEAD` 一律回 `index.html` 200，**除非首段属于别人**；两个「别人」都是**推导**出来的、没有第二份手写清单：① 活路由表的首段（今天 `api`/`health`/`docs`/`redoc`/`openapi.json`）—— 这是**承重的一半**，未知 `/api/` 路径必须留在 JSON 404，否则每个客户端拼写错都变成 200 的网页；② build 自己的目录（今天 `assets`）—— 缺失的子资源是坏部署，用 HTML 回答会把干净的 404 变成远端的 MIME 错误。**未知非 API 路径给 shell、由前端路由具名 404**，理由写在类文档里：反过来要求 Python 存一份客户端路由表副本，正是 `V2-P5-011` 在 `CORS_ALLOWED_METHODS` 上量到的缺陷（一件事两处陈述、无人保持相等、副本已经落后）；代价是拼错地址返 200，另一条路的代价是**能用的页面**在有人往 `routes.ts` 加一条时 404。**关于 `vite dev` 的三个假设有两个被实测证伪**（vite 8.1.5，curl 直打）：它**没有任何扩展名规则**（`/no-such.page`、`/stocks/000001.SZ` 都是 200 shell），`/assets/missing.js` 也回 shell，而它对 `/api` 的保护来自 `vite.config.ts` 里**手写的 proxy 清单**（已经漏了 `/docs` 与 `/redoc`），不是来自回落。故这两点**刻意不抄开发**。也刻意不看 `Accept` 头：让 `curl` 与浏览器得到不同答案的规则，会让一次生产事故在终端里复现不出来。**证明在真服务器上做**：`web/e2e/production-routing.spec.ts` 跑在 `uvicorn openalpha_cn.api.app:app`（`Dockerfile` 的入口）+ 真 `pnpm build` 上，**不用 `openalpha serve`**，因为 `cli.main()` 会把 `.env` 并进环境；`OPENALPHA_RUNTIME_DIR` 每次 `mktemp -d`。把回落换回原样，该文件 **8 红 3 绿**，红的正是六个深链接 + 刷新保址 + 应用内 404。实测门：`tests/unit/test_spa_addressability.py` 17 项；`test_surface_parity.py` 计数**未动**（回落是 `StaticFiles` 子类而非路由，它是「build 怎么被服务」而不是本 API 的一项能力）|  | — |
| `V2-P5-028` | ~~**28 个 CLI 命令无视 `OPENALPHA_RUNTIME_DIR`**：运行时目录写成 typer 选项默认值 `Path("./runtime")`，从不询问 `load_config()`，已导出的环境变量输给了编译期默认值~~ **已修（选项默认值改 `None` + 惰性回落 config + 走活 Typer 树的结构守卫）** | 技 | P0B-002, P5-026 | **验收方给的清单说是 8 个，实测是 28 个** —— 文件里每一个带 `--runtime-dir` 的命令，无一例外。漏掉的 20 个含 `report export`、`evidence build`、`shortlist` 四个、`model` 四个、`jobs` 四个、`factor` 两个、`portfolio` 两个、`validation` 两个。**这正是本行不写清单而写结构守卫的理由**：手工枚举做过一次、当场就少了 20 个，所以 `test_no_command_hardcodes_the_runtime_directory_as_an_option_default` 遍历**活的 Typer 注册树**，任何 `--runtime-dir` 的默认值是 `Path` 就红，新加的命令不需要谁记得改夹具。**生产阻塞的形状**：`Dockerfile:48` 是 `ENV OPENALPHA_RUNTIME_DIR=/data`，同文件还有 `WORKDIR /data` 与 `VOLUME ["/data"]`；`uvicorn openalpha_cn.api.app:app` → `create_app()` → `config.runtime_dir` 打开的是 **`/data/state.sqlite3`**，而 `docker exec … openalpha migrate status` 把 `./runtime` 相对工作目录解析成 **`/data/runtime/state.sqlite3`** —— 那个文件不存在，于是运维看到「schema version 0、8 条 pending」，说的却是一个没有任何进程在服务的库，**并且在挂载卷上凭空造出一个诱饵库**，接着 `migrate run` 会去迁移这个诱饵。**它同时废掉了 `V2-P5-026` 的整个运维面**：`migrate status` 的 `repaired`/`unrecorded` 两段、以及 `migrate run` 报告自己修了什么，只经由这一组命令出口。**实测最坏的一条不在那 8 个里**：在导出了变量的干净工作目录里跑 `openalpha jobs list`（一个只读命令）**造出诱饵库并且把它迁移了**，日志里对着一个没人服务的文件打出 `baseline` 与 `create_validation_results` 的 `migration_applied`。**违反的是本仓自己写下的优先级**：`config.py` 模块文档 —— 「已导出的真实环境变量永远压过……字段的编译期默认值」。**修法**：28 个全部声明 `Path \| None = None`，函数体第一句 `runtime_dir = _resolved_runtime_dir(runtime_dir)`。`None` 是唯一能把「调用者没给」与「调用者要 `./runtime`」分开的默认值，而只有前者可以回落到 `load_config().runtime_dir`。**惰性是刻意的**，与紧邻其上的 `_resolved_config_digest` 同形、同理由（P0.B Finding 2）：`load_config()` 原子地校验每个 `OPENALPHA_*` 字段，所以传了 `--runtime-dir` 的调用者根本不碰配置校验，一个无关的坏字段（例如非数字的 `OPENALPHA_MAX_REQUEST_BYTES`）永远挡不住他；省略该旗标的调用者则确实吃到校验，得到具名 `ConfigError` 与 exit 1 —— 当另一条路是**静默地对着错误的数据库动手**时，这是正确的取舍。两半在同一个坏环境下一起断言，防止走散。**六个行为用例砍掉三个，因为那三个的夹具分不开两个答案** —— 正是本仓反复在自己夹具上抓到的缺陷：`shortlist list`、`model predictions`、`migrate prune-backups` 面对空运行时目录时**两边都不创建任何东西**，「cwd 里没有诱饵」在坏树上同样通过。故参数化行为测试只保留真正会打开数据库的三个（`migrate status`/`migrate run`/`jobs list`，修前实测留诱饵、修后实测不留），`shortlist list` 改用另一条：只往**导出目录**里种一个合法内容地址 `sla_…`，让两个答案在输出本身上就不同（解析 `./runtime` 会打印空列表）。**红过才算**：7 条新断言在未改的 `cli.py` 上全红且红在预测的那一行；另外 3 条钉的是不变行为（显式旗标压过环境变量；两者都没有时仍是 `./runtime`），两棵树上都绿。**顺带补齐运维面**：28 个选项现在都带同一段 help，并写明优先级 —— 此前有 8 个**完全没有 help**，且 28 个里没有一个提到 `OPENALPHA_RUNTIME_DIR`（它们无视该变量时这是准确的，改完之后就成了沉默的遗漏），而容器里的运维只能靠 `docker exec … openalpha migrate status --help` 得知这件事。该守卫第一次跑时**只有 `panel doctor` 红**，查下来是渲染而非文本：该命令选项名够长，80 列默认宽度下 Rich 会把 `OPENALPHA_RUNTIME_DIR` **在词内**硬折行 —— **改夹具而不是改源码**，反过来就是在修一个并不存在的缺陷。28 个随后逐一做了结构复核：默认值 `None`、函数体第一句是解析器、`--help` 均能正常渲染。**实测门**：`pytest tests/unit` 3246 passed / 1 skipped（基线 3235）、`tests/integration/test_cli_migrate.py` 8 passed、`ruff`/`mypy` 干净；并且**用装好的 `openalpha` 二进制而不只是 `CliRunner` 复现与复验**：修前 `path` 为 `runtime/state.sqlite3` 且 cwd 下出现诱饵，修后 `path` 为导出目录的绝对路径且 cwd 全干净 |  | S48, T16 |
| `V2-P5-029` | ~~**`V2-P5-026` 的对账只做了一半：`DROP TABLE` 之后审计行还在，于是没有任何东西检查过它**，被删掉的表永久缺失且每个运维面都报告「健康」~~ **已修（按 schema 检查已记录的迁移 + 新增 `damaged` 段 + 两句与实测不符的行文改正）** | 技 | P5-026 | **`V2-P5-026` 声称「按 schema 对账，不信任任何一个计数器」，实测它完整地信任其中一个。**`_unrecorded()` 按 `migration.name not in (_recorded_names \| _repaired_names)` 取集合，因此 `effect_present` **只对历史没提过的迁移**求值；「效果缺失但审计行仍在」这一类从未被检查。**实测（库已在 head，`DROP TABLE validation_results` 且不动 `schema_migrations`，这正是 `DROP TABLE` 的真实行为）**：五次 `migrate run` 之后 `user_version=8`、`recorded=[1..8]`、`pending=[]`、无 `schema_repairs` 表、无 `validation_results` 表；`migrate run` 每次都说「schema version 8 is up to date; nothing to do」，`migrate status` 印八行 `applied` 再无其他。**表没了，永久地，而且每一个运维面都报告健康** —— 与 `V2-P5-026` 要终结的那一类静默卡死同一类，只是从另一扇门进来。**旧夹具在结构上抓不到它**：`tests/integration/test_cli_migrate.py:318-321` 的两个修复用例永远是 `DROP TABLE` **加** `DELETE FROM schema_migrations` 一起做，其中一个的 docstring 还把这对操作说成「从引擎角度看一张被删掉的表就长这样」—— **不是**，`DROP TABLE` 根本不碰 `schema_migrations`；而删掉审计行恰好就是 `_unrecorded` 赖以成立的条件，**夹具被造成了守卫必然触发的样子**，历史说谎的那一半从来没被测过。**修法**：新增 `_damaged()`（水位线以下、审计表**有**它、有谓词、且谓词答 False）；`_repairable()` 把 `_unrecorded`（`recorded=False`）与 `_damaged`（`recorded=True`）**按版本号归并**而不是拼接 —— 拼接会让每个 damaged 都排在每个不可判定 gap 之后，于是版本 2 的 damaged 会被版本 5 的 undecidable 永久挡住，尽管它在前面三个版本；`run_migrations` 的提前返回加上 `and not damaged`（它必须进这个门，因为门后第一件事就是 `_take_backup`，修复会改 schema，理应和 pending 一样先备份）；`MigrationStatus.damaged` 与 `migrate status` 的 `damaged` 段（文本与 `--json` 同步）。**刻意的两条边界**：①`_damaged` **不**排除已在 `schema_repairs` 里的名字 —— 排除就等于用自己的账本止损，一张修好后再次被删的表会永久隐形，正是本行要关掉的缺陷；止住第二遍的是谓词答 True，那是关于数据库的事实，不是关于我们记账的事实。②三个数据改写迁移（`effect_present is None`）**永不出现在 `damaged` 里** —— `damaged` 的含义是「问过 schema，schema 答没有」，问不了的就既不能说它完好也不能说它损坏，这与 `V2-P5-026` 拒绝为不可判定迁移猜答案是同一条规矩。**由此暴露并修掉一个自己引入的隐患**：`schema_repairs` 主键是 `(version, name)`，旧设计下一个迁移不可能被修第二次（修过的名字被永久排除），现在可能了，裸 `INSERT` 会把「被删两次」这种事故变成 `sqlite3.IntegrityError` → `MigrationFailedError`，故改 `ON CONFLICT (version, name) DO UPDATE`，并补 `test_the_same_migration_can_be_repaired_twice` 钉住。**顺带改正第二句与实测不符的行文**（详见 `V2-P5-026` 行内已就地更正的两段）：「停下来并上报，交给人」停的是对账循环不是进程 —— 实测 `create_app` 成功、`GET /health` 返回 `200 {"status":"ok"}`；且一个不可判定的迁移**永久挡住排在它后面的每一次修复**（v5 挡 v6，五次 `migrate run` 后 `repaired=[]` 不变）。排序纪律保留，但由它派生的一句假话已修：`migrate status` 过去对每个 unrecorded 都印「其效果无法通过检查 schema 确定」，对 v6 那是错的（它有谓词，只是循环在 v5 断了、从没问过），现在三种情形分别措辞并点名是谁挡住了它。**红过才算**：引擎尚未改动时就存在的 9 条新断言全红，且红在各自预测的那一行；其后追加的 4 条（`--dry-run` 预览、修复行的措辞、`<=` 边界）改用「一个精确回退该行为的变异体被杀」来证明它能分开两个答案。**变异清扫覆盖本次改动的 `cli.py` 与 `storage/migrations.py`：23 个变异体、23 个被杀、0 个存活**（基线先证绿，每个变异体 120s 硬超时，`finally`+`atexit`+SIGINT/SIGTERM 保证还原，事后与清扫前副本逐字节比对）。**`<=` 边界那条测试正是这次清扫逼出来的**：`<` 在第一轮存活，因为其余用例都是「库在版本 8、损坏版本 2」，没有一条站在水位线上 —— 而 `<` 会漏掉每个库的**头部迁移**，这是最寻常的一种损失。**两个既有测试因此转红并已按新的正确答案更新**（不是回归）：`test_a_projection_missing_only_its_index_reports_its_effect_absent` 与 `test_a_partly_indexed_query_path_migration_reports_its_effect_absent` 的夹具**给自己从未执行过的迁移写了审计行**，这正是本行要检出的损坏类，多出来的修复项是对的。**实测门**：`pytest tests/unit` 3246 passed / 1 skipped；`tests/integration` 全绿；`ruff`/`mypy` 干净；修复路径同时用装好的 `openalpha` 二进制复验（不只 `CliRunner`）|  | S48, T16 |
| `V2-P5-030` | ~~**未知 API 路径在每个非 GET 方法上答 405 而不是 404，且两个 owner 集按大小写敏感比较**~~ **已修（先存在再谈方法 + 归一化段名）** | 技 | P5-027 | **本行由 P5 技术验收提出，在 `37c4273` 上用原始 ASGI 复现**（绕开 httpx 归一化），对照基线是同一进程里的 `create_app(web_dir=None)`：`POST/PUT/PATCH/DELETE/OPTIONS /api/v1/nope` 挂载后全是 `405 application/json`，不挂载是 `404`；`GET /API/v1/nope`、`/Api/v1/nope`、`/api./v1/nope`、`/api /v1/nope`、`/HEALTH/x`、`/Docs/x`、`/OpenAPI.json/x`、`/Assets/missing.js`、`/ASSETS/missing.js` 全是 `200 text/html`（shell），不挂载全是 `404`。**两者都正是类文档自己说它存在就是为了防止的那句话**：「一次客户端拼写错误、一个过期调用方……回来的是 HTML 200，而按 `response.ok` 分支的调用方把网页读成了载荷」。`405` 断言「这个资源在，只是不收这个动词」——对一个拼错、改名或下线的 API 路径，两半都是假的；类文档只推理过 `POST /portfolio`（一个**客户端**地址，那里 405 才是诚实的），`test_spa_addressability.py:260` 也只钉了那一条。**`/Assets/` 是最要紧也最不会被发现的一个**：macOS 大小写不敏感，`/Assets/index-abc123.js` 在本机解析到真文件、回落根本不被咨询；在 `Dockerfile` 出货的 Linux 上查找失败、`Assets` 不是 `assets`，于是 `<script>` 要的东西被以 `text/html` 送回 —— 正是 build 目录 owner 存在要防的 MIME 错误，且**只在生产可复现**。**修法两条**：(a) `StaticFiles` 在查找**之前**检查方法，所以 405 先于存在性；现在一个非客户端位置的 405 会把请求当 `GET` 重放一次问「build 里到底有没有」，没有就改答 404 —— **用 starlette 自己的话问**，不是抄一份查找规则：目录无 `index.html` 与真文件的答案不同，只有它知道哪个是哪个。`POST /portfolio` 仍是 405（页面真在，动词真错），`POST /assets/index-abc123.js` 仍是 405（文件真在），`POST /assets/missing.js` 改为 404。(b) 两个 owner 集改按 `_normalised_segment`（casefold + 去尾部点与空格）比较。**刻意比 HTTP 更宽**且不对称：客户端区域若与 owner 撞名，`test_every_client_area_is_an_address_the_production_server_serves` 立刻点名报红；保留命名空间静默答成网页则无人可查，且由调用方而不是测试读到。**验收提的 `//api/v1/nope` 被实测证伪**：它经客户端看是 HTML 200，原始 ASGI 显示服务端答 `404 application/json` —— `httpx` 把前导 `//` 折叠成 `/v1/nope`，量到的是它自己的 URL 处理；这条已写进测试模块文档，作为「先查客户端再查服务端」的下一位读者的提示。**顺带删掉 `api/app.py:1792-1793` 重复的 `job_store = storage.job_store`**（合并残留，ruff/mypy/3235 项单元测试全绿放行）。**实测门**：`tests/unit/test_spa_addressability.py` 17 → 36 项（新增 17 项先全红后全绿）；`test_surface_parity.py`、`test_cors_method_surface.py` 未受影响；原始 ASGI 复测每一行都与不挂载基线逐字相同 |  | — |
| `V2-P5-031` | ~~**离线守卫在测试函数体之外完全失效**：`_depth` 由函数级 autouse fixture 抬起，模块导入与所有宽作用域 fixture 的 setup 全在守卫之外~~ **已修（守卫升到整个 item 与 collection）** | 测 | P4-105, P4-039 | **本行由 P5 技术验收提出、在 `37c4273` 上用一个放进真实测试树的探针复现**（跑完即删），每个阶段都向本进程内的 127.0.0.1 监听端口发起连接：`module-import-time`、`session-scoped fixture setup`、`module-scoped fixture setup`、`class-scoped fixture setup` 四个阶段**全部 NOT REFUSED，连接完成**，只有 `inside a test body` 抛 `OfflineSuiteViolation`。**class 级是验收清单之外的第四个，本行实测补上。**「取一次、复用」正是 session 级 fixture 存在的那句话，而它恰好落在守卫看不见的地方；`tests/conftest.py:274-303` 声明了四条限制（子进程、`getaddrinfo`、`AF_UNIX`、`ctypes`），**这一条不在其中** —— 比不声明更糟：来查自己 fixture 是否被覆盖的人，看到的是一张没提这个问题的清单。**选择抬作用域而不是声明限制，并且是用测量决定的**：全树 `scope="session"` 的 fixture 在 `tests/e2e/` 之外**一个都没有**，37 个 module 级全部离线，整树 5475 项在守卫下 collect 干净（e2e 的 60 项照常 deselect）。**用 hook wrapper 而不是更宽的 fixture，因为更宽的 fixture 表达不了豁免**：宽作用域 fixture 先于任何函数级 autouse fixture 实例化，所以一个 session 级守卫会在 `tests/e2e/` 自己的 session 级 `built_panel` 抓真实面板时把 `_depth` 按在 1 上，而出路会变成「一个 fixture 去减另一个 fixture 拥有的计数器」。`pytest_runtest_protocol` 把整个 item 括起来（setup/call/teardown），所以该 item 拉起的宽作用域 fixture 就在它的括号内 —— 未标记的被守，`e2e` 的根本不进括号；`pytest_collection` 括住导入期。**剩下的那条限制被声明并且被测量**：session 级 fixture 的 *teardown* 在最后一个 item 的 protocol 之后跑，不在括号内；`test_the_scope_the_guard_does_not_reach_is_the_declared_one` 用 AST 断言 `tests/e2e/` 之外没有任何 session 级 fixture，所以今天不存在这样的 teardown。**红/绿证据**：探针在子 pytest 里跑，`-p conftest` 加 `PYTHONPATH=tests` 装载**本仓库自己的** `tests/conftest.py` 作插件（不是抄一份），带 hook 五阶段全 `REFUSED`、不带 hook 五阶段全 `OPEN` —— 后者就是本测试的非空性。**实测门**：`pytest tests/unit` 3258 passed 1 skipped（基线 3235） |  | — |
| `V2-P5-032` | ~~**`tests/unit/test_offline_suite.py` 声称三个开关各有测试，`OPENALPHA_E2E` 那个不存在**~~ **已补（子解释器实测 `require_opt_in`）** | 测 | P5-031 | **本行由 P5 技术验收提出并经 grep 复核**：文件小节标题是「switch 1」「switch 3」，**没有 switch 2**；`OPENALPHA_E2E`（不含 `_RUNTIME_DIR`）在 `tests/e2e/` 之外只出现两次 —— `pyproject.toml:63` 的一条注释，以及模块文档里那句声称它被测过的话。开关是真的，测试不是。**这发生在唯一以「说出这种差异」为职责的文件里**，所以修法是补测试而不是改那句话。在**清空该变量的子解释器**里驱动 `e2e_support.require_opt_in`，断言它 `Skipped` —— 因为该函数在调用时读 `os.environ`，一个只在恰好没设该变量的机器上通过的测试，正是本文件存在要阻止的那类声明。顺带把模块文档改成记录这次落差本身 |  | — |
| `V2-P5-033` | ~~**`V2-P5-004`：运行前就加载好的 ctypes 句柄能在会话内开一个全新 socket 并把字节发出去**~~ **已声明为第八条 `KNOWN_PAPER_LIMITATIONS`（实测驱动，不可在进程内关闭）** | 技 | P5-004 | **本行由 P5 技术验收提出、在 `37c4273` 上经 loopback 复现**：`libc.socket` / `libc.connect` / `libc.send` 全部发生在 `refusing_outward_calls()` **之内**，一个审计事件都不抛，**12 字节送达**（`b'BROKER-ORDER'`）；对照组 `socket.socket` 与 `ctypes.CDLL` 在同一块内都被 `PaperPortfolioReachedOutward` 拒绝。**它严格宽于已声明的 D（向预连接 fd 写）与 E（交给另一个线程）**：不需要预先连好的描述符，也不需要预先派生的子进程，**每一次 socket 调用都在会话期间**。把 `ctypes.dlopen` 放进 `OUTWARD_AUDIT_EVENTS` 制造了「ctypes 已被覆盖」的印象 —— 被覆盖的只有*加载*。`tests/offline_guard.py` 声明了这条限制，`backtest/paper.py` 没有。**修法是声明而不是关闭**，理由写在条目里：审计钩子只能看到 CPython 选择发布的事件，经已加载库的裸系统调用什么都不发布；seccomp 一类的边界是调用方**启动进程**的性质，不是它 import 的库的性质。**测试断言的方向是「字节到达」而不是「被拒绝」**：断言拒绝就是在描述本模块没有的保证，而只调一次 `libc.socket` 然后耸肩的测试会在该调用因无关原因失败的平台上变绿。哪天有进程内机制关掉它，红的就是这条断言。`REGISTRY_ENTRY_COUNTS` 7 → 8、`DOCSTRING_TOTALS` entries 367 → 368、table_entries 297 → 298 |  | — |
| `V2-P5-034` | ~~**`KNOWN_INDEX_MEMBERSHIP_LIMITATIONS` 是唯一没有集合字面量等式的手写注册表**，九个 code 里三个在全套件中无处具名~~ **已补（membership → equality）** | 测 | P4-038 | **`test_known_limitation_registries.py` 自己的模块文档早已量过并写下这条**：「按 AST 测量，34 个里 32 个有一份成员恰为该注册表 code 集的字面集合，`KNOWN_INDEX_MEMBERSHIP_LIMITATIONS` 与派生的 `KNOWN_PANEL_LIMITATIONS` 是没有的那两个」。本行关掉可关的那一半。`test_every_named_limitation_carries_a_measurement` 用的是六条 `in codes` 成员断言覆盖九个条目 —— 正是该模块文档说的那个弱点：「用等式而不是成员，因为成员断言是可加的：它看得见被改名的 code，看不见被删除的」。没被具名的三个是 `scheduled_review_takes_effect_before_it_is_published`、`silent_truncation_at_the_response_cap`、`no_revision_history`。**验收「交换两个 code 后 7 passed」这句在本仓库实测不成立，逐条记下**：把 `weights_drift_between_publications` 换成另一注册表的 code → 被 `test_no_code_recurs_across_two_registries_except_where_it_is_declared_to` 杀；改成全新名字 → 被 code 字面量绑定杀；与 `KNOWN_PRICE_LIMITATIONS` 做**真正的对调**（两边同时换、规模与复现表都不变）→ **也被杀**，但杀它的是 `test_the_review_window_is_a_named_limitation_and_carries_its_measurement` 这个碰巧按 code 取 detail 的无关测试，**不是系统性守卫**。本行把偶然的捕获换成系统性的：现在等式先红。改完 AST 复测，40 个手写注册表**全部**有等式 |  | — |
| `V2-P5-035` | ~~**`test_surface_parity.py` 只检查名字存在、不检查路由↔方法的对应，且 WebSocket 与 Mount 完全在计数之外**~~ **已修（应答类型对应 + 键入无 `methods` 的路由对象）** | 测 | P5-013 | **本行由 P5 技术验收提出、在 `37c4273` 上逐条复现**：把 `POST /api/v1/screen` 的 SDK 方法与 `GET /api/v1/watchlist` 的对调 → **`5 passed`**；给 `create_app` 加一个 `@application.websocket(...)` → **`5 passed`**。前者因为 `test_every_named_sdk_method_and_cli_command_actually_exists` 检的是**子集**（表里每个名字都解析得到真东西），键没动、派生的缺口集合没动、每个计数都成立；后者因为 `_routes()` 跳过 `methods is None` 的对象，而那正是 `WebSocketRoute` 与 `Mount`。**两条刻意不走的路，各有实测**：① **不是命名规则** —— 两个面用相反的命名法（`report_list`/`list_reports`、`panel_gate`/`panel_clearance`），实测路由端点名与 SDK 方法名 48 行里只有 **6** 行相等；② **不是调用图** —— `OpenAlphaSDK` 是进程内的，方法直接到达能力而不经路由，两者之间没有边可走。剩下的、也正是对调真正破坏的，是**应答的类型**：声明了 response model 的路由与声明了返回注解的 SDK 方法都在说这项能力产出什么，一项能力的两个面产出同一样东西。37 个配对行里 **20** 行两侧都说得出（对调即红），**17** 行至少一侧什么都没声明（`-> JSONResponse`、`-> None`），这 17 由 `test_the_rows_this_type_check_cannot_constrain_are_counted_rather_than_skipped` **计数**而不是静默跳过 —— 数字往下走和往上走一样红。**存活的那个对调被报出来而不是藏起来**：同一路径上答同一类型的两个方法分不开（`GET`/`POST /api/v1/reports` 对调后 `9 passed`），因为没有任何类型能把「列出」和「新建」分开；那属于本文件抬头自己划的 reachability / equivalence 分界的另一侧。**WebSocket 与 Mount 现在键为 `WEBSOCKET <path>` / `MOUNT <path>`**（`mount("/")` 被 starlette 存成 `path == ""`，归一化回 `/`，否则键成带尾空格的 `MOUNT ` 像个错字）。**键入逻辑抽成 `_surface_of(application)` 供两处共用**：出货应用没有 websocket 也没有 mount，该分支经 `_route_entries` 不可达 —— 实测删掉它全文件仍绿；重写一份键入逻辑来「证明它能用」就是本文件存在要阻止的那种「同一规则的第二份陈述」。**又一个由新测试证伪的旧判断**：`_application` 起初写 `create_app(web_dir=None)` 以为足够，实测**不够** —— `web_dir` 为 `None` 时 `create_app` 仍读 `OPENALPHA_WEB_DIR`，导出该变量的开发机会量到一个**挂载过**的应用（多一行 `MOUNT /`），CI 里没有；真正起作用的是清空该变量，`test_an_exported_web_dir_does_not_change_the_surface_this_file_counts` 就是证伪它的那条测试。**变异实测 7 个杀 6 个**，唯一存活的是上面具名报出的同路径对调 |  | — |
| `V2-P5-036` | ~~**冲突标记闸门把 7 字符的 Markdown setext 下划线当成冲突分隔符，同时看不见 `\|\|\|\|\|\|\|` 和无标签的裸标记**~~ **已修（三种明确标记 + 分隔符只在有明确标记的文件里算）** | 测 | P4-* | **本行由 P5 技术验收提出、逐条复现**：原模式 `^(?:<<<<<<< \|=======$\|>>>>>>> )` 下 —— `Summary` + `=======`（7 个等号的 setext H1 下划线）**被判为冲突标记**（误报），9 个等号则漏过；`\|\|\|\|\|\|\| merged common ancestors` **漏**；无标签的 `<<<<<<<` 与 `>>>>>>>` **两个都漏**。**尾随空格是承重的而它不该是**：`git` 有标签时写标签、没有时写裸标记（`merge-file` 与若干编辑器的解决器就是这么写的），于是整个问题里最没有歧义的两个字符串因为少一个空格被漏掉。**`\|\|\|\|\|\|\|` 完全不在名单上**：那是 `merge.conflictStyle = diff3`/`zdiff3` 写的 base 段，一次半解决的 diff3 冲突可以只剩它。**`=======` 移到 `AMBIGUOUS_SEPARATOR`**：它是唯一一个同时也是正常文本的标记，只在文件里**另有**一个明确标记时才算 —— 因为 `git` 从不单独写分隔符（它写在 `<<<<<<<` 和 `>>>>>>>` 之间），而 setext 下划线永远是单独的。让正确的 Markdown 变成红构建的规则，在一个用 Markdown 写成的仓库里是要付代价的；今天树里没有触发它，这是它一直没被付账的唯一原因。**放弃的方向被声明而不是被发现**：一次手工删掉首尾标记只留分隔符的解决不会被抓 —— 那是人在编辑文件，本闸门本来就无法推理，它存在是为了「没人看过的标记」。十二种字符串逐一参数化驱动（含 `cat <<EOF`、行内引用的 `` `<<<<<<< HEAD` ``、6 个和 8 个尖括号），另加一条把冲突文档与 setext 文档并排分开的测试 |  | — |
| `V2-P5-037` | ~~**`testDiscipline.test.ts` 只 `stat` 文件名，且它的下限比出货的 ratchet 低 3–5 点**~~ **已修（导入+断言的静态检查 + 下限抬到出货值）** | 测 | P5-020 | **本行由 P5 技术验收提出、在 `37c4273` 上复现两处**：① 新增一个 `probeDrift.ts` 会红并点名它自己，但补一个只有 `it("works", () => {})`、**既不 import 也不断言**的 `probeDrift.test.ts` 就 **`6 passed`** —— 满足文件名检查的最便宜办法是一个什么都不满足的文件；② 它的下限写着 `≥90/84/89/91`，而 `vite.config.ts` 出货的是 **93/87/94/94**，于是「只能往上」可以被往回走三、三、五、三点而本测试仍绿 —— **下限之下还有一层下限**，正是一个 ratchet 唯一不能有的东西。**修法**：(a) 每个同目录测试必须 import 它具名的那个模块**且**含至少一个 `expect(`。两条都是静态性质，**都不是证明**（一个测试可以 import 了主体然后断言无关的东西）；它们去掉的是**空文件**，而那正是文件名检查唯一会招来的东西 —— 这是 `test_known_limitation_registries.py` 那句「字面量必须落在可执行的测试代码里」的前端形态，弱在同一个已声明的地方。写下时 24 个同目录测试**全部**已满足，没有任何一个被祖父条款放行。(b) 下限改为出货的 93/87/94/94。抬 config 就得在同一个提交里抬这里，这是代价，也是 ratchet 有意义的代价。**红/绿实测**：基线 `7 passed`；只加模块 → 红并点名 `probeDrift.ts`；再加空测试 → 红在 `requires each co-located test to import its subject and assert something`；把 statements 93 → 92 → 红在 `keeps the thresholds block at the ratchet the tree actually stands on`；还原 → `7 passed`。`pnpm test` 439 passed / 30 files（基线 438/30），`pnpm lint` 与 `tsc -b` 干净，覆盖率 93.19 / 87.52 / 94.41 / 94.36 未动 |  | T14 |
| `V2-P5-038` | **`test_feature_ledger_test_tree.py` 看不见同目录内的行对调，89/185 行不在 AST 覆盖内** —— 修前置（fix-forward），本行只把它记成一行而不是留在散文里 | 测 | P5-023 | **该模块自己的文档已经声明并推理过这条残余**：「这个形状仍然抓不到的是*一个目录内部的对调*（删掉具名 `a.py` 的行、加一条具名同目录 `b.py` 的），计数与总数都不变。该残余对 `acceptance_kind="pytest"` 的行由 `_validate_pytest_acceptance` 覆盖（AST 检查具名测试函数存在），其余接受」。**本行补的是量**：185 行里 `pytest` 96、`legacy-prose` 85、`ci-job` 1、`not-applicable` 3 —— 即 **89 行**在 AST 覆盖之外，接近一半。**判为 fix-forward 而不是就地关闭，理由是两条关法都比缺陷贵**：(a) 把 85 条 `legacy-prose` 迁成 `pytest` 接受，是一次内容迁移而不是守卫改动；(b) 用一份 185 项的路径集合字面量做等式，会在**每一次**合法的 ledger 编辑上报红，而该模块正是权衡过这一点才选了按目录计数（`--check` 已把 CSV→summary→md 逐字节钉住）。两条都需要一次单独的决定，不该塞进一次守卫扫尾。**没有静默留下**。**【2026-08-29：加了棘轮，债只许降】** `test_the_unvalidated_acceptance_rows_are_the_number_recorded_above` 把 `legacy-prose` 行数**等式钉在 85**。选它而不是那两条被否的关法，因为第三条路更便宜也更准：`summary.json` 里的 `legacy_acceptance_rows` 虽已被 `--check` 逐字节钉住，但**图钉允许涨和允许降一样容易**——顺手更新产物就过了；棘轮不允许。新功能必须点名一个测试，存量债只能在偿还它的那次提交里把常数往下调。**上限不是 0，且写进了 docstring**：`OA-IFACE-006`/`OA-IFACE-007`/`OA-OPS-002` 指向 `web/src/App.test.tsx` 与 `web/e2e/golden-flow.spec.ts`，没有任何 `pytest` node id 能寻址它们——要么等第四种接受类型、要么永远是散文，棘轮对此不预判。**另澄清本行原文写宽了的一处**：`build_feature_coverage._load` 早已对**每一行**（不分接受类型）检查证据路径存在，**85/85 条 legacy-prose 的路径今天就被验着**，含那三条 `web/` 的（实测：把 `OA-IFACE-006` 指向不存在的 `App.test.NOPE.tsx`，`--check` 当场抛错）；真正没被机械检验的只有"证据是一个文件而不是一个具名测试函数"这一条。按 TDD 做：先加第 86 行制造违规、再写测试、**看它红**，再恢复看它绿。**独立验收 agent 随后揪出三处**，均已改：(1) 覆盖数是 85/85 不是 82/85——我那个 82 量的是"多少条点名了 `tests/*.py`"，却当成覆盖率写了；(2) 失败消息**不**点名闯入者（测试自己的 docstring 正写着这是有意的），我的叙述与代码自相矛盾；(3) **设计上的**：`<=` 会让余量在偿还后静默累积——迁走 10 行、计数变 75 而上限仍 85，于是可以无声再加 10 条。**一道随着你做好事而悄悄变弱的守卫**，正是本轮反复拒绝的形状，已改成 `==`：增长红、偿还也红直到常数在同一提交里跟着降。**同时把它抓不到的写进 docstring**：同一提交里"迁走一条 + 新增一条"净零仍会通过，且常数本身可被人改高——守卫买到的是**这两件事都必须成为受审查文件里可见的、有意的改动**，而不是静默漂移 |  | — |
| `V2-P5-039` | ~~**`V2-P5-007` 的 family-size 守卫在分段报告里分不开两个答案**：8 个夹具全部满足 `declared_family_size == len(cohorts)`~~ **已补（一条判别性夹具）** | 测 | P5-007, P5-009 | **本行由 P5 技术验收提出、在 `37c4273` 上复现**：代码是对的（没有任何地方从 `len(cohorts)` 重算 `family_size`），但把 `family_size=request.declared_family_size` 改成 `len(cohorts)`、把 `/(request.declared_family_size*penalty)` 改成 `/(len(cohorts)*penalty)`，`tests/unit/backtest/test_segmented_reporting.py` **28 passed（基线 28）**、`tests/integration/test_segmented_reporting_interfaces.py` **13 passed（基线 13）** —— 两个变异体都存活。同一变异在 `multiple_testing.py` 被杀 7 次、在 `outcome_statistics.py` 被杀 1 次，因为那两处有判别性夹具。`segmented_reporting.py:655` 的 `validate_there_is_exactly_one_family` 在 `declared_family_size=20` 上确实会抛，**守卫是有能力的 —— 只是没人喂过 `declared > len(cohorts)` 的用例**。**一条夹具关掉它**：`WIDE` 七个桶、声明 14。`family_size` 必须是 14 而不是 7；`most_permissive` 从 `7*0.125/7` 减半到 `7*0.125/14 = 0.0625`，落到四观测桶的下限 `0.125` **之下** —— 于是 `test_a_bucket_whose_floor_lands_exactly_on_the_line_is_admitted_and_not_missed` 里那两个「正好在线上」的桶，在调用方**真正跑过**的那个族里是不可能拒绝的，`hypotheses_that_could_ever_reject` 从 4 变 0。声明你没发表的假设，代价就是发表你确实发表的那些时失去分辨率，而那正是声明它们的全部意义。**变异实测 3 杀 3**（含把 `<` 改成 `<=`）。顺带更正 `segmented_reporting.py:43` 引用的不存在测试名 `test_four_axes_are_one_family_and_not_four`（真名是 `..._three_axes_..._three`） |  | — |
| `V2-P5-040` | ~~**`test_source_cited_tests.py` 只审带路径的引用，占多数的裸引用（109 条）一条没审**~~ **已修（裸具名 + 减去 `src/` 自己的符号）** | 测 | P4-* | **由 `V2-P5-039` 更正 `segmented_reporting.py:43` 时发现**：那条引用是 `` `test_four_axes_are_one_family_and_not_four` ``，一个不存在的名字，而全库唯一负责审引用的模块**看不见它** —— 它只认 `tests/<path>.py::<name>` 这一种写法。**这与它自己被写来抓的缺陷同形**：一份手工维护的对应关系，只在有人碰巧那样拼写的实例上被审。实测：带路径的 60 余条被审，**裸的 109 条被读过去**。**规则不是「反引号里以 `test_` 开头的标识符」**，因为 `test_day_count`、`test_set`、`test_sections` 是出货模型的**字段**（`backtest/walk_forward.py`、`domain/alpha_model.py`），散文提到字段就是在提字段；所以**在 `src/` 自己声明过的裸名不读作引用**。两侧都从树里推导，没有任何人维护的名单 —— 那正是本文件存在要有的性质。减去 `src/` 符号后 109 条待审，**当天就有两条已经失效**：`panel_factors.py:5980` 的 `test_a_write_that_would_drop_a_stored_build_is_refused`（真名 `test_the_refused_merge_leaves_the_partition_exactly_as_it_found_it`，其文档字面写着本处那句「一次拒绝什么都不改变」），以及 `storage/migrations.py:36` 的 `test_new_database_lands_on_baseline_and_defers_the_demo_migration`（真名 `..._applies_baseline_and_validation_results_then_...`，改写成带路径的换行形式，因为名字有 88 字符）。**又一处「不做什么」由测量决定**：第一版会先剥掉带路径的引用以免重复计数，实测**剥掉零条** —— 两个模式不可能同时匹配一段（带路径的名字前面是 `::` 的第二个冒号，而裸模式要求那里是反引号）。既然是空操作就删掉，并把这个理由本身写成 `test_the_two_citation_forms_cannot_both_match_one_span`。第一版的去重测试断言的是**按名字**的划分，实测**错了**：`test_the_engines_period_selection_is_the_domains_filing_for` 在 `domain/factor.py:673` 带路径、在 `panel_factors.py:4926` 裸写，两个集合里都合法出现，按名字的划分根本不存在 —— 该性质是**按出现**的，测试改在构造的句子上驱动 |  | — |
| `V2-P5-041` | ~~**四个页面都把 API 拒绝渲染成裸 JSON**：服务端写的那句「先构建它（`openalpha panel build …`）」被埋在标点里~~ **已修（在客户端唯一知道线格式的那层解包）** | 技 | P5-014, P5-017, P5-018 | **本行由 P5 产品验收提出，在 `94a0af2` 上以真实服务器复现**：`openalpha serve` 指向临时 runtime 目录、`curl` 实测三种体，`web/src/api/client.ts:21-22` 的 `throw new Error(await response.text())` 让四个页面的 `error.message` 原样上屏。**状态码不是判别式，这点被实测证伪了原始工单的措辞**：工单称三种体皆为 `422`，而 panel 面的 `panel_unreadable` 实测是 **`409`**（`docs/api/http.md` 第 385 行本就写着「`409` 携带两种体」），且同一 `409` 还承载没有 `detail` 键的 gate 判决 —— 所以新读法只看体、只在无体可读时才退回状态码。**四种体各有一行**（`{reason, message}` 对象含 `declared_ceiling_exceeded` 变体、pydantic 字段错误**列表**、裸字符串 `detail`、非 JSON 文本），**列表不被压平**：`loc` 是唯一说明「哪个字段」的东西，`V2-P4-051` 把这个形状钉住正是为了让调用方分得开 `dataset` 与 `year`，故每条渲染成 `body.evidence[1].payload.quality_flags[1]：<msg>`，截断哨兵 `{"loc": [], "type": "errors_elided"}` 渲染成裸句而不是前面挂个空路径。**过程中抓到一个「按构造为真」的断言**：新测试初版写作 `rejects.toThrow(<那句话>)`，在**未修复**的客户端上直接变绿 —— 因为裸 JSON blob **包含**那句话作为子串，子串匹配器分不开「那句话」和「那句话裹着 JSON 标点」，而后者正是本缺陷；改成 `toBe` 全等后才见红。新增 `web/src/api/refusal.ts` + 17 条同址测试，`client.test.ts` 11 → 13 |  | S48 |
| `V2-P5-042` | ~~**每个因子实验页都显示 `所需字段：[object Object]`，而漂移检测被豁免到看不见它**~~ **已修（类型是缺陷本身；豁免收窄并指向一个真在跑的检查）** | 技 | P3-015, P5-017 | **在真实服务器上实测而非采信转述**：seed 一个真实密封实验到临时 runtime 目录、`openalpha serve` + `curl`，`GET /api/v1/factors/experiments/fxp_3c31ffda36fe1d75227eff70` 答 `"required_fields": [{"column": "close", "dataset": "daily"}]`，而 `web/src/types.ts:399` 写的是 `string[]`。**三个守卫同时失效，且失效方式各不相同**：(a) `typesContractDrift.test.ts` 把 `FactorExperimentEnvelope` 整个列进 `INTENTIONALLY_UNMAPPED_TYPES`，**豁免是按类型的，于是里面每个字段都被豁免**；(b) `FactorExperimentPanel.test.tsx` **确实**断言了这个字段（`/close、adj_factor/`），但 `fixtures.ts:378` 供的是 `["close","adj_factor"]` —— 夹具是照着**错的类型**手写的，所以断言按构造为真；(c) CLI 一直是对的（`factor_view._decides` 打印 `reads daily.close`），于是只读 Python 的人看不见分歧。**先把生产缺陷复现进单测**：夹具改成服务器真实形状后，面板当场渲染出 `[object Object]、[object Object]`。**修法是类型、不是渲染**：`{dataset, column}[]`，渲染改为 `dataset.column`（与 CLI 同一拼写，因为 `close` 在多个数据集里都存在，光有列名有歧义）。**豁免问题的取舍**：签入 JSON schema 被**否决** —— 该路由不声明 `response_model`，返回的是 `JSONResponse(content=experiment_view(...))`，Python 对象**就是**契约，手写一份 schema 摆在旁边等于再造**第二份**手工镜像，能以 `types.ts` 刚刚犯的同一种方式漂走，拿一个镜像校验另一个镜像只能证明两个人犯了同一个错；仅收窄豁免措辞则什么都不校验。故取**活服务器契约测试**，并沿用 `test_spa_addressability.py` 早已确立的先例（一个从 Python 读 TypeScript 源文件、再对真实 app 校验的单测）：新增 `tests/unit/test_web_factor_definition_mirror.py`，逐字段比对 `FactorDefinition.model_json_schema()` —— schema 由 pydantic **生成**，没有第三份东西要维护；0.02s，留在 `tests/unit`，`pnpm test` 保持离线。**变异实测 2 杀 2**：把 `required_fields` 改回 `string[]` 见红并具名该字段（`'string[]' != 'object[]'`），把 `family` 改回 `string` 见红具名 `family`。**顺带发现第三处同源缺陷**：`family` 镜像成 `string`，于是两个夹具带着 `price_momentum` / `price_reversal` —— **没有任何服务器能发出的值**（模型只声明五个，实测线上是 `momentum_reversal`）；改成字面量联合后 `tsc` 当场拒绝两个杜撰值。**豁免措辞收窄**为具名「`spec.ic.definition` 这一块由某模块校验、其余仍不校验」，并新增一条守卫：豁免理由里指向的仓库文件必须真实存在（实测把它指向不存在的模块即见红并具名类型与文件）—— 否则「在别处校验了」这句散文会随着重命名悄悄变成谎话 |  | S48 |
| `V2-P5-043` | ~~**`evidence build` 与 `research run` 的四种拒绝全是裸 Python traceback**，真正的话只在最后一行~~ **已修（四条一行拒绝，各自点名出路）** | 技 | P5-013 | **本行由最终产品验收提出、在 `94a0af2` 上逐条复现**（`.venv/bin/openalpha`，非内部导入）。四例，四个 rich traceback，退出码都是 1：① 缺列的 CSV → `ProviderFailure: Cannot read ev_bad.csv: 'summary'` —— 那是 `KeyError` 的 `repr`，**点了一个缺列而没点契约**，把 `summary` 补上再跑只会换成 `event_time`；现在 `providers/file.py::REQUIRED_COLUMNS` 把八列契约、`payload`/`payload_json` 两种拼法、可选的 `source_uri`，以及**这一行实际带了哪些列**一起说出来（末一项才是把表头笔误 `Summary` 从谜团变成一眼可读的差集）。为此 `MissingColumnsError` 是 `LookupError` 子类而非 `KeyError`：`KeyError.__str__` 就是 `repr()`，正是那对引号的来源；`fetch()` 的子句由 `KeyError` 放宽到其基类 `LookupError`，**原先能翻译的每一种故障仍然翻译**。② `kind=filing` → `ValueError: unsupported evidence kind: filing`，**七个受支持的 kind 一个没点**；改为从 `_NORMALIZERS` 自己读出，第八个 kind 加进去时消息同一个提交里跟上。③ `research run` 收到 CSV → `json.loads` 的 traceback，`JSONDecodeError` 说的是「第 1 列」，描述的是文件而不是错误 —— 而这个错误很自然，两条命令各收一个路径、只有一条收 CSV；现在点名本参数要的格式和产出它的命令。④ 跨 subject 的证据 → pydantic 三行校验报告包住一句话，`--subject` 全程未出现；`validate_evidence` 现在点名 payload 里**另外**带了哪些 subject（分号前那句原样保留，因为 `sdk.py::export_report` 的文档逐字引用它作为自己收窄的依据），`_one_line` 把它渲染成一句话。**捕获是逐语句的，不是一个大 `try`** —— 理由就是 `research_run` 文档里已有的那句：一个宽 `try` 会让其中任一个替其余的回答。**顺带一个未被验收测到、同一语句上的**：`--as-of` 默认 `""`，省略它就到了 `datetime.fromisoformat("")`；照 `_panel_as_of` 的措辞补上。退出码全部仍是 1，已按 1 分支的 CI 作业不受影响 |  | 27 |
| `V2-P5-044` | ~~**`research run --run-id` 默认字面量 `local-run`，第二个 subject 必然撞车**~~ **已修（默认保持固定，拒绝点名 `--run-id` 并说明为何固定）** | 技 | P5-043 | **本行由最终产品验收提出、在 `94a0af2` 上复现**：研究四个名字（入围闸门允许的最少数量）得到 `rc=0` 然后 **`rc=1, rc=1, rc=1`**，`RunConflictError: run_id conflicts with an immutable request: local-run`，traceback 里 `--run-id` 一次没出现。**撞上它是正路而不是边角**：入围闸门只在足够多的名字被研究过时才收下一份 shortlist，所以连着研究几个 subject 就是这条命令的日常用法。**在「让默认唯一」与「保持固定并点名旗标」之间是选后者，理由写在 `RUN_ID_DEFAULT` 的文档里**：`run_id` 不是运行的标签而是运行身份的一部分 —— `RunManifest` 存它，`request_digest` 在它上面计算，`refuse_a_restated_request` 比对两者，好让一个 id 永不指向两个不同的请求；一个每次调用都不同的默认会让**同一条命令跑两次产生两次运行**，那正是本仓库处处失败关闭的性质：一次无法被认出是重跑的重跑，就是一次无法复现的重跑。于是撞车是真实行为而非缺陷，`V2-P5-044` 修的是拒绝从不这么说。`RUN_ID_REMEDY` 刻意**不**像 `NO_CALENDAR_REMEDY` 那样三面共用：SDK 与 HTTP 两面根本没有默认 `run_id`（`ResearchRunRequest.run_id` 是无默认的必填字段），这次撞车只有命令行够得着，也只有命令行有旗标可点。`--run-id` 同时补上 `_RUN_ID_HELP`，因为读 `--help` 的人是最便宜的告知位置 |  | 27 |
| `V2-P5-045` | ~~**`date_gap` 的拒绝一个旗标都不点，而修法是 `--as-of`**~~ **已修（`DATE_GAP_REMEDY`，三面各自拼写）** | 技 | P1-012 | **本行由最终产品验收提出、在 `94a0af2` 上复现**：`--as-of` 正确地默认为「现在」，所以一份为 1 月建的面板在 8 月被问时必然失败 —— `daily cannot be read at 2026-08-26T...: ['date_gap']; 157 required date(s) are absent from daily, starting at 2026-01-19`。这句话**准确且没用**：它描述的是仓库，而仓库没有毛病；日期错了的是**问题**。补上 `--as-of` 后同一条命令 `rc=0`。**两条出路都点名，因为两条都真且不可互换**：当面板是有意的历史面板时，改问题的日期才对；当它确实落后了，取数才对 —— 只提后者会把一个在重放 1 月的人白白送去联网（那正是 `V2-P5-046` 记下的、`subject_missing` 当时犯的错）。措辞按 `NO_CALENDAR_REMEDY` 的规矩三面各自拼写（`--as-of` / `as_of` / `as_of`），因为这条 detail 由 `V2-P1-016` 的 HTTP 面、SDK 与 `panel doctor` **逐字**共用，只写命令行拼法会在三面里错两面。**测量更正一处**：验收把这条描述为一条 stderr 拒绝，实测它是 `panel doctor` **stdout** 上的一行 `BLOCKING daily date_gap: ...`（且该命令的 `--json` 本来就正常输出 21,382 字节）—— 引文里那种 `daily cannot be read at ...` 的形状来自 `panel_ingest` 的读路径而非本命令。缺的东西一样，位置不一样 |  | 27 |
| `V2-P5-046` | ~~**`subject_missing` 只打个数、从不说是哪个 subject，而它给的两条出路都是错的**~~ **已修（点名缺失的 subject + `--exchange` + 库里实际有什么）** | 技 | P1-012 | **本行由最终产品验收提出、在 `94a0af2` 上复现**：`openalpha panel doctor --dataset daily --year 2026 --as-of 2026-01-17T12:00:00+08:00` → `the SSE calendar cannot be read at ...: ['subject_missing']; 1 required subject(s) are absent from trade_cal. Build it first (...), or state on the record that this run has no calendar (--no-calendar ...)`。但 `trade_cal` **已经建好且健康** —— 它装的是 `SZSE`。重建会去取 SSE（付费、慢），`--no-calendar` 是把这项检查丢掉；真正的修法是 `--exchange SZSE`，同一条命令就收，且返回 `rc=0 READY daily`。**`missing_items` 一直握着答案，人类输出只打了它的长度**。两处改动：① `subject_gap_issue` 的 detail 现在点名缺的 subject（`SAMPLE_LIMIT=6` 封顶后计数 —— `required_subjects` 在指数成分/全域读上是一个截面，无上限的列表会把几千个代码塞进终端里一行）；② `panel_view._calendar_remedy` 在**且仅在**库能支撑时给出更窄的出路：cause 是 `subject_missing`（分区在，交易所不在）**且**普查点得出至少一个别的交易所。其余情形 —— 分区真的没有、catalog 坏了、库里只有已经问过的那一个 —— 原样保留 `NO_CALENDAR_REMEDY`，因为那时候「建它」确实就是答案，点 `--exchange` 只会把人送到一个后面填不出东西的旗标前。普查走 `panel_ingest.stored_calendar_exchanges` 跨缝取得而不是在 `panel_view` 里读：该模块在 `RESEARCH_PLANE_DATASETS` 里的行是 `named=frozenset()`（够得着十五个数据集、一个都不自己点名，因为它渲染的是别人的答案），本地一句 `read_coverage(TRADING_CALENDAR_DATASET, ...)` 会为了一句话把那条架构声明变成假的 |  | 27 |
| `V2-P5-047` | ~~**CLI 上 `validation` 与 `report` 都只有读面、没有写面，而两份 README 反复宣称「三个面等价」**~~ **已修（两条写命令 + 拒绝逐字节相等）** | 产 | 013, 008, 022 | **由最终产品验收从命令树与 52 条路由表测出**：`openalpha validation` 只有 `statistics`／`segmented`，`openalpha report` 只有 `export`，写面只存在于 `POST /api/v1/backtests/validate`／`POST /api/v1/reports` 与 `sdk.validate_outcome`／`sdk.create_report`。于是一个只用命令行的操作者**永远填不满这两个 store**，而 `validation statistics` 对没有存量的 signal 是**具名拒绝**（`V2-P5-007` 的规矩）—— 正确，且在没有可达写面时是**唯一**答案。**两条诚实出路之间由测量而非偏好决定**：`OutcomeApiRequest.research` 与 `ReportApiRequest.research` **都是 `dict[str, Any]`**，故两个写面都**不需要**那种进程内对象 —— 而进程内签名正是 `SDK_ONLY` 给 `construct_portfolio_from_ranking` 的理由，即真正不可能有第二个面的那一类；且这串字节**已有出货命令在打**：`openalpha research run` 打的就是 `result.model_dump_json()`。故 `research run > run.json → validation record → validation statistics` 这条环**整个闭合在终端里**，写面遂建而非改文案。**入参取文件而非一堆旗标**，`validation segmented --plan` 的理由：一次观测的窗口、两个价格、基准与成本**只有放在一起才有意义**，逐旗标拼出的半份观测会逼这条命令为每个缺口发明一个默认值，而这里每个默认值都是一句关于市场的断言。**`parse_research_result` 与拒绝文案迁到新的中立顶层模块 `research_result_io`**：`cli.py` 对 `openalpha_cn.api` **本来没有任何 import 边**（`serve` 靠字符串 `"openalpha_cn.api.app:app"` 找 uvicorn），从路由模块取解析器会把 FastAPI 装到每一次 `openalpha version` 背后；现在**一个函数为三个面写同一句拒绝**，故「逐字节相等」是结构性的而非两位维护者要各自记住的约定 | 集成：`tests/integration/test_validation_and_report_writer_faces.py` —— CLI `--json` stdout 与路由 `200` 正文逐字节相等（两条命令各一）；CLI stderr 与 `detail.message` 逐字节相等（篡改 `signal_id`，两条命令各一）；`validation record` 写入后 `validation statistics` 能真的聚合（**两次观测**，因为 `MINIMUM_INTERVAL_SAMPLE_SIZE` 是 2 —— 写这条测试时实测撞到）；`report create` 写入后 `report export` 能按 id 取回。单元：`test_surface_parity.py` 由 `cli_commands` 33 → 35 逐条对账 | S13 |
| `V2-P5-048` | ~~**两份 README 与 `docs/` 里印的命令行从没被跑过；因子工作流第 3、4 步并列而实际跑不通**~~ **已修（文档命令逐字执行 + 四个缺陷）** | 产 | P4-094, P4-100 | **`V2-P4-094` 一个频道之外已经做过这个形状**（把 `openalpha model --help` 里的命令行解析出来逐字跑），理由是「一条没跑过的 `--help` 示例和其他任何断言一样只是一个声明」。文档是同一种声明，且从没被跑过。**四个缺陷，其中三个是同一个失败**：一个示例的**来源被修好、拷贝没有**。(1) **第 3、4 步并列而不相连**：`--tier` 指的是**写到哪一档**而不是**够到哪一档**，第 3 步（`--tier processed`）exit 0，第 4 步（带 `--neutralization`）随即 exit 1 `No neutralized partition of this factor is registered in this panel at all` —— 第 3 步要跑**两次**，一档一次，此前无人写下；第 4 步还整个省掉了 `--as-of`，于是读墙钟，成败取决于哪天跑。(2) **上面那句拒绝印的补救对任何一档都跑不通**，且**三个缺旗标是一个一个跑出来的**：`--as-of` 必填故三档都 exit 2；接着 `--max-staleness-days` 让三档都 exit 3 —— 它是**缺席即拒**而非 Click 的 `required`，故任何读 `Parameter.required` 的检查都看不见它；再接着 `processed`／`neutralized` 各自在 `--transform`／`--neutralization` 上 exit 3。`FACTOR_TIER_BUILD_ARGUMENTS` 现按档带上必填形状，值是尖括号占位符（一个时刻、一个中性化都是关于研究的**声明**，消息不能替读者发明），并按 `FACTOR_TIER_DATASETS` 的先例在 `factor_view` 与 `shortlist_view` 各存一份、由测试钉住相等。(3) **`model evaluate`／`daily-run` 在三处仍是 `V2-P4-094` 自己那两条坏示例**（`README.md`、`README.en.md`、`docs/HANDOFF_CURRENT.md`）—— 那一行改了 `--help` 打的两条，拷贝原样留着。(4) **`docs/HANDOFF_CURRENT.md` 仍带 `--waive-max-staleness`**，`V2-P4-100` 实测它 exit 1 后已从 `--help` 去掉。**这个测试看不见什么是逐条声明的**：付费 provider（`panel build` —— 本仓**没有任何离线拿到面板的路径**，这正是该测试自己的 fixture 要绕开的东西）、起服务（`serve`）、本机凭据（`doctor`）、参数是示意占位地址的行，以及答案取决于墙钟因而 exit code 度量的是 fixture 建到哪天而非那一行的命令（`panel doctor`／`data-check`）。**它也看不见散文** —— 一条跑得通而旁边段落说着另一回事的命令对它不可见，而 `V2-P5-047` 更正的正是那个形状 | 集成：`tests/integration/test_documented_command_lines.py` —— 50 条文档命令行全部按**实时参数表**校验选项存在（不用 `parse_args`，见下）；可达子集逐字执行并要求 exit 0；`EXECUTED`／`NOT_EXECUTED` 是**等式**故新增文档命令必须二选一落座；拒绝里印的补救按实时 `Parameter.required` 校验并**填入占位符真跑**。**首版自身有一个假绿并被测出**：`isinstance(x, click.Group)` 在本树**恒为 False**（Typer 0.20 的 `typer._click.core.Command` 不继承 `click.Group`），故解析循环从不进入、50 条断言**全部空转** —— 由等式测试报出 `{''}` 才发现，现改为按 `hasattr(..., "commands")` 鸭子判定 | S13 |
| `V2-P5-049` | ~~**`openalpha --version` exit 2 `No such option: --version`**~~ **已修（eager 根回调，与 `version` 逐字节相等）** | 产 | — | 由最终产品验收测出，小但**是一个人打的第一条命令**，而这个 build 的 `openalpha version` 印得好好的。现为 eager 根回调、复用 `version` 自己那一句 f-string，并**断言两者 stdout 逐字节相等**（一个 build 号的两种打法正是第二种拼写会招来的漂移）。**`is_eager=True` 是一个被上报的存活突变体，而被它取代的那句话正是本行自己写的**：初版说它「承重」，理由是 `no_args_is_help=True` 会让非 eager 的旗标被解析、找不到子命令、再以第二个理由 exit 2。**删掉它，整个 CLI 套件全绿，装好的二进制照样打印版本号并 exit 0** —— 根回调自己的参数在 group 去找子命令**之前**就被处理，故此处没有任何东西需要 eager 抢先。旗标按 Click 的惯用写法保留，而「有测试钉住它」这句话不保留。**回调命名为 `_root_options` 而非 Typer 惯用的 `main`**：本模块已把 `main` 绑给 `console_scripts` 入口，两者同名让入口**静默覆盖**了回调，只因装饰器已先捕获函数对象才没坏，而 `ruff` 一直在报 `F811` | 单元：`tests/unit/test_cli.py` —— `--version` 与 `version` stdout 逐字节相等；`--version` exit 0 且不打 `Usage:`（eager 的那条） | — |
| `V2-P5-050` | ~~**`test_a_cross_section_earlier_than_the_registrys_own_availability_is_refused` 断言的墙已经倒了；该测试的前提被真实行证伪**~~ **已改写（改为断言「按行扣留且扣留是对的」）** | 技 | P4-076, P4-100 | **本行由本仓首次付费 e2e 全量运行测出，并回答了 `test_shortlist_workflow_online.py` 自己在文档串里留下的那个问题**。该文档串写着：这条测试「登记处才是拒绝方」的读法早于 `V2-P4-076`，而 `076` 之后的面板会让它 **skip 而不是 fail**，所以「拿一份新面板重跑这套件，才能定下现在是哪个数据集先答话」；`V2-P4-100` 当时选择修那句话而不是声称那次测量。**它没有 skip，也没有拒绝**：面板持有 155 个早于 `registry_knowable_from` 的交易日，在其中最新的一个（2026-08-24T17:30+08:00，登记处分区自身可得时刻是 2026-08-25T00:00+08:00）上 `factor build` **exit 0、算出 60 个 subject**。**答案是「没有任何一个数据集先答话」**：全年 157 个交易日里**抽样 11 个**（最新的六个，加一路回到年内第二个交易日的散点）**十一个全部可建**，其中 8 个端到端可入围，每一个都按自己那天定价、可上市集合随日历单调增长（1 月 5,470 → 8 月 5,550）。**其余 146 个没有扫，所以本行的断言是「抽样跨度内任何位置都没有墙」而不是「每个交易日都可入围」**；另外 3 个不可入围的失败在 `namechange` 语料的**年内覆盖**（某证券已知的第一个名字在年中才生效）而非任何可得性墙，是语料范围问题不是时点问题。**且扣留是对的，不是漏网的前视**：那个把 `registry_knowable_from` 顶上去的证券（2026-08-25 上市、按自身午夜可得）在早时刻的读里**不在**、在晚时刻**在**且可上市集合恰好长 1；`listed_on` 对快照之后的任何一天**抛错而不作答**（`UniverseHorizonError`），所以快照不会被误当成常驻成分；写下的观测 `input_session_first/last` 是 as_of 当天及之前那两个交易日，六十个值与那两天收盘价之比在**浮点相等**意义下逐个相等（并与当天存下的 `pct_chg` 在其四位小数精度内一致）。`adj_factor` 是 `076` 有意留在 `read_if_ready` 上的那一面（`V2-P4-086` 点了移动它所需的两处修改），在早时刻**确实仍然拒绝**，本行把它作为「唯一还站着的墙」写进断言；它没挡住这次构建，是因为 `factor build --tier raw` 与 `load_shortlist_cross_section` 在这条路上根本够不到它。**顺带记下 fixture 的保守性**：`FETCHED_BLOCKING_DATASETS` 与 `_prediction_instants` 仍按分区可得时刻推导可入围时刻，其中三个已不再设卡 —— 现在这是**保守**而非必要，收窄它能让本模块比较两个真实交易日而不是一天里的两个时刻，本次不动，理由是这轮的差事是取测量而不是就着测量重建 fixture |  | S13 |
| `V2-P5-051` | ~~**`test_a_correction_published_after_the_read_serves_neither_version` 点名 `not_yet_knowable`，而该规则已够不到这个数据集**~~ **已改写（改为断言逐事件日普查的拒绝）** | 技 | P4-034, P4-076 | **本行由本仓首次付费 e2e 全量运行测出**。该测试注入一条与原行矛盾、`available_time` 晚于读时刻的更正行，然后期待 `PanelStorageError` 匹配 `not_yet_knowable`；实际拒绝来自 `V2-P4-034` 的**逐事件日普查**：`suspend_d year=2026 cannot be read at ...: its date census counts 32 row(s) dated 2026-04-28 ... and the visible slice carries 31 of them (1 row(s) withheld in all)`。**判定是「普查是对的」而不是「普查抢跑遮住了本该先到的规则」，理由是后者已不存在**：`V2-P4-076` 把 `load_suspensions` 从 `read_if_ready` 移到了 `_read_visible_event_dated_rows`（`panel_ingest.load_suspensions` 的文档串就是那次移动的记录，理由是最新交易日上的一次停牌会让该年每一个更早的截面都不可入围）。**注入之前实测**：`load_suspensions` 在 2026-08-24T17:30+08:00 正常作答，而 `suspend_d` 2026 分区的 `max_available_time` 是两天之后的 2026-08-26T16:30+08:00 —— 在 `read_if_ready` 下这是教科书式的 `not_yet_knowable`，它没有触发。**而普查恰好是这次注入的正确答案而非顺序上的巧合**：`suspend_d` 是 `ClockStrategy.daily_close`，一条 2026-04-28 的停牌按那天 16:30 可得，不可能在四个月后才变得可得；普查看到分区在该事件日声称 32 行而可见切片只有 31 行，于是按 `load_suspensions` 自己的话拒绝 ——「短掉的那份答案与那行根本不存在无法区分」。**该测试原本另外两条断言原样成立**（晚时刻的 `SuspensionError` 「nothing in the corpus decides between them」、`panel doctor` 的 `domain_rebuild_refused` 点名该证券），说明失效的只是那一句规则名。新断言钉住普查句子、**注入行自己的事件日**、以及「共扣留 1 行」这个由本次注入而非由面板决定的数字，并**保留一条 `assert "not_yet_knowable" not in refusal`**，好让把这扇门放回 `read_if_ready` 的改动在这里变红而不是悄悄把 `076` 拆掉的墙重新砌起来 |  | S13 |
| `V2-P5-052` | ~~**`test_one_registered_prediction_is_one_document_through_all_three_faces` 比的两样东西从来就不是同一个对象**~~ **已修（按同一渲染器合并 `limitations`，并把第三面真正比进来）** | 技 | P4-098 | **本行由本仓首次付费 e2e 全量运行测出**，且**不是一次三面分歧**：`over_the_command_line == over_http` 通过，逐字段 diff 是 **16 项完全相同、左边多 1 项 `limitations`、0 项取值不同**。原因写在 `model_view.held_prediction_view` 自己的文档串里：它是 `prediction_view` **加上** `KNOWN_MODEL_VIEW_LIMITATIONS`，而 `daily_view` 内嵌的是**不带** `limitations` 的 `prediction_view`，因为那份 body 在顶层已经带过一次登记，嵌套第二份就是同一份十五段话在一个答案里出现两次。于是 `fitted.first["prediction"]` 是 `prediction_view(record)`、`fitted.first["limitations"]` 是它旁边的登记，而单独交出一条记录的那两个面渲染的是 `held_prediction_view(record)`。**同一文件里的兄弟断言早就写对了**：`test_a_second_daily_run_on_a_later_instant_does_not_destroy_the_first` 写的是 `{**fitted.first["prediction"], "limitations": fitted.first["limitations"]}`，并带着点名 `V2-P4-098` 的注释；`V2-P4-098` 加键时更新了那一条、漏了这一条，而 `bcddd0d` 的提交信息自己记着「被周限打断，没跑过任何套件」。本行按兄弟那条**一字不差**地拼写合并式，理由就是这次漂移本身：一条规则两处拼写正是两个面分家的起点。**同时补上第三面**：`sdk.held_prediction` 按设计交出 `PredictionRecord` 而不是渲染结果，所以原来的 `isinstance` 加一个字段，在一个读到**另一条记录**的 SDK 上照样会通过 —— 现改为把它交回的记录送进同一个 `held_prediction_view` 再与另外两面比，这才是「三个面共用一个渲染器」这条规矩真正的断言形状 |  | S13 |
| `V2-P5-053` | ~~**`web/pnpm-workspace.yaml` 里那条 override 正是把高危版本按在原地的东西；而钉住字面版本号的那条测试，挡下的是安全修复本身**~~ **已修（三条 override 升版 + 断言改为读机制而非版本）** | 技 | — | CI 的 `web` 作业在**未改动的检出上**就红：`pnpm audit --audit-level high` 退出 1，三条高危全是 dev 传递依赖（`undici` 经 `vitest > jsdom`、`brace-expansion` 经 `eslint > minimatch`、`nanoid` 经 `vite > postcss`），都不进生产产物。两次独立验收都把它当作"需要一个带到期日的豁免决定"交回给用户——**而它根本不需要决定**。`brace-expansion` 早已被钉在 `5.0.8`，那是加这条 override 时的**修复版**；通告后来前移到 `>=5.0.9`，于是这条为关闸门而加的 override 变成了把闸门顶开的东西。**版本号是对某一时刻的断言，而没有任何东西会重读它。** 更糟的是 `test_quality_workflow_covers_supported_platforms_and_locked_dependencies` 写的是 `assert "brace-expansion: 5.0.8" in pnpm_workspace`——把那个时刻钉成了不变量，所以升版时**测试会拦下修复**。改法：三条 override 升到已修版本，断言改为"这三个包在 override 表里有名字"（读机制）加"CI 仍接着 `pnpm audit --audit-level high`"（读那个真能看见新通告的闸门）；闸门本身不在单测里跑，它要网络，`tests/conftest.py` 在 `tests/e2e` 之外拒绝网络。已用删去一条 override 证其可红。量得：8 项（5 中 + 3 高）→ 1 中，`--audit-level high` 退出 0；459 前端测试、lint、tsc、build、`--frozen-lockfile` 全绿 |  | S13 |
| `V2-P5-054` | ~~**CI 在这个 fork 上从未运行过一次；本会话所有"绿"都只是本地量的**~~ **已修（手动 `workflow_dispatch` 首次触发，并由此测出 055/056）** | 技 | — | `quality.yml` 只在 push 到 `main`、任意 `pull_request`、或 `workflow_dispatch` 时触发。工作分支 `feat/v2-p0a` 领先 `origin/main` **356 个提交**且没有 PR，于是 CI 一次都没读到过这些字节——`gh run list -R licai918/openalpha-cn` 返回**空**。更早一次误判值得记：不带 `-R` 的 `gh run list` 解析到了 `upstream`（`ss8875/…`，push URL 已被人为禁用），列出的是**别人仓库**的 dependabot 记录，我据此差点得出"CI 跑过"的相反结论；`gh` 没有默认仓库时会自己挑一个远端。手动触发后，**第一次运行就红了两处**（见 `V2-P5-055`/`V2-P5-056`），两处都是本地五道门禁结构上看不见的 |  | S13 |
| `V2-P5-055` | ~~**mypy 的 `follow_imports = "skip"` 豁免盖住了 `numpy` 这个包，却盖不住它唯一用来声明类型的那个 `.pyi` 文件**~~ **已修（补 `follow_imports_for_stubs = true`）** | 技 | — | CI 的 Python 3.12 两条（ubuntu 与 windows）都在 Type check 步骤退出 2：`numpy/__init__.pyi:737: error: Type statement is only supported in Python 3.12 and greater [syntax]`，且"errors prevented further checking"——**我们自己的代码一行都没被检查**。`[tool.mypy]` 把 `python_version` 钉在 `"3.11"`（正确，仓库支持 3.11），而 3.12 上 `uv sync` 装的是 `numpy==2.4.6`，其存根用了 PEP 695 的 `type` 语句。豁免**本来就写着** `numpy`/`numpy.*` + `follow_imports = "skip"`，注释写的正是"第三方存根可能用执行解释器的语法"——**它写对了意图，选错了作用面**：`follow_imports` 对 `.pyi` 默认不生效，除非 `follow_imports_for_stubs = true`。3.11 上完全不可见（解析到的 numpy 存根还没有这个语法），本地更不可见（本机 venv 压根没装 numpy，`test_seeding.py` 一直 skip）。已确认 mypy 1.20.2 的 `PER_MODULE_OPTIONS` 收此键 |  | S13 |
| `V2-P5-056` | ~~**web 作业从不安装 uv，而 `production` 那组 Playwright 测试要启动真正的 ASGI 服务端**~~ **已修（web 作业补 `setup-uv` + `uv sync`）** | 技 | — | `pnpm test:e2e` 在 CI 上死于 `/bin/sh: 1: uv: not found`，退出 127，**一个浏览器都没打开**；audit/lint/unit/build 四步全绿，只有它红。`playwright.config.ts` 的第二个 `webServer` 是 `uv run uvicorn openalpha_cn.api.app:app`——它**必须**是真服务端，因为 `V2-P5-027` 修的正是"书签 URL 只在开发服务器下可书签"，dev server 结构上证不了这件事。而 `quality.yml` 的 web 作业只装 Node/pnpm/Chromium，没有任何 Python 步骤。本地永远看不见：`uv` 总在 `PATH` 上。这条与 `V2-P5-054` 是同一件事的两面——**没跑过的 CI 里，一个从未被满足的前置条件可以一直写在那里** |  | S13 |
| `V2-P5-057` | ~~**六个模块各写了一套"把 `--help` 还原成文本"的办法，没有一套剥 ANSI**~~ **已修（共享 `tests/cli_help.py` + 一条禁止第七种拼法的审计）** | 技 | — | CI 首次跑完套件，五条断言同时红：`assert 'floor on this option is 4' in '\x1b[1m …'`。根因是**呈现层**：终端上 Rich 给 `--help` 上色，转义序列落在**句子中间**，对人眼毫无差别的子串就匹配不上。六处各自写了各自的还原：两处只折叠空白，三处另去 `│`（因为选项表折行会在行间夹一个框线），一处把 `COLUMNS` 调宽，一处用 `"".join` **不带空格**拼（因为限制码是单 token 会被折断）——**六种拼法，无一剥 ANSI**。本地不上色所以六处全绿；CI 设了 `FORCE_COLOR`，于是本仓套件第一次在那里跑就红了五条。已本地复现：`FORCE_COLOR=1 uv run pytest <那条>` 当场红。改法照本仓自己的成例（`test_run_mode.py`：只留一处声明，另加审计防止再抄）：新建 `tests/cli_help.py` 只做**撤销呈现**（剥 ANSI、去框线、`COLUMNS=200`、顺带断言 `--help` 退出 0），**不替站点决定怎么拼**——因为 `"".join` 与 `" ".join` 是关于"在匹配什么"的主张，helper 替所有人选一个就会悄悄弄坏另一个；再加 `test_no_other_test_module_renders_help_for_itself` 以 AST 扫源码树，把 `"--help"` 字面量锁在一个模块里。ANSI 模式刻意宽于 Rich 今天会发的 `m` 序列——本缺陷的本质就是"没人预料到的序列落进句子中间"。量得：六个模块 127 条，裸跑与 `FORCE_COLOR=1` 下均全绿 |  | S13 |
| `V2-P5-058` | ~~**`ctypes.dlopen` 这一项在 Linux 上从未被激发过；它是被另一个事件顶替着通过的**~~ **已修（`find_library` 移出守卫块）** | 技 | — | `test_every_refused_event_is_one_this_test_provokes_rather_than_a_name_in_reserve` 的整个用意是"`OUTWARD_AUDIT_EVENTS` 里每一项都被真的激发过——列着却无人抛出的项，其覆盖是未测量的"。CI 上它红了，而**红出来的内容比红本身重要**：期望 `ctypes\.dlopen`，实得 `subprocess.Popen`。因为 `refuse(lambda: ctypes.CDLL(ctypes.util.find_library("c")), ...)` 把 `find_library` 写在了 lambda 里，即写在 `refusing_outward_calls()` **块内**，而 Linux 上 `find_library` 会先 shell 出去调 `gcc`/`objdump` 找库——于是守卫先抓到 `subprocess.Popen`，`ctypes.CDLL` 根本没执行到。macOS 走 dyld 不外派进程，所以本地一路走到 `CDLL`，测试常绿。**也就是说：在 `Dockerfile` 实际发布的那个平台上，`ctypes.dlopen` 守卫的覆盖一直是未测量的**，而这条测试的名字宣称的正好相反。改法：`find_library` 提到块外求值，并断言它非 `None`（否则 `CDLL(None)` 会 dlopen 主程序，仍能通过而证不了东西） |  | S13 |
| `V2-P5-059` | ~~**离线守卫在 Windows 上说的不是它在 POSIX 上说的那句话：它拦下的是解释器自己的管道**~~ **已修（只放行 `socket.socketpair` 自身栈帧内的那次 connect）** | 技 | — | Windows 作业 **194 failed / 111 errors**，而其中 **281 条失败是同一个根因**：`OfflineSuiteViolation: socket.connect(('127.0.0.1', …))`。调用栈是 `TestClient.__enter__` → anyio portal → asyncio `ProactorEventLoop._make_self_pipe` → `socket.socketpair()` → Windows 上 `socket.py` 的纯 Python 模拟（bind/listen/**connect**/accept）。**没有一条测试在联网**——Windows 没有 `socketpair(2)`，CPython 用回环 TCP 模拟它。所以缺陷不是那 281 条红，红是症状；缺陷是**这道守卫在两个平台上含义不同**。修法必须窄：按 `socket.socketpair` 的**代码对象身份**放行（沿栈上溯，因为事件是从 C 方法里抛的，模拟帧在其上方），**绝不按地址**——`V2-P4-039` 立的地址盲（"回环上的数据报与可路由主机上的同等对待"）不能为一个平台怪癖而废。顺带纠正我自己写错的前提：起初我断言 POSIX 上 `socket.socketpair` 是 builtin、`__code__` 为 `None`、该分支是死码——**实测不是**，`socket.py` 在两个平台都包一层以返回 `socket.socket` 对象，所以常量两边都是代码对象，分支是活的；差别在于**那个函数体做什么**。已加四条测试，其中两条用 monkeypatch 把常量指向本地函数，使这条只在 Windows 上生效的分支在 POSIX 上也可被激发 |  | S13 |
| `V2-P5-060` | ~~**九处把仓库相对路径按运行平台的分隔符拼成字符串，去和写着 `/` 的字面量比较**~~ **已修（全部改 `.as_posix()` + 一条 AST 审计）** | 技 | — | `str(path.relative_to(SOURCE))` 在 Windows 上给出 `panel\store.py`，白名单写的是 `panel/store.py`，于是**整个集合被报成未列出**——`test_query_callers`、`test_visible_read_callers`、`test_dotenv_loading_is_a_single_deliberate_module`、`test_feature_ledger_test_tree` 三条，全是这一族。而**同树已有七处写着 `.as_posix()`**：树是半改完的，又一次"改了一部分副本"。`windows-latest` 写在 `quality.yml` 的矩阵里、且被紧邻的那条测试断言着——这是本仓**声明**的支持平台，而这份声明从未被检验过，因为 CI 从未在这条分支上跑过（`V2-P5-054`）。审计的第一版是正则扫源码，**它把自己的 docstring 当成了代码**（散文里就写着这个惯用法）——正是 `test_known_limitation_registries.py` 建来制止的那种误计；已改为 AST，只看可执行的 `str(x.relative_to(…))` 调用。已用写回一处证其可红 |  | S13 |
| `V2-P5-061` | ~~**两条守卫的"每一项都被真的激发过"等式，在缺少 `AF_UNIX`/`sendmsg`/`posix_spawn` 的平台上无法成立**~~ **已修（按平台减去激发不了的项，而非整条跳过）** | 技 | — | Windows 上 `'socket' object has no attribute 'sendmsg'`、`module 'socket' has no attribute 'AF_UNIX'`。三条测试。最省事的改法是整条 `skipif`，但那会把**本可激发的那几项也变成未测量**：`test_paper` 的 `socket.__new__`、`subprocess.Popen`、`os.exec`、`ctypes.dlopen` 在 Windows 上都能激发，`test_offline_suite` 的 `connect`/`sendto` 地址报告同样能。所以改成显式的 `UNPROVOKABLE_HERE` 集合（在本项目发布的平台上为空集）从等式里减去，并把只能在有 `sendmsg` 的平台跑的那半拆成独立测试。`socket.sendmsg` 仍留在 `GUARDED_AUDIT_EVENTS` 里：守卫命名的是 CPython **能**抛的事件，不是这台机器恰好能激发的事件——按平台改守卫集合，等于让守卫的含义随运行环境漂移，而那正是 `V2-P5-059` 刚修掉的病 |  | S13 |
| `V2-P5-062` | ~~**`_pearson` 的三处求和是裸 `sum`，于是同一份输入在两个受支持的解释器上算出不同的相关系数——直到不同的内容地址**~~ **已修（改 `math.fsum`，并把因此移动的每一处实测值重新量过）** | 技 | — | CPython **3.12 给浮点 `sum()` 加了 Neumaier 补偿求和**，这是本仓套件第一次在 3.12 上真正跑起来（此前它停在 Type check，见 `V2-P5-055`），于是 6 条只在 3.12 红。追到源头是 `backtest/factor_ic.py::_pearson` 的协方差与两个离散度用的是 `sum`——而同模块四周全是 `fsum`。**最尖锐的证据不在测试里**：`test_the_register_lists_what_it_holds_in_custody_order_and_not_by_content_hash` 里五条记录的 `prd_*` 内容地址在两个解释器上**不同**；改后两边逐字节相同。**内容地址不该是"算它的那个解释器"的属性。** 更值得记的是本仓自己早就写下过这句话：`alpha_baseline.rankable` 的 docstring 说排序不是为了整洁，因为"`_pearson` 按参数自身顺序累加乘积……一个在最后一位悄悄移动的 fit 仍会打破每一个内容地址"，并给出实测 190/400（六名）、347/400（六十名）；**它选择了让调用方排序，而不是让算术与顺序无关**。改后同一测量在两个解释器、每个规模上都是 **0/400**。排序保留（两个调用方必须取同一总体），但理由已按实测改写。连带重新量过并改正的写死值：`149→143`、`153→150`（两处：`factor_redundancy` 的散文与测试）、`point.ic 0.9999999999999998→1.0`、`raw_correlation == 0.0` 改为 `approx(0, abs=1e-15)`（其 docstring 本就写的是"近乎零"）、摘要排序的索引 `(2,1)→(4,0)`。`sum!=fsum` 的对抗夹具也被 3.12 吃掉了两处，一并重造并**用变异证明可红**：`test_alpha_tree` 的 `[1e16,1,-1e16,1]` 换成两尺度抵消，`_pearson` 的顺序无关性新增三条参数化截面。变异表（逐个 `fsum` 改回 `sum`）：协方差 3.11/3.12 均抓到；两个离散度**仅 3.12 抓到**——已按实测写进 docstring，不含糊成"都覆盖" |  | S13 |
| `V2-P5-063` | ~~**三条测试把"这台机器"当成了不变量：一个写死的 POSIX `PATH`、一个写死的异常类名、一个按某台笔记本校准的墙钟**~~ **已修（环境按平台给引导集、异常类向平台问、墙钟换成增长比）** | 技 | — | 均由 Windows 首跑测出，且都不是 Windows 的问题，是**把局部事实当普遍事实**。(a) `test_import_time_filesystem` 用 `env={"PATH": "/usr/bin:/bin", …}` **替换**整个环境以隔离配置——Windows 的 `python.exe` 没有 `SYSTEMROOT` 根本起不来，子进程在 import 任何东西之前就退出 1，而 `check=True` 把 stderr 捕获后**丢掉了**，失败信息只有"exit status 1"。先让失败会说话（把子进程 stdout/stderr 带进断言），再按平台给最小引导集，并断言子进程只带一个 `OPENALPHA_` 变量以保住隔离本意。(b) `test_a_command_that_breaks_is_not_reported_as_an_unhealthy_panel` 断言 stderr 里出现 `NotADirectoryError`——文件占了目录位置时 POSIX 抛这个，**Windows 抛 `FileExistsError`**。改成在测试里**实地激发一次同样的操作、读出这个平台抛的类名**，而不是枚举想得到的那些；若哪天这个操作不再抛，测试会 `pytest.fail` 而不是被一个 `or` 悄悄变成空断言。(c) `test_whole_market_batch_completes` 断言 `elapsed < 60.0`，其注释写明它是 **O(N²) 回归的绊线、不是基准测试**——但 60 秒是写它那台笔记本的属性：同一次运行在我这台是 14s、**在 Windows runner 上是 602s**，于是绊线响的是慢机器不是回归；反过来，在足够快的机器上它也会**漏掉**回归，而这半边没人会注意到。改成两点比值：本跨度上二次增长预测 ~123×、线性 ~11×，四倍之差，且任何时钟频率都不改变商。本机实测 10.4×（略低于线性，因为小规模那次付了同样的固定开销），上界取 30×。已用把上界压到 5× 证其可红，失败信息自带解释：`8.0x for 11.1x the work, where quadratic would predict 123x` |  | S13 |
| `V2-P5-064` | ~~**SPA 回退按 `/` 取首段，而 `StaticFiles.get_path` 交给它的是"带 OS 专有分隔符"的路径——于是在 Windows 上这道归属检查整个失效，每一条请求都被发回 HTML shell**~~ **已修（按平台自己的 `os.sep`/`os.altsep` 切分）** | 技 | — | 修完 `V2-P5-059/060/061/063` 后，Windows 从 **194 failed + 111 errors** 降到 **31 failed / 5566 passed**，而剩下 **26 条挤在同一个文件**：`test_spa_addressability.py`。根因是一行：`raw = path.split("/", 1)[0]`。它的 docstring 正确地写着"`path` 已被 `StaticFiles.get_path` 归一化过"——而 starlette 那个方法**自己的 docstring** 写的是返回"with **OS specific path separators**"，实现是 `os.path.normpath(os.path.join(*route_path.split("/")))`。Windows 上 `GET /api/v1/nope` 变成 `api\v1\nope`，`split("/")` 一刀不切，整条字符串去和归属集比较，两边都不匹配——**这个类精心推导的两个归属者（API 路由表、构建目录）同时失效**。后果正是 `V2-P5-027`/`V2-P5-030` 立起来要防的那句话，从没人检查的那个分隔符进来：缺失的 `/assets/*.js` 对 `<script>` 标签回 `200 text/html`，`POST /api/v1/nope` 回一个页面。修法取自 `os` 而非写死 `"/\\"`，所以它说的是"这个解释器拿什么切路径"，而不是点名两个想得到的平台；POSIX 上 `os.altsep` 为 `None`，行为逐字节不变。切分抽成可注入分隔符的 `_first_segment`，**使这段只在 Windows 上生效的逻辑在 macOS 上可被测到**——变异验收：`PATH_SEPARATORS` 改成 `""` 或 `"\\"` 各红 28 条，`_first_segment` 只认 `/` 红 1 条。另修同轮暴露的三条：(a) `_without_store_path` 之后残留的路径改为一律 `/`——**它是 `disclosable` 的，跨进程发给不在这台机器上的调用方**，`this service's panel store\adj_factor\2026\data.parquet` 既不是可用路径也不是两个部署会一致的标识符；(b) `Counter(str(Path(p).parent) …)` 把整个 ledger 按 `tests\e2e` 建键——`V2-P5-060` 的审计只认 `relative_to` 没认 `parent`，已拓宽，并把 `resolve`/`absolute` **明确排除**（它们回答"在这台机器上的哪里"，正是必须带平台分隔符的那个问题——树里七处 `str(x.resolve())` 全是喂前缀替换的）；(c) `find_library("c")` 在 Windows 返回 `None`，改用 `msvcrt` **保住测量**而不是减进 `UNPROVOKABLE_HERE` |  | S13 |
| `V2-P5-065` | ~~**四个模块各有一份逐字节相同的"抹掉存储路径"实现，其中三份的 docstring 明确指名了原件——**知道自己是副本并说出来，并不等于不是副本**~~ **已修（留一处实现，另三处指名它 + AST 审计）** | 技 | — | `V2-P5-064` 在 `panel_view._without_store_path` 里加了分隔符归一化，Windows 上那条消息**照旧**输出 `this service's panel store\adj_factor\2026\data.parquet`——因为 `factor_view`、`model_view`、`shortlist_view` 各自持有一份副本，改动只到了四分之一。三份副本的 docstring 都写着"这是 `panel_view._without_store_path` 的规则及其实测理由"，**它们指名了原件，却仍把函数体抄了一遍**。这条是被 Windows 撞出来的，但缺陷与平台无关：任何对这段逻辑的修改都只会命中四分之一。修法照本仓成例（`test_run_mode.py`：只留一处声明，另加审计防止再抄）：`panel_view.without_store_path` 成为唯一实现，另三处改为一行委托；新增 `test_only_this_module_replaces_the_store_path_with_the_placeholder` 以 AST 扫源码树，断言只有一个模块执行 `x.replace(_, PANEL_STORE_PLACEHOLDER)`。已在 `factor_view` 恢复一份副本证其可红。顺带：新增的跨模块名字被**接缝表**（`test_panel_ingest_import_isolation`）当场拦下——那张表要求每个模块声明它跨接缝取用的每一个名字，三行已补 |  | S13 |
| `V2-P5-066` | ~~**这个工作流里唯一构建生产产物的作业，被一个它无法指名其中一条腿的矩阵挡着，从未运行过一次**~~ **已修（`container` 不再 needs `python`）** | 技 | — | `container`（"Container persistence recovery"）是 `Dockerfile` 的镜像被构建、`deploy/compose.yml` 的栈被启动、**证据被证明挺过一次重启**的唯一地方——也就是这个工作流全部的生产验证。它写着 `needs: [python, web, security]`，而 `python` 是覆盖 `ubuntu-latest` **与** `windows-latest` 的矩阵。`needs` **无法等待矩阵中的某一条腿**，于是 Windows 一红整个 container 作业就被跳过——而本仓迄今每一次分发 Windows 都是红的：实测四次运行全部 `skipped`。也就是说，本轮此前所有"绿"（含 ubuntu 3.11/3.12 全绿）**都还没有触及那个真正要发布的产物**。这条依赖原本是成本启发式（"别在明显坏掉的代码上花构建分钟数"），它悄悄做的却是另一件事：**扣住唯一能证明所发之物可用的证据**。`web` 与 `security` 仍然把关，且跑在同一棵树上。未采用"把矩阵拆成两个 job"的方案：那需要把 10 个步骤原样抄一份，而 GitHub Actions 不支持 YAML 锚点——在一轮刚以 `V2-P5-065`（四份副本，三份指名了原件）收尾的会话里，为一个成本启发式付副本的代价不成立。已加 `test_the_container_job_is_not_gated_behind_a_matrix_it_cannot_name_a_leg_of` 断言之，并用把 `python` 加回去证其可红 |  | S13 |
| `V2-P5-067` | ~~**一次普通的读请求撞上一次写，就会崩——面板目录的并发设计默认只读与读写连接可以在同进程共存，而 DuckDB 拒绝这件事**~~ **已修（读写锁：读者并行、读者与写者互斥）** | 技 | — | **我先前把这一族判成"需要 Windows 机器才能定位"，那是错的——错在我没把完整错误读完。** 读完之后：`Can't open a connection to same database file with a different configuration than existing connections`。DuckDB 每进程每文件共享一个实例，拒绝配置不同的第二个连接；而 `panel/store.py` 的 7 处目录读全是 `read_only=True`、2 处写是读写。**已在 macOS 上用 `duckdb.connect` 直接复现**，所以它与平台无关，Windows 只是把调度稳定成必现。新写的 `test_a_read_running_while_a_write_holds_the_catalog_does_not_crash` 在修前 **64 次操作崩 28 次**，本机 3/3 稳定。影响面比测试暴露的更大：`query()` 是只读、`write_partition` 是读写，**API 服务里一次读请求撞上一次面板构建就会抛 `ConnectionException`**。模块 docstring 说"任意数量的并发读者可以并行"——这在**读者之间**为真，它没说、也不成立的是读者与写者。修法不是"一把锁锁住一切"：那会把崩溃换成回归（`test_profile_query_survives_eight_concurrent_threads…` 要求读并行）。DuckDB 禁止的恰好是"读者+写者"，所以用读写锁，只排除这一件事。两种重入被**当场解决而不是写成注释**（存储层里写成注释的死锁就是死锁）：同线程读中套读只取一次锁；写者块内触发的读直接放行。另加写者等待计数以防读流饿死写者。同轮修掉两条测试自身的缺陷：(a) 截断夹具用一条 `COPY (SELECT … read_parquet(?)) TO ?` **读写同一个文件**——duckdb 写临时文件再 move 覆盖，而目标正被扫描着，POSIX 允许、Windows 拒绝；(b) 改用两个不同路径后才发现，**duckdb 先绑 `TO ?` 再绑 `read_parquet(?)`**——原写法两个参数是同一个字符串，这个顺序问题**完全不可见**，已改成一句一参。量得：加固套件 25 passed 连跑三遍无死锁，panel 集成 896 passed，unit 3341 passed |  | S13 |
| `V2-P5-068` | ~~**一条名为"按内容哈希检查逐字节还原"的 e2e 测试，从头到尾没碰过内容哈希——而它的 docstring 还与所测模块的原文正好相反**~~ **已修（改为断言存储层真正保证的那两件事）** | 测 | — | 由当前 HEAD 的付费 e2e 全量重跑带出（59 passed / 1 合法 skip / 38:59，指向已存面板故不重建）。`test_the_defect_free_panel_still_answers_after_a_partition_was_moved_and_put_back` 的 docstring 写着"Byte-for-byte restoration, checked against the catalog's own content hash"，而它的断言只有 `path.exists()` 与 `count(*) > 0`——**一个少了一百万行的分区照样满足**。已实证：把 `stk_limit/2026` 重写成少一行（1190637 vs 目录记载 1190638，魔数与格式完好），**现有测试原样通过**。docstring 的第二句"`content_hash` 在每次就绪评估时从文件重算"同样不成立——`_content_hash` 全模块只有一个调用点，在 `write_partition` 内、对内存里的行求值；而 `store.py` 自己的原文恰恰是反过来说的："目录里什么都没变，**所以任何哈希比对都看不见它**"。**产品无缺陷**：存储层已完整声明它保证什么（两端 Parquet 魔数 + 页脚行数对账 `panel_partitions`）与够不着什么（就地改值，由 `return_path_disagreement`/`close_disagreement` 读列去抓，**是披露而非辩解**）。所以修的是测试：断言改成存储层真正保证的那两件事，并新增与 `catalogued_path` 对称的 `catalogued_row_count`。按 TDD 做且顺序被刻意反转——**先把面板弄坏、证明旧测试仍绿**（这才是这条缺口的 RED），再写断言看它红，再从 pristine 逐字节还原看它绿。面板全程可逆：跑前已备份并校验，跑后所有 Parquet 分区逐字节等于开跑前 |  | S13 |
| `V2-P5-071` | ~~**两面各自重述的\*\*常量\*\*有等式测试钉着，两面各自重述的\*\*函数\*\*没有**~~ **已修（补两条等式守卫）** | 测 | — | 按本轮反复出现的缺陷形状主动扫描，而非等它再咬一次。AST 比对 `src/` 全部函数体，得**六组跨模块完全相同**。逐组读 docstring 与 `import-linter` 契约后，**重复本身多数是被分层逼出来的、且作者已配了守卫**：`average_ranks`/`panel_factors._average_ranks` （`backtest` 禁 import `panel_factors`）由 `test_factor_ic.py:354` 明写「同一条规则的两个实现」钉住；`FACTOR_TIER_DATASETS`/`FACTOR_TIER_BUILD_ARGUMENTS` 两面重述，由 `test_both_faces_spell_a_tiers_build_arguments_the_same_way` 钉住，其 docstring 明写「restated rather than imported because this module may not see `factor_view`」。**缺的是两个函数**：`_board` 与 `_unbuilt_factor_remedy` 同样被两面重述、同样不能 import，却**钉在了空处**——而 `shortlist_view._board` 的 docstring 还指着另一面的推理（`V2-P5-065` 刚在一层之上拆掉的形状）。**代价是实测过的**：`V2-P4-067(b)` 的修改落在 `shortlist_view`，而该行自己的复现命令走 `factor_view`，行还被关掉了。修法用作者成例而非合并（分层不允许）：`test_both_faces_classify_a_board_the_same_way` （语料覆盖四个板块与 `689`/`301` 两个前缀边界，并断言四个板块都被触达，否则一致性证不了东西）与 `test_both_faces_offer_the_same_unbuilt_factor_remedy`。两条都按 TDD **先让一份副本分岔以制造 RED**（`300`→`301`；文案改一词），看它红且消息点名分歧，再逐字节还原看它绿。~~**未修并说明**：`freeze_payload`×2（两个 Pydantic 模型的校验器，合并需 mixin）、`note_for`×3（同在 `domain`，第三份已有 `test_factor_neutralization.py:438` 部分覆盖）、`storage` 两个 3 行构造函数——均无等式守卫，风险低于上述两个但**同类**，是否补由用户定~~ **v2 收尾清理批次（2026-09-09）已全部裁决**：`freeze_payload` 已合并进 `FreezePayloadMixin`（`de27977`）；`note_for` 已合并进 `NoteLookupMixin`（`247c1d9`）；`storage` 两个 3 行构造函数经裁决**不合并**，理由记录在该提交中（`fe1d701`） | 单元：两面 `_board` 与 remedy 逐字符相同 | S13 |
| `V2-P5-072` | ~~**组合构建把\"全清单名次\"当成\"待加权列表里的位次\"，于是任何**获准子集**都被自己必然带的名次缺口拒掉**~~ **已修（两个适配器都在入口重编号）** | 修 | — | 由跑手册第 5 步实测撞出，不是推演：34 只获准标的名次是 `1,2,4,6,…`（`researched` 只留有信号的那些；只要有任一未研究名排在某个获准名之前就有缺口——几乎总是，但**并非必然**：若未研究的恰是清单末尾几名，获准名次就是 1..k，修前照样能建），`portfolio construct` 报 `candidate ranks must be exactly 1..34`。**读代码定根因而非猜**：`ConstructionCandidate.rank` 的语义就是「待加权列表里的位次」——它只被用来排序，以及**按位次**切档（`_tier_sizes` 的 docstring 自述「只是两个计数的函数」，配对走 `ordered[position : position + size]` 的切片）。所以那条拒绝话术**一半是错的**：缺口挪不动任何档位边界，并列才真的会让切点依赖迭代序。错在两个适配器 `candidates_from_ranking` 与 `candidates_from_shortlist_answer` 照抄了**清单全域**的名次。修法是在两个入口各加一次 `_renumbered`：按存下来的名次排序、重编为 1..n。**并列必须挪到入口去拒**，否则被自己吞掉——重编号后的名次永远是 1..n，原来那道守卫再也看不见重复；而并列和缺口不同，位次切档吸收不了它（两个相等名次让二者之间的顺序未定，谁进高档取决于迭代序）。这一条也是先红后绿：`DID NOT RAISE PortfolioConstructionError`。缺口守卫留在原处，仍然管着手搓 `ConstructionCandidate` 的调用方（SDK 提供这条路）。**改动了一条既有契约并已记录**：`test_..._reads_the_order_...` 原先断言适配器保持数组顺序、照抄名次（`[2, 1]`），现在断言按名次排序后重编号（`[1, 2]`）；数组顺序两种写法下都到不了权重，因为 `_ordered_candidates` 切档前必排序。TDD 先红：两条新测试分别报 `assert [1, 2, 4, 6, 7, 9] == [1, 2, 3, 4, 5, 6]` 与那条拒绝异常，再绿。 **独立验收查出七处并已全数修掉**（这条的价值多半在这里）：①**修复的另一半 `candidates_from_ranking` 零覆盖**——摘掉它的 `_renumbered`，3349 条单元测试全绿；而那一侧的缺口同样真实（`CrossSectionScreen.select` 按位次给 `ShortlistEntry.rank` 赋值、`rank_candidates` 强制 `candidate.rank == entry.rank`、`CandidateRanking.candidates` 剔除 `unresearched`），且正是 `construct_portfolio_from_ranking` 所在的接缝，现已用真 screen 造真 ranking、研究一个非前缀子集来盖住。②**我新写的一条测试是恒真式**：名字与 docstring 都说「同名标的落在同一档」，实际两臂名字集合不同、只比 tier/weight **列表**（对任意六个名字都相等），反序变异下保持绿色；已改为固定 subject、按 subject 比对、并把预期映射写死。③**「子集名次必然有缺口」是假命题**，而它是立论，出现在五处含提交标题——若未研究的恰是清单末尾，获准名次就是 1..k，修前照样能建；已逐处改正（已推送的提交标题改不了，在此存证）。④缺口拒绝仍在说被本修复证伪的理由，钉它的测试把错理由写进函数名——均已改。⑤两个适配器、`ConstructionCandidate.rank` 字段、`OpenAlphaSDK.construction_candidates` 都没说 `rank` 被改写——四处补齐。⑥`POST /api/v1/portfolio/construct` 的拒绝清单未列新增的并列——已列。⑦`score` 与 `industry_code` 透传无测试（两个变异都全绿）——已补。 | 单元：获准子集被加权而非拒绝；固定 subject 后重编号前后落同一档且映射写死；ranking 侧同样重编号；score 与 industry 透传 | S13 |
...
- **新增 `PanelStore.read_visible_at`**：跑**同一张** `evaluate_readiness` 规则表，
  ...
  `not_yet_knowable` 一个）里时，才改用 `WHERE available_time <= as_of AND revision_time <= as_of`
  扫描而不是拒绝（本条当初只写 `available_time` 一个时钟，修订时钟是后来补齐 `is_visible_at`
  两个时钟时加上的）；
...
合起来（**以下是撤回前的原文结论，按本节惯例保留；上一条注里的两次撤回同样适用于这一段**）：
**年 Y 中任意交易日的残差，其 `available_time` 都落在 Y 年最后一个 session 之后。**
...
> **撤回（同上）**：`V2-P4-026` 与 `V2-P4-027`/`028` 之后，两个外部读都按 `as_of` 落在的那一天取，
> 残差带的是自己面板的时刻，年内 `as_of` 读得回来；所引测试仍在，断言的是「算出来之前不可见」这条
> 更窄的性质。剩下的边界只有一个会话宽。
...
`tests/unit/test_factor_volatility_liquidity.py` 的
`test_the_reason_no_residual_ships_is_a_property_of_the_panel_and_of_the_window`
钉住 —— **谁哪天接入了指数点位序列，那条测试就红，这个声明就必须重审**。
（那一天到了：`V2-P3-016` 之后该测试改名为
`test_exactly_one_dataset_carries_a_level_and_exactly_one_channel_reaches_it` 并反向钉住。）
...
声明由 `tests/unit/test_factor_value_family.py` 的
`test_no_stored_statement_projection_carries_a_deducted_profit_column`
