...
| `V2-P5-049` | ~~**`openalpha --version` exit 2 `No such option: --version`**~~ **已修（eager 根回调，与 `version` 逐字节相等）** | 产 | — | 由最终产品验收测出，小但**是一个人打的第一条命令**，而这个 build 的 `openalpha version` 印得好好的。现为 eager 根回调、复用 `version` 自己那一句 f-string，并**断言两者 stdout 逐字节相等**（一个 build 号的两种打法正是第二种拼写会招来的漂移）。**`is_eager` 是承重而非装饰**：`app` 带 `no_args_is_help=True`，非 eager 的旗标会被解析、找不到子命令、再以第二个理由 exit 2 —— 由一条独立测试钉住。**回调命名为 `_root_options` 而非 Typer 惯用的 `main`**：本模块已把 `main` 绑给 `console_scripts` 入口，两者同名让入口**静默覆盖**了回调，只因装饰器已先捕获函数对象才没坏，而 `ruff` 一直在报 `F811` | 单元：`tests/unit/test_cli.py` —— `--version` 与 `version` stdout 逐字节相等；`--version` exit 0 且不打 `Usage:`（eager 的那条） | — |
