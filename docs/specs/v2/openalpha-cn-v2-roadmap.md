...
| **P4** | 候选排序与模型基线 | 84 | 6–7 周 | 15–18 周 | 契约升版一次完成 + 预测先落库 |
| **P5** | 组合、验证与工作台 | 24 | 6–8 周 | 15–20 周 | 归因对账 + 多重检验 + 4 页可用 |
| | **合计** | **178** | **28–36 周** | **70–90 周** | |
...
| `V2-P4-084` | **同一个缺陷隔一个接缝：`_PanelInputs.label` 只捕 `LabelError`，而三个同侪错误都是普通 `ValueError`** | 产 | 080 | **`V2-P4-080` 修复时扫描发现，未在该轮修**。`factor_view._PanelInputs.label` 用 `except LabelError` **单独**包裹 `label_outcome`，而 `StockUniverseError`/`AdjustmentError`/`PriceDataError` 经 import 验证**全部是普通 `ValueError`** —— 与 `080` 同类：一个被设计成判定的领域错误，在守卫之外抛出，会被洗成 exit 5 / 500。另有 `shortlist_view.py:1249` **裸调** `registry.listed_on(session)`，而 `factor_view.py:2717` 对同一个调用**是有守卫的**。**为什么此前不可达**：`tests/panel_fixtures.py` 的 `_universe_batch` 给每只证券一行上市记录且与其它构造器共用同一个 `SECURITIES` 元组，故**没有任何生成面板能带出「有因子行、无 `stock_basic` 行」的证券**；`_factor_batch` 是完全叉积、无 `omit`。这正是 `V2-P4-080` 记下的那条推广的又一个实例 —— fixture 让数据集比真实语料更规整 | 集成：三个同侪错误各自必须给具名拒绝而非 5/500 | S48 |
| `V2-P4-085` | **另有四处 fixture 构造器让数据集比真实语料更规整（仅缺口，未遮活缺陷）** | 测 | 080 | **`V2-P4-080` 修复时扫描发现**：`_industry_assignments`、`_limit_batch`、报表行构造器、`_index_weight_batch` 四处各自给出比真实语料更完整的形状。`080` 已修 `_name_records`（新形状 `name_history.effect_after_every_priced_session` **替换**而非追加 `securities[3]` 的语料 —— 丢掉基线行正是全部内容所在），`084` 记了 `_universe_batch`/`_factor_batch` 这两处遮着活缺陷的；本行记其余四处，作为「fixture 是否让数据集比真实语料更规整」这条审计的待办清单 | 测试：四处各自能表达真实语料里存在的不完整形状 | — |
