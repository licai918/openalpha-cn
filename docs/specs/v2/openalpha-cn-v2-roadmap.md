...
| `V2-P5-072` | ~~**组合构建把\"全清单名次\"当成\"待加权列表里的位次\"，于是任何**获准子集**都被自己必然带的名次缺口拒掉**~~ **已修（两个适配器都在入口重编号）** | 修 | — | 由跑手册第 5 步实测撞出，不是推演：34 只获准标的名次是 `1,2,4,6,…`（`researched` 只留有信号的那些，`admitted ⊂ shortlist` 时缺口是**必然**的），`portfolio construct` 报 `candidate ranks must be exactly 1..34`。**读代码定根因而非猜**：`ConstructionCandidate.rank` 的语义就是「待加权列表里的位次」——它只被用来排序，以及**按位次**切档（`_tier_sizes` 的 docstring 自述「只是两个计数的函数」，配对走 `ordered[position : position + size]` 的切片）。所以那条拒绝话术**一半是错的**：缺口挪不动任何档位边界，并列才真的会让切点依赖迭代序。错在两个适配器 `candidates_from_ranking` 与 `candidates_from_shortlist_answer` 照抄了**清单全域**的名次。修法是在两个入口各加一次 `_renumbered`：按存下来的名次排序、重编为 1..n。**并列必须挪到入口去拒**，否则被自己吞掉——重编号后的名次永远是 1..n，原来那道守卫再也看不见重复；而并列和缺口不同，位次切档吸收不了它（两个相等名次让二者之间的顺序未定，谁进高档取决于迭代序）。这一条也是先红后绿：`DID NOT RAISE PortfolioConstructionError`。缺口守卫留在原处，仍然管着手搓 `ConstructionCandidate` 的调用方（SDK 提供这条路）。**改动了一条既有契约并已记录**：`test_..._reads_the_order_...` 原先断言适配器保持数组顺序、照抄名次（`[2, 1]`），现在断言按名次排序后重编号（`[1, 2]`）；数组顺序两种写法下都到不了权重，因为 `_ordered_candidates` 切档前必排序。TDD 先红：两条新测试分别报 `assert [1, 2, 4, 6, 7, 9] == [1, 2, 3, 4, 5, 6]` 与那条拒绝异常，再绿。 | 单元：获准子集被加权而非拒绝；重编号后同名标的落在同一档 | S13 |
