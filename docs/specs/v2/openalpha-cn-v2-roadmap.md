...
- **量级**：fixture 面板上 `read_if_ready` 在年中 `as_of` 整块拒绝，`read_visible_at`
  ...
  （读 + 分组 + 定级 + 求值）**冷 1.95 秒、热 1.91 秒**，约 2.9 微秒/行；复审补救后同一形状重测为
  **冷 2.24 秒、热 2.26 秒**（约 3.3 微秒/行，多出的是会话并集与每只标的两次二分查找）；
  同一个分区**写**要 288 秒（重测 350.6 秒，该数字随机器变化，只主张量级差）。
  这个数字同时是「不引入 numpy/pandas」的依据（见 ADR-0003 的 2026-08-11 更新）。
