...
| `V2-P5-070` | ~~**112 行台账在渲染时丢掉最后一列——两张表的表头漏了「验收」，另有 7 行把未转义的 `\|` 写在行内代码里**~~ **已修（表头补列 + 73 行补空格 + 7 行转义），并加机械守卫** | 测 | — | **由 GitHub 官方渲染器判定，不是我的推理**：`gh api -X POST /markdown -f mode=gfm` 实测三条——①`` `x\|y` `` 被切成两格且**代码块不成立**（输出 `` `x `` / `` y` ``）；②超出表头的单元格**被静默丢弃**（3 格行进 2 列表，第三格消失）；③短行在**行尾**补空格。据此扫描全 `docs/`：**112 行 / 1134 行**不匹配，集中在 2 个文件。根因是**表头而非行**：P4 表声明 6 列而 114 行里有 92 行是 7 格（`ID\|标题\|类型\|依赖\|说明\|验收\|PRD`），于是那 92 行的 **PRD 引用在页面上不存在**；P5 表同病，比例相反。P3 表 19 行全 6 格、**本来就对**——我第一版脚本只按表头文字匹配，给它错加了一列，**已从备份重来并加上「该表确有 7 格行才加列」的条件**。7 行管道问题不能按「左右两格反引号都不成对」判——`` `a\|b\|c` `` 的中间格反引号数为 0；正确判据是**沿单元格累计反引号奇偶**，奇则该边界落在代码块内。修后官方渲染器确认 `` `\|\|\|\|\|\|\|` `` 与 `` `openalpha jobs register\|list\|due\|run` `` 都还原成完整代码块（后者此前被切成四个单元格）。**内容零丢失已逐字符证明**：剥去分隔符与空白后 seam-audit 完全相同，roadmap 恰好 +10 字符 = 两个表头的「验收」(4) + 两个分隔行的 `---`(6)。守卫 `test_every_documentation_table_row_fits_the_header_it_is_under` 扫描全 `docs/` + README + CHANGELOG，**先写后修**：写完即报 112 行红，修到 0 | 单元：转义管道不切分单元格、未转义的切分 | S13 |
