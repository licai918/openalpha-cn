...
| `V2-P5-069` | ~~**一个按"互相吻合的两台机器"裁剪的性能余量，被矩阵里第三台机器击穿**~~ **已修（阈值 4.0→3.0，并记下三台机器的实测）** | 测 | — | `7e3d07c` 的 CI 上 **Python 3.12 / windows 红**、其余六绿，而同样的代码在 `13414d1` 上七绿——即它是**侥幸通过**的。失败是 `test_the_columnar_path_costs_a_small_fraction_of_the_row_wise_path_per_row`：`3.92x` 对阈值 `4.0x`，**差 2%**。该阈值的推理本身很扎实（作者实测 `--cov` 下 4.88–5.23、36 个样本，并按 `sys.gettrace()` 区分了两种 regime），问题在**余量是按两台互相吻合的机器裁的**：本机 macOS arm64 实测 traced 下沿是 **4.70**（比作者写的 4.88 还低），Windows 是 **3.92**。**而这个比值的精度本来就不吃劲**——该文件自己记着：把 `to_rows()` 退回逐行推导式**仍得 6.7x**、高于 6.0 阈值，即**它抓不到它看上去在防的那个回归**；真正抓它的是旁边那条结构测试（每次 `to_rows()` 恰好一次转置）。所以阈值的职责只剩它真能做的那件：**发现列式路径不再本质上更便宜**。3.0 比任何机器产出过的最低值再低 23%，而退化到持平会落在 1.0 附近、仍然红。**未改动的 6.0 及其未知也写进了 docstring**：CI 永远在 `--cov` 下跑，所以矩阵里没有任何一台机器验过它，它的余量靠的正是那两台互相吻合的机器。已用把阈值顶到 99 证断言仍是活的（`assert 5.02 >= 99.0`）| S13 |
