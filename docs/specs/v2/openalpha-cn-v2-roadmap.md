...
| `V2-P5-020` | ~~组件隔离测试 + 前端覆盖率门~~ **本行原文三处与实测不符，已于 `V2-P5-014` 更正**：① "当前**无任何组件被隔离渲染**" —— `V2-P5-019` 交付时四个面板各自就有隔离渲染的 `*.test.tsx`，共用 `src/test/panelStateContract.tsx` 一套断言；② "`error` 态从未被渲染" —— 同一套件的 `renders a role="alert" carrying the message for {blocked,failed}` 正是为此写的；③ "`web/vite.config.ts` 无 coverage 键" —— 该文件自 2026-08-07 起就有 `coverage.thresholds`，且带"只升不降"的棘轮注释。**本行的实质内容因此已由 `019` 与 `014/015/016` 共同交付**：截至 `014`，隔离渲染的组件有 7 个（4 个原面板 + `DataHealthPanel`/`ShortlistIndexPanel`/`ShortlistDetailPanel`），覆盖率门为 92/84/92/93（实测 92.53/84.31/92.30/93.90），261 条测试。**棘轮已在 CI 生效**（`.github/workflows` 的 `web` job 直接跑 `pnpm test`，而 `coverage.enabled: true` + `thresholds` 使覆盖率不达标即退出非零 —— 写本行更正时先假设"只在本地生效"，读工作流后证伪，故不写进结论）。**本行可关** | 测 | 019 | `web/vite.config.ts` 的 `coverage.thresholds` | T14 |
