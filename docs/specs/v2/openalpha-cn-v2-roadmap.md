...
| **P1** | 面板数据平面 | 16 | 5–7 周 | 13–18 周 | 8 组数据集全部通过契约 + 未来数据 fail-closed |
| **P2** | **PIT 红队闸门** | 9 | 2 周 | 5 周 | **必过，否则不得进 P3** |
| **P3** | 因子层 | 15 | 5–6 周 | 13–15 周 | 首批因子出齐 raw/processed/neutralized 三档 |
| **P4** | 候选排序与模型基线 | 22 | 6–7 周 | 15–18 周 | 契约升版一次完成 + 预测先落库 |
| **P5** | 组合、验证与工作台 | 23 | 6–8 周 | 15–20 周 | 归因对账 + 多重检验 + 4 页可用 |
| | **合计** | **110** | **28–36 周** | **70–90 周** | |
...
| `V2-P1-009` | 数据集⑥`index_weight`（沪深300/中证500/中证1000） | 技 | 005 | 受积分约束，可能降级为自建静态股票池 | 同上 | S11 |
| `V2-P1-010` | 数据集⑦行业分类历史 | 技 | 005 | 中性化前提；受积分约束可能降级为市值中性化 | 同上 | S11 |
...
**风险**：全历史首次构建耗时以小时计 ⇒ 必须断点续传 + 本地缓存，不要做成一次性长任务。若 `V2-P0A-004` 探测出 `index_weight` 或行业分类不可用，009/010 降级并在数据目录中标注该限制。
...
| `V2-P3-004` | 中性化：行业 + 市值（横截面回归） | 技 | 003 | 依赖 `V2-P1-010`；不可用时降级为市值中性化并标注 | S19, D8 |
...
| `V2-P5-008` | gross/net 并列 + 置信区间 + 样本数 | 技 | 007 | — | S61, S62 |
| `V2-P5-009` | 分段报告（行业/市值/流动性/市场状态）+ 多市场状态 walk-forward + 基准对照 | 技 | 008 | — | S59, S60, S64 |
...
| `V2-P5-015` | 页面①数据体检 | 产 | 014, P1-012 | 面板覆盖/新鲜度/缺失修正/就绪状态 | S73, S48 |
| `V2-P5-016` | 页面②候选清单 + 个股详情 | 产 | 014, P4-005 | 排序/分数/置信度/排名变化/证据链/失效条件/可交易性告警 | S74, S75, S78 |
...
| 2 | Tushare 积分档位 | 由 `V2-P0A-004` 能力探测测定 | 若 `index_weight` 或行业分类历史不可用：`V2-P1-009`/`V2-P1-010` 降级为自建静态股票池 + 市值中性化，`V2-P3-004` 随之降级，PRD S11/S19 状态需更新，且每份因子报告须标注该限制 |
