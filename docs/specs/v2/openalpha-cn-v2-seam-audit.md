...
| F17 | `ModelProvider` 是 LLM-JSON 形状，无法表达面板 fit/predict；`ModelRegistry`/`ModelRetryPolicy`/`SQLiteModelUsageStore` 全部 LLM 专用 | `models/base.py:32-40`；`models/governance.py:15`、`:39`、`:73` | `V2-P4-011` |
| F18 | manifest 没有量化模型版本槽，且现有槽已被滥用 | `domain/run.py:56-57`；`runtime/engine.py:131-134` 把 agent id 塞进 `model_versions` 并写死 `"baseline/v1"`，`:135` 永远 `prompt_versions=()`，`:160-161` 镜像进 ledger | `V2-P4-010` |
| F19 | 路由只按证据家族交集选 agent ⇒ **无 evidence family 的 agent 永不被路由**，因子/模型型 agent 无路可走 | `runtime/router.py:12-23` 读 `payload["family"]`；`agents/baseline.py:48`、`:84`、`:120` 声明家族集 | `V2-P4-008` |
| F20 | `AgentContext` 只带单标的证据元组，没有特征/面板句柄 | `agents/base.py:12-20` | `V2-P4-009` |
...
| F25 | 已声明但全库无人使用的扩展点 —— 可直接用于面板/因子查询暴露给 agent | `tools/base.py:54-62 ResearchTool`；`tools/` 在 `src/` 内无 importer | `V2-P4-009` |
...
| F31 | 同名操作语义分叉：CLI `evidence build` **不落库**，SDK `build_file_evidence` 落库 | `cli.py:91-121` 只打印；`sdk.py:92` 追加 | `V2-P5-013` |
...
| F33 | 同能力默认值分叉 | `PortfolioApiRequest.limits` 默认 `PortfolioLimits()` vs SDK `None`；`ScreeningApiRequest.criteria` REST 有默认、SDK 必填 | `V2-P5-013` |
...
| # | Finding | 证据 | 关闭 issue |
|---|---|---|---|
| F62 | 只有 1 个面板用判别式 `state` 联合，其余 3 个用 ad-hoc `loading`/`error` 布尔；`degraded`/`stale`/`blocked` 在 `web/src` 中**根本不存在** | 好模式：`EvidencePanel.tsx:9`（`"idle"\|"loading"\|"ready"\|"empty"\|"error"`，分支于 `:66`/`:73`/`:74`/`:77`/`:82`）；其余：`DecisionPanel.tsx:6-7`、`ReplayPanel.tsx:7-8`、`AttributionPanel.tsx:8-9` | `V2-P5-019` |
...
| F98 | **无任何调度原语**（grep `cron\|scheduler\|apscheduler\|celery` 全库零命中）。`daily` 模式需要：带 next-fire-time 与 lease/lock 列的持久作业表、按交易日的幂等键、错过窗口的 catch-up/skip 政策、市场日历依赖、崩溃恢复 —— **全部不存在，且没有迁移机制去加这些表** | 唯一异步是 `BackgroundTasks` + `runtime/batch.py:106` ThreadPoolExecutor，进程内、重启即失（除 `storage/batch.py:120-141` 的一次性 `recover_interrupted` 扫描） | `V2-P5-010` |
| F99 | **`src/` 内零 logging 配置** ⇒ 无日志的调度器不可运维 | grep `logging` 无配置 | `V2-P0B-007` |
| F100 | 请求体大小检查**只看 `content-length` 头** ⇒ chunked 请求完全绕过，且流式过程中从不计数 | `api/app.py:180`、`:181`（`if raw_length is not None`）、`:191-196` | `V2-P5-012` |
| F101 | CORS 硬编码只允许 `GET`/`POST` ⇒ v2 REST 面若用 `PUT`/`DELETE`/`PATCH` 会在 CORS 层被挡；且全库无任何认证授权 | `api/app.py:281-287` | `V2-P5-011` |
| F102 | 缺 `Strict-Transport-Security`、`cross-origin-embedder-policy`、`cross-origin-resource-policy`；header 是 append 而非 replace；`--no-server-header` 只在 `Dockerfile:47` 传，`cli.py:180` 不传 ⇒ `openalpha serve` 泄漏 `server:` 头 | `api/app.py:154-168`、`:199-204` | `V2-P5-012` |
