...
测试要求 300 个案例全部成功，每个案例的两遍结果一致（第二遍用空的存储从头重算，不读第一遍存下的状态），
每个案例的归因条款加上显式残差 `unexplained_return` 等于净主动收益（`ValidationResult` 构造时校验）。
它不统计前视：证据在决策时刻尚不可见的案例在加载语料时就被整体拒绝，所以任何能加载的语料，回放报告里的
前视违规计数都恒为 0；测试检查的是这条拒绝本身——把一个案例的 `as_of` 挪到其证据可见之前一分钟，同一份
语料就无法加载。
