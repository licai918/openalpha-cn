...
KNOWN_NEUTRALIZATION_LIMITATIONS: Final[tuple[NeutralizationLimitation, ...]] = (
    ...
    NeutralizationLimitation(
        code="the_two_foreign_inputs_are_read_whole_partition_so_a_mid_year_as_of_is_refused",
        detail=(
            "THIS IS THE ISSUE'S SHARPEST OPERATIONAL CONSTRAINT AND IT IS NOT A THINNING OF THE "
            "CROSS SECTION -- IT IS A REFUSAL OF THE WHOLE BUILD. Both foreign inputs are read "
            "through panel_ingest's loaders, which take PanelStore.read_if_ready, and that call "
            "decides not_yet_knowable on a partition's MAX available_time. So a partition is "
            "unreadable at every as_of that precedes its newest row, whatever the as_of's own "
            "session had. Measured on the generated fixture panel at as_of 2026-01-12T04:00Z, "
            "which is noon Asia/Shanghai on the sixth of ten sessions: (1) daily_basic blocks, "
            "because the year's last session becomes available 2026-01-16T08:30Z -- so no "
            "neutralisation can be built at ANY as_of inside the year its price partition covers, "
            "and a whole-year backfill is a year-end operation; (2) index_member_all blocks "
            "whenever one stored assignment opens later, measured with an interval opening "
            "2026-01-14 whose partition then has a max available_time of 2026-01-13T16:00Z, and "
            "on the real corpus that shape is the annual constituent review moving hundreds of "
            "names at once (613 assignments start 2021-07-30, 255 on 2022-07-29). This is "
            "roadmap section 11's problem, which V2-P3-002 solved for the FACTOR's own inputs by "
            "adding PanelStore.read_visible_at, and which is deliberately NOT solved here: taking "
            "the row-filtered door for a foreign dataset would mean answering, for daily_basic "
            "and index_member_all, the question tests/unit/panel/test_visible_read_callers.py "
            "makes every new caller answer -- whether a withheld row can be told from an absent "
            "one -- and for index_member_all it cannot, which is exactly why "
            "SecurityIndustryHistory.answerable_through exists. The levers a caller has today are "
