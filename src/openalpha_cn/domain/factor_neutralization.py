...
KNOWN_NEUTRALIZATION_LIMITATIONS: Final[tuple[NeutralizationLimitation, ...]] = (
    ...
    NeutralizationLimitation(
        code="a_stored_membership_year_left_unread_refuses_the_day_rather_than_answering_it",
        detail=(
            "THIS ENTRY HAS BEEN RETRACTED TWICE AND BOTH RETRACTIONS ARE PART OF IT; THE RENAME "
            "IS THE SECOND ONE. It began as the_two_foreign_inputs_are_read_whole_partition_so_a_"
            "mid_year_as_of_is_refused and said no neutralisation could be built at ANY as_of "
            "inside the year its price partition covers -- that a whole-year backfill was a "
            "year-end operation and that a residual for any trading day of year Y carried an "
            "available_time after Y's last session. V2-P4-026 retracted the daily_basic half: "
            "load_daily_valuations takes panel_ingest._read_visible_price_session, which scans "
            "the year partition with WHERE available_time <= as_of AND revision_time <= as_of "
            "under a trade_date filter, so "
            "a session that published before as_of is answerable from inside its own year and a "
            "session that did not is refused BY NAME rather than answered with an empty cross "
            "section. It was renamed to the_industry_input_is_read_whole_partition_so_a_mid_year_"
            "as_of_can_be_refused and its remaining half read: index_member_all is read through "
            "load_industry_histories, which takes PanelStore.read_if_ready, and that call decides "
            "not_yet_knowable on a partition's MAX available_time, so a membership year is "
            "unreadable at every as_of that precedes its newest assignment. V2-P4-028 RETRACTED "
            "THAT HALF TOO. load_industry_market_cap_cross_section now reads through "
            "panel_ingest.load_industry_cross_section (V2-P4-027's door), which takes the DAY as "
            "an argument: a membership event later than the day being priced cannot change who "
            "covered that day, so it no longer refuses the read. Measured on the same fixture "
            "the old entry quoted -- the generated panel plus an interval opening 2026-01-14, "
            "whose 2026 partition's max available_time is 2026-01-13T16:00Z -- the cross section "
            "assembled on 3 of the window's 10 sessions before this change and on 10 of 10 after "
            "it, and the shape that produced the shortfall is exactly the real corpus's annual "
            "constituent review (613 assignments start 2021-07-30, 255 on 2022-07-29). "
            "WHAT REMAINS, AND IT IS NOW THE WHOLE OF THIS ENTRY, IS THE COST THE OLD ENTRY "
            "MENTIONED IN PASSING AS A LEVER: membership_years is how a caller narrows the read, "
            "and an assignment's close is filed as its own row in its own year -- so a STORED "
            "membership year at or before the day being priced that the caller did not name can "
            "hold the close of an interval this cross section would otherwise report as current. "
            "That is refused BY NAME, naming the year to add, rather than answered from an "
            "interval with no end in it. It is a refusal of the whole build and not a thinning "
