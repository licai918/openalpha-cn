...
KNOWN_SUSPENSION_LIMITATIONS: Final[tuple[SuspensionLimitation, ...]] = (
    ...
    SuspensionLimitation(
        ...
        detail=(
            ...
            "headroom is the part worth watching, and an earlier version of this entry stated "
            "it at the wrong resolution: the annual figures (6,867 rows on 2024-06-28, 7,216 on "
            "2025-08-01, 7,734 on 2026-08-10 -- +349 then +518) put the crossing 'inside a "
            "year', which is not a useful thing to say about a 66-row margin. Re-measured "
            "session by session on 2026-08-10, the last fourteen published cross sections run "
            "7,705 / 7,706 / 7,707 / 7,709 / 7,710 / 7,712 / 7,715 / 7,716 / 7,721 / 7,723 / "
            "7,725 / 7,727 / 7,733 / 7,734 (2026-07-22..2026-08-10), i.e. +2.231 rows per "
            "session: 66 / 2.231 = 29.6 sessions, about six weeks. daily's equivalent margin is "
            "465 of 6,000, at a tenth of the rate. When it goes, the fetch still refuses rather "
            "than storing a short session, and the stk_limit descriptor now declares a measured "
            "page_size of 4,000 so the refusal triggers a limit/offset re-fetch instead of "
            "ending the build: measured on the live endpoint, offset 0 and offset 4,000 "
            "concatenate into a sequence element-for-element identical to the un-paged "
            "response, so a paged partition carries the same content_digest and the same store "
            "content_hash a one-shot partition would have. Splitting by subject is NOT the way "
            "out and was wrongly named as one: stock_basic carries 5,878 codes against this "
            "cross section's 7,733, and the 2,194 it does not carry are exactly the funds and B "
            "shares this table's stk_limit_covers_funds_and_b_shares_as_well_as_stocks entry "
            "names, so sharding by the stored universe would drop them silently. suspend_d is "
            "nowhere near its own cap (1,466 rows on its worst measured session, 2015-07-09) "
            "and declares no page_size for that reason -- paging is opt-in behind a "
            "measurement, because on this API it is measurably unsound for namechange."
        ),
    ),
)
