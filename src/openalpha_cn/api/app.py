...
from openalpha_cn.research_result_io import (
    parse_research_result as _parse_research_result,
)
from openalpha_cn.research_result_io import research_refusal_detail
...
def _research_refusal(error: Exception, *, index: int | None) -> HTTPException:
    """Envelope `research_refusal_detail`'s body as this service's `422`.

    The body itself moved to `openalpha_cn.research_result_io` in `V2-P5-047`, because
    `openalpha validation record` and `openalpha report create` refuse the identical input and
    must do it in the identical words -- and `cli.py` importing this module to get them would
    have put FastAPI behind every `openalpha version`. What is left here is the only part that
    is actually about HTTP: the status code.
    """
    return HTTPException(status_code=422, detail=research_refusal_detail(error, index=index))
