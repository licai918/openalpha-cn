...
KNOWN_STORAGE_LIMITATIONS: Final[tuple[StorageLimitation, ...]] = (
    ...
    StorageLimitation(
        ...
        detail=(
            ...
            "and every src/ reader that answers with what it reads takes one of them: the "
            "whole-partition read_if_ready(), which every caller spells assessed(...).read"
            "(...), and the row-filtered read_visible_at(); the one allowlisted exception, "
            "panel_ingest.carry_stored_rows_forward, answers with nothing it reads. Both are "
            "opt-in rather than structural -- pinned by "
            ...
            "availability clock. The four statement datasets are not such datasets -- measured "
            "on one income row announced 2024-04-20 and re-announced 2024-11-10, an assessed "
            "read at 2024-08-30 hands back ('000001.SZ', 999.0, '2024-11-10'), the version "
            "nobody had read that day, while read_visible_at returns no rows and counts one "
            "withheld. No src/ reader takes this door to a dataset that revises "
            "(tests/unit/panel/test_whole_partition_doors_never_hold_a_revision.py), and a "
            "library caller reading one should call read_visible_at() instead. "
