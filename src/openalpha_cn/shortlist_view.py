...
KNOWN_SHORTLIST_VIEW_LIMITATIONS: Final[tuple[ShortlistViewLimitation, ...]] = (
    ...
    ShortlistViewLimitation(
        ...
        detail=(
            ...
            "`SignalFrame` its research run produced, and no store this module reads holds that "
            "frame -- `SQLiteRunRepository` holds `RunManifest` and `DecisionLedger`, "
            "`ResearchReport` holds a `signal_id` rather than the frame, and the aggregate frame "
            "a run produces is never persisted at all: only its ID is, and only when it is "
            "not an abstention, in `decisions.signal_ids` (`runtime/engine.py:120-122`, "
            "`storage/migrations.py:895-900`). The one place a whole `SignalFrame` is stored is "
            "the recovery plane's node checkpoints (`storage/migrations.py:544-547`), which are "
            "keyed by run and node and hold a node's own output rather than a run's conclusion. "
            "So a caller supplies the evidence plane's "
    ...
    ShortlistViewLimitation(
        ...
        detail=(
            ...
            "**run**, and not the `SignalFrame` beside it: no store this face reads holds the "
            "frame a run produced (see "
            "`the_evidence_plane_is_supplied_rather_than_run_by_this_module`), so there is "
            "nothing to resolve one against, and a caller who holds a real `run_manifest_id` can "
            "still file an invented conclusion under it. The frame is checked for internal "
            "consistency -- `shortlist_evidence` verifies its `signal_id` against its own "
            "content and "
