...
**上面这句话在 `V2-P4-061` 之前并不成立。** K 线与涨跌停带走的是「整分区」那道闸：它拿
分区里**最新的一行**的可知时刻去判 `not_yet_knowable`，而一个分区就是一整年。于是面板只
要往前多走一个交易日，同一年里**更早**的每一个横截面就都读不出来了——
`daily cannot be read at …: ['not_yet_knowable']`——能撮合的只剩最新那一个。两天的榜没法
对比，昨天的榜没法重跑，已经发出去的榜事后没法复核。现在这两个读走的是估值面板早就在走的
那条「按 as_of 看得见什么」的单会话读。
