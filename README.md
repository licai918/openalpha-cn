...
| 模型治理 | 模型能力注册、408/429/5xx 分类重试；Token/尝试次数/估算成本账本要接入自带用量追踪的 Provider 才会写入，出厂路径不会自动记账 |
...
- **模型治理与核算**：按模型能力选择兼容端点，对 408/429/5xx 分类重试；Token、尝试次数与按用户单价估算的成本有持久账本，但要接入自带用量追踪的 Provider 才会写入，出厂路径不会自动记账。
