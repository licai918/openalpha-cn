...
本轮已经把与高星项目对账后最关键的增强能力落到源码和测试中：

| 研究脑区 | 已落地能力 |
|---|---|
| 数据与证据 | 链邻合同型 Provider（出厂路径仅 `doctor` 调用）；Bearer 认证、客户端限流、PIT/修订语义与显式错误分类 |
| 规模化研判 | SQLite 持久批量队列；1–8 并发、逐项进度、协作式取消与重试，进程重启后被中断的项重新排队 |
| 模型治理 | 能力注册表与能力元数据只是模型客户端库里的定义，没有代码据此选择端点；408、429、500、502、503、504 六个状态码有界重试要在代码中接入模型 Provider 才会用上，出厂路径不调用模型；Token/尝试次数/估算成本账本要在构造 Provider 时传入 `usage_store` 才会写入，出厂路径不会自动记账 |
...
- **四时钟与可见性**：分别记录事件发生、首次可知、系统入库和数据修订时间；可见性要求首次可知与修订时间都不晚于决策时刻，修订晚于决策时刻的记录在决策时刻不可见；决策前修订过的记录带 `revised_after_initial_availability` 标记、被风险门降级。剩余缺口：同日只差 `update_flag` 的财报更正对，时钟分不开；面板平面只存了更正后那一版的财报，在 `[ann_date, f_ann_date)` 内该期不可见；证据平面第一次入库就是修订版的，修订生效前也没有这条证据——后两处是缺失，不是前视。
...
- **模型治理边界**：模型客户端库定义了能力注册表与能力元数据，但没有代码据此选择端点，出厂路径与接入 Provider 之后都一样；在代码中接入模型 Provider 后对 408、429、500、502、503、504 六个状态码有界重试（其余 5xx 不重试），出厂路径不调用模型；Token、尝试次数与按用户单价估算的成本有持久账本，但要在构造 Provider 时传入 `usage_store` 才会写入，出厂路径不会自动记账。
...
- **同一研究内核**：实时研究与历史回放共用 `run_cycle`，避免线上逻辑和回放逻辑各走一套；结果验证、多日组合回测与 `model daily-run` 走各自的路径。
- **确定性回放**：仓库在 `tests/fixtures/replay/` 附了一份 60 个交易日、300 个合成事件的冻结语料，
  `openalpha replay run` 要调用方把它的路径传进来（镜像只拷 `src/`，装好的包与容器里没有这份语料）；每个事件跑两遍，第二遍用空的存储从头重算，两遍结果不一致即记为失败。证据在该事件决策时刻尚不可见（前视）的语料，在加载时就被整体拒绝，不会进入回放。
...
- **批量任务中心**：任务、并发上限、逐项进度、取消和重试均持久化，进程重启后被中断的项重新排队。
- **研究产品接口**：提供结构化筛选、观察池和内容寻址报告中心。

### 🛡️ 可复现的工程底座

- **完整复现清单**：`EvidenceSnapshot` 内容寻址；`RunManifest` 记录代码、配置、Provider、模型、随机种子和环境版本；Prompt 版本字段已预留、目前恒为空，仓库内的 prompt 由 `code_commit` 钉住。
...
`MarketBar` 和限制条件，逐项经过 T+1、整手、停牌、涨跌停、费用与敞口检查后产生
不可变 `PortfolioTransition`：通过的是成交，未通过的是写明原因的拒单，二者都写进账本；它不会根据研究结论自动下单，也不连接实盘券商。
