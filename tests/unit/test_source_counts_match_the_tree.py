...
def _call_sites(attribute: str) -> int:
    total = 0
...
FACTS: Final[dict[str, Callable[[], int]]] = {
    ...
    "gated_door_call_sites": lambda: _call_sites("read_if_ready") + _call_sites("read_visible_at"),
}
...
CLAIMS: Final[tuple[CountedClaim, ...]] = (
    ...
    CountedClaim(
        SRC / "panel" / "store.py",
        "so their {gated_door_call_sites} callers see no change at all",
    ),
    CountedClaim(
        SRC / "panel" / "store.py",
...
"""Every counted sentence this file holds. A new one is two lines: the template, and the fact."""
