...
def test_the_growth_family_reads_a_filing_and_no_price_at_all() -> None:
    ...
    **These three are not the whole of that partition, and the equality names the rest rather than
    weakening to a containment.** `V2-P3-010`'s quality family reads filings and no price either,
    and it ships in the same build, so "period-only" is seven definitions and not three. The
    exclusivity this test still buys is that *nothing else* is on the period axis alone: the three
    named here plus the four named there, and every remaining factor declaring a session reach.
