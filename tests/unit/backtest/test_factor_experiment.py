...
def test_every_shipped_factor_produces_all_three_tiers_under_an_identity_of_its_own() -> None:
    ...
    **What this proves and what it does not.** It drives the *report* half over every declared
    factor: the four studies run for real on each definition, `build_factor_experiment` binds
    them, the artifact carries `FACTOR_TIER_ORDER` once, and those identities are pairwise
    distinct --
